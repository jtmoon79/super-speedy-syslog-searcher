...
choose_install_dir() {
    ...
    for dir in "${HOME}/.cargo/bin" "${HOME}/bin" "/usr/local/bin"; do
        [ -d "${dir}" ] || continue
        [ -w "${dir}" ] || continue
        echo "${dir}"
        return 0
...
main() {
    ...
    zip_path="${WORKDIR}/s4.zip"
    url="https://github.com/jtmoon79/super-speedy-syslog-searcher/releases/download/${VER}/s4_${target_triple}_v${VER}.zip"
