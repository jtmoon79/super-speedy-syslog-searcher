# CHANGELOG<!-- omit in toc -->

<!--
Helper script `tools/changelog-link-gen.sh` can generate the addendum of markdown links for this CHANGELOG.md.
-->

Manual changelog for [super speedy syslog searcher](https://github.com/jtmoon79/super-speedy-syslog-searcher).

---

<!-- TODO per release: update TOC -->
<!-- Table of Contents updated by "Markdown All In One" extension for Visual Studio Code -->

- [Unreleased](#unreleased)
- [0.10.83](#01083)
  - [New](#new)
  - [Changed](#changed)
  - [Fixed](#fixed)
- [0.9.82](#0982)
  - [New](#new-1)
  - [Changed](#changed-1)
  - [Fixed](#fixed-1)
- [0.9.81](#0981)
  - [New](#new-2)
  - [Fixes](#fixes)
  - [Changes](#changes)
- [0.8.80](#0880)
  - [New](#new-3)
  - [Changes](#changes-1)
  - [Fixes](#fixes-1)
- [0.7.79](#0779)
  - [New](#new-4)
  - [Changes](#changes-2)
  - [Fixes](#fixes-2)
- [0.7.78](#0778)
  - [Changes](#changes-3)
  - [Fixes](#fixes-3)
- [0.7.77](#0777)
  - [New](#new-5)
  - [Changes](#changes-4)
- [0.7.76](#0776)
  - [Changes](#changes-5)
  - [Fixes](#fixes-4)
- [0.7.75](#0775)
  - [Changes](#changes-6)
  - [Fixes](#fixes-5)
- [0.7.74](#0774)
  - [New](#new-6)
  - [Changes](#changes-7)
  - [Fixes](#fixes-6)
- [0.7.73](#0773)
  - [New](#new-7)
  - [Changes](#changes-8)
  - [Fixes](#fixes-7)
- [0.7.72](#0772)
  - [New](#new-8)
  - [Changes](#changes-9)
  - [Fixes](#fixes-8)
- [0.6.71](#0671)
  - [New](#new-9)
  - [Changes](#changes-10)
  - [Fixes](#fixes-9)
- [0.6.70](#0670)
  - [New](#new-10)
  - [Changes](#changes-11)
- [0.6.69](#0669)
  - [New](#new-11)
  - [Changes](#changes-12)
- [0.6.68](#0668)
  - [New](#new-12)
  - [Changes](#changes-13)
- [0.6.67](#0667)
  - [New](#new-13)
  - [Changes](#changes-14)
  - [Fixes](#fixes-10)
- [0.6.66](#0666)
  - [Changes](#changes-15)
  - [Fixes](#fixes-11)
- [0.6.65](#0665)
  - [New](#new-14)
  - [Changes](#changes-16)
- [0.6.64](#0664)
  - [New](#new-15)
  - [Changes](#changes-17)
  - [Fixes](#fixes-12)
- [0.6.63](#0663)
  - [New](#new-16)
  - [Changes](#changes-18)
  - [Fixes](#fixes-13)
- [0.6.62](#0662)
  - [Fixes](#fixes-14)
- [0.6.61](#0661)
  - [New](#new-17)
  - [Changes](#changes-19)
  - [Fixes](#fixes-15)
- [0.6.60](#0660)
  - [New](#new-18)
- [0.5.59](#0559)
  - [Changes](#changes-20)
  - [Fixes](#fixes-16)
- [0.5.58](#0558)
  - [New](#new-19)
  - [Changes](#changes-21)
- [0.4.57](#0457)
  - [Changes](#changes-22)
  - [Fixes](#fixes-17)
- [0.4.56](#0456)
  - [New](#new-20)
  - [Changes](#changes-23)
  - [Fixes](#fixes-18)
- [0.3.55](#0355)
  - [New](#new-21)
  - [Changes](#changes-24)
  - [Fixes](#fixes-19)
- [0.3.54](#0354)
  - [New](#new-22)
  - [Fixes](#fixes-20)
- [0.3.53](#0353)
  - [New](#new-23)
  - [Changes](#changes-25)
- [0.2.52](#0252)
  - [New](#new-24)
- [0.2.51](#0251)
  - [New](#new-25)
- [0.2.50](#0250)
  - [New](#new-26)
  - [Changes](#changes-26)
  - [Fixes](#fixes-21)
- [0.2.49](#0249)
  - [Changes](#changes-27)
  - [Fixes](#fixes-22)
- [0.2.48](#0248)
  - [New](#new-27)
  - [Changes](#changes-28)
  - [Fixes](#fixes-23)
- [0.2.47](#0247)
- [0.2.46](#0246)
  - [New](#new-28)
  - [Changes](#changes-29)
  - [Fixes](#fixes-24)
- [0.1.45](#0145)
  - [New](#new-29)
  - [Changes](#changes-30)
- [0.1.44](#0144)
  - [New](#new-30)
  - [Changes](#changes-31)
  - [Fixes](#fixes-25)
- [0.1.43](#0143)
  - [New](#new-31)
  - [Changes](#changes-32)
- [0.1.42](#0142)
  - [Changes](#changes-33)
- [0.1.41](#0141)
  - [Changes](#changes-34)
  - [Fixes](#fixes-26)
- [0.1.40](#0140)
  - [New](#new-32)
  - [Changes](#changes-35)
- [0.1.39](#0139)
  - [Changes](#changes-36)
- [0.1.38](#0138)
  - [New](#new-33)
  - [Changes](#changes-37)
- [0.0.37](#0037)
  - [New](#new-34)
  - [Changes](#changes-38)
- [0.0.36](#0036)
  - [New](#new-35)
  - [Changes](#changes-39)
  - [Fixes](#fixes-27)
- [0.0.35](#0035)
  - [New](#new-36)
  - [Fixes](#fixes-28)
- [0.0.34](#0034)
  - [New](#new-37)
  - [Fixes](#fixes-29)
- [0.0.33](#0033)
  - [New](#new-38)
- [0.0.32](#0032)
  - [New](#new-39)
  - [Fixes](#fixes-30)
- [0.0.31](#0031)
  - [New](#new-40)
- [0.0.30](#0030)
  - [New](#new-41)
  - [Changes](#changes-40)
- [0.0.29](#0029)
  - [Changes](#changes-41)
- [0.0.28](#0028)
  - [New](#new-42)
  - [Changes](#changes-42)
  - [Fixes](#fixes-31)
- [0.0.27](#0027)
  - [New](#new-43)
  - [Changes](#changes-43)
- [0.0.26](#0026)
  - [New](#new-44)
  - [Changes](#changes-44)
  - [Fixes](#fixes-32)
- [0.0.25](#0025)
  - [New](#new-45)
  - [Changes](#changes-45)
  - [Fixes](#fixes-33)
- [0.0.24](#0024)
  - [New](#new-46)
  - [Changes](#changes-46)
- [0.0.23](#0023)
  - [New](#new-47)
  - [Changes](#changes-47)
  - [Fixes](#fixes-34)
- [0.0.22](#0022)
  - [New](#new-48)
  - [Changes](#changes-48)
  - [Fixes](#fixes-35)
- [0.0.21](#0021)
  - [New](#new-49)
  - [Fixes](#fixes-36)

---

<!--
TODO per release:

1. Developers must manually create the sections. Do not create empty sections.
2. Developers must manually prefix categories (listed below).

Run `tools/changelog-link-gen.sh` after done editing the sections.

---

Sections:

  ### New

  ### Fixes

  ### Changes

Categories:

  (LIB) - changes to the library source
  (BIN) - changes to the binary source (bin.rs, CLI options, etc.)
  (DEBUG) - changes to the either source only affecting debug builds
  (BUILD) - changes to the build (i.e. Cargo.toml)
  (DOCS) - changes to docstrings ("rustdocs"), docs.rs stuff, READMEs
  (CI) - changes to github workflows, codecov
  (LOGS) - chnages under `logs/`
  (RELEASE) - release-specific
  (TEST) - changes only to `src/tests`
  (TOOLS) - changes to scripts under `tools/`
  (PROJECT) - changes to READMEs, CHANGELOG, other non-source tweaks

Helpful `git log` command for generating changelog entries:

    R1=0.7.75
    R2=0.7.76
    git log --pretty=format:'- %s %Cred([%H])%Creset%C(yellow)%d%Creset' --abbrev-commit ${R1}...${R2} | sort
-->

## Unreleased

[unreleased diff]

[unreleased diff]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/latest...main

<!-- TODO per release: Add Section(s) -->

---

## 0.10.83

_Released 2026-07-31_

_MSRV 1.88.0_

[0.9.82..0.10.83]

- FileHandleManager allows reading more files than the OS may allow per-process [Issue #270]
- streaming XZ [Issue #12]
- streaming TAR files [Issue #13]
- CLI options `-a` and `-b` improvements:
  - allow clock overide "*!01:02:03*" in relative offsets
  - accept timezone
  - allow datetime separator `T` or ` `
  - renamed long options to `--after` and `--before`
- CLI option `--blocksz` removed
- embed Windows exe info in Windows binary
- add regex #45 for Darwin OS `wifi.log`
- updated 78 dependencies

### New

- (AI) add draft CLAUDE.md ([37a30267])
- (AI) CLAUDE.md dev tips: release_Opt0, no rustfmt ([9648beeb])
- (BENCH) add bench_ranges.rs ([bdf4b239])
- (BIN) -a -b allow clock overide "!01:02:03" in relative offsets ([4bad64e8])
- (BIN) alloc_tracker allow env var S4_ALLOC_TRACKER_LINK ([854bffd0])
- (BIN) allow -a -b accept timezone ([6ab2c273])
- (BIN) allow datetime separator 'T' or ' ' for -a -b ([3c679fda])
- (BIN) CLI rename --dt-before --dt-after to --before --after ([a3141c2f])
- (BIN) embed Windows version_info ([4e72f534])
- (BIN) --summary print thread stack size ([3ab52452])
- (BIN) --version CPU features, rm dep opt-level and impl manually ([2dce79d5])
- (BUILD) build.rs allow unset CARGO_CFG_TARGET_FEATURE ([81cb3630])
- (CI) add .github/copilot-instructions.md ([afe78eee])
- (DOC) README badges for github downloads ([b6f5e925])
- (LIB) rm --blocksz CLI, keep env S4_BLOCKSZ ([3acad09e])
- (LIB) add regex 45 ([77c38995])
- (LIB) add summary stats: longest line, longest sysline ([361a6549])
- (LIB) BlockReader does streaming XZ [Issue #12] ([ac39b5e9])
- (LIB) stream tar files using extra thread [Issue #13] ([875587e7])
- (LIB) FileHandleManager to allow many open files [Issue #270] ([2b18f59b])
- (LIB) FileHandleManager allow FileHandleUnamanged, support EvtxReader, JournalReader, PyEventReader [Issue #270] ([c4398e1e])
- (LIB) FileHandleManager adjust open_max to open error 'too many open file handles' ([75c16e41])
- (LIB) FileHandleManager fix pending reservations counting ([678196ee])
- (LIB) FileHandleManager fix clippy error, debug_panic ([ae27b1f9])
- (LIB) FileHandleManager fix summary open_max_default ([32c361f4])
- (LIB) FileHandleManager fix warning on Windows 'unused variable' ([e73e2c18])
- (LIB) FileHandleManager refactor to reserve unmanaged handles [Issue #270] ([7c51ee62])
- (LIB) FileHandleManager type OpenCountType OpenMaxCountType ([7f8118bc])
- (LIB) FileHandleManager summary_stat managed_open_count_hi unmanaged_count_hi count_hi ([5b9948b4])
- (LIB) FileHandleManager FILE_HANDLE_OPEN_MAX_DEFAULT depend on OS Family ([eed4e97d])
- (LIB) FileHandleManager PyEventReader create FileHandleUnamanged for Python process ([816d4f14])
- (LIB) FileHandleManager use FILE_HANDLE_MANAGER for all temp files [Issue #270] ([4f2ea4e9])
- (LOGS) add dtf5-6b.\*.log.\* compression variations ([00a92328])
- (TEST) FileHandleManager filehandlemanager_tests.rs fix check open_max_default ([a58f2409])
- (TEST) test_unescape_str all BACKSLASH_ESCAPE ([694ce3d3])
- (TOOLS) add stack-sizes.sh ([ba2d4f40])
- (TOOLS) add tools/xmllint.sh ([b76c930d])
- (TOOLS) performance-plot.sh add plot blocksz x time ([18098f47])
- (SUBPROJECT) lzma_rs add /tests, adjust test paths, rm junk files ([6c29b163])
- (SUBPROJECT) lzma_rs all features enabled (comment conditional), rm from Cargo.toml [Issue #12] ([21f11792])

### Changed

- (BIN) s4.rs alloc Select struct once, revise over time; prior alloc'd on every message wait ([3042b50a])
- (BIN) stack_size updates ([503a5ae0])
- (BUILD) build.rs better handle failed 'git reg-parse' command ([97116211])
- (CI) build steps timeout-minutes: 35 ([67c5a8d4])
- (CI) rename workflow to 'Build, Test' ([4772bae1])
- (CI) Run s4 S4_FILE_HANDLE_OPEN_MAX=12 ([a1d2eed8])
- (CI) Run s4: S4_FILE_HANDLE_OPEN_MAX=8 ([5029d051])
- (CI) run s4 ulimit=15 ([12217e26])
- (CI) run s4 ulimit=20 ([f0efa728])
- (CI) Run s4 with head, tail ([9a80ee59])
- (CI) simplify s4 runs, add run pipe grep ([9c23d8a4])
- (CI) use release profile release_Opt0 ([0a1e78ce])
- (DOC) README reorganize sections, perf table highlight ([85a86363])
- (DOC) filehandlemanager.rs link [Issue #270] ([31a77692])
- (LIB) blockreader.rs re-use current thread name for XZ Decoder thread ([3514cb99])
- (LIB) blockreader.rs streaming files reject offset requests backwards ([be714133])
- (LIB) `DT_PATTERN_MAX=2`, rm redunant regex ([a178f0c9])
- (LIB) dependabot: bump quinn-proto from 0.11.14 to 0.11.16 ([0e55b5df])
- (LIB) dependabot: bump trybuild from 1.0.116 to 1.0.117 ([88bdfbb0])
- (LIB) bump anyhow 1.0.103 ([f810a846])
- (LIB) bump anyhow 1.0.104 ([2d14cc1f])
- (LIB) bump arrayvec 0.7.8 ([b8b69662])
- (LIB) bump bitflags 2.13.1 ([b1d4fee0])
- (LIB) bump bstr 1.13.0 ([40ad3c39])
- (LIB) bump bstr, limit features ([eb063b0a])
- (LIB) bump bytemuck 1.25.2 ([1e390bc5])
- (LIB) bump camino 1.2.5 ([f19bab1a])
- (LIB) bump cc 1.4.0 ([5f588677])
- (LIB) bump cfg_aliases 0.2.2 ([0855ed09])
- (LIB) bump clap 4.6.4 ([f3fc4fc3])
- (LIB) bump crossbeam-* ([aa215046])
- (LIB) bump crossbeam-channel 0.5.16, memchr 2.8.3, regex 1.13.0, list-features 1.0.0 ([46bf32e3])
- (LIB) bump dashmap 6.2.1 ([3ce67599])
- (LIB) bump displaydoc 0.2.6 ([ae5edd9d])
- (LIB) bump either 1.17.0 ([4371f19f])
- (LIB) bump fastrand 2.5.0 ([1fb92f06])
- (LIB) bump getrandom 0.4 ([c219df0e])
- (LIB) bump glob 0.3.4 ([ef547255])
- (LIB) bump goblin 0.10.7 ([df2d3132])
- (LIB) bump h2 0.4.15 ([7cae5f65])
- (LIB) bump http 1.4.2 ([68e712a2])
- (LIB) bump hyper 1.11.0 ([8ebef82d])
- (LIB) bump inferno 0.12.8 ([ab724b88])
- (LIB) bump jobserver 0.1.35 ([c8084707])
- (LIB) bump libc 0.2.189 ([a0a2af25])
- (LIB) bump libredox 0.1.18 ([e8d041fd])
- (LIB) bump lru 0.18.1 ([fe5f8757])
- (LIB) bump lzxd 0.2.7 ([08005574])
- (LIB) bump macro-string 0.3.0 ([9d5f0ba2])
- (LIB) bump memmap2 0.9.11 ([1c02a824])
- (LIB) bump numtoa 1.0.1 ([556ed6c9])
- (LIB) bump portable-atomic 1.14.0 ([6039af39])
- (LIB) bump quinn 0.11.11 ([6ca9634b])
- (LIB) bump quote 1.0.47 ([59890260])
- (LIB) bump rancor 0.1.2 ([6ba2a65b])
- (LIB) bump rand 0.10.2 ([d52775ad])
- (LIB) bump rand rend ([fa3e01aa])
- (LIB) bump ref-cast 1.0.26 ([ba3402f1])
- (LIB) bump regex 1.13.1 ([1186b43f])
- (LIB) bump rkyv 0.8.17 ([0d01efed])
- (LIB) bump rustc-hash 2.1.3 ([2c397230])
- (LIB) bump rustls 0.23.42 ([45e88462])
- (LIB) bump rustls ([f03803b0])
- (LIB) bump rustversion 1.0.23 ([21904d9b])
- (LIB) bump sha1 0.10.7 ([88d1f864])
- (LIB) bump simd-adler2 0.3.10 ([8fe8c8a2])
- (LIB) bump smallvec 0.15.2 ([d4eee96c])
- (LIB) bump socket2 0.6.5 ([08d1c801])
- (LIB) bump spin 0.10.1 ([a0115e09])
- (LIB) bump syn 2.0.119 ([9b299c66])
- (LIB) bump target-triple 1.0.1 ([810b1e55])
- (LIB) bump thiserror 2.0.19 ([fe33e542])
- (LIB) bump thread_local 1.1.10 ([6d0db2ea])
- (LIB) bump time 0.3.54 ([3911166e])
- (LIB) bump tinyvec 1.12.0 ([a1f34aba])
- (LIB) bump tokio 1.53.1 ([ee73c533])
- (LIB) bump toml 1.1.3 ([61313d52])
- (LIB) bump toml_parser 1.1.3 ([d56af20c])
- (LIB) bump trybuild 1.0.118 ([1164b0d0])
- (LIB) bump twox-hash 2.1.3 ([e89197d0])
- (LIB) bump typenum 1.20.1 ([9dbbb8aa])
- (LIB) bump unicode-segmentation 1.13.3 ([4a9cfcaf])
- (LIB) bump uuid 1.24.0 ([840191a6])
- (LIB) bump wasip2 1.0.4+wasi-0.2.12 ([504d126e])
- (LIB) bump webpki-roots 1.0.9 ([51ebb581])
- (LIB) bump winnow 1.0.4 ([53cfb929])
- (LIB) bump yoke 0.8.3 ([95fe0fbd])
- (LIB) bump zerocopy 0.8.55 ([c133d6ce])
- (LIB) bump bytes 0.12.1 ([6e9c45f1])
- (LIB) bump lz4_flex 0.14.0 ([d98ed95e])
- (LIB) bump zimj 1.0.23 ([ba7e2b5c])
- (LIB) bump futures-* 0.3.33 ([c4648b6b])
- (LIB) bump Cargo.lock update socket2 downgrade ([fa384bc1])
- (LIB) bump proc-macro2 1.0.107 ([15491d06])
- (LIB) bump zeroize 1.9.0 ([c650d61f])
- (LIB) unpin dependencies ([64ff8e2b])
- (LIB) EvtxReader track stats event_largets_accept,processed ([3d4c952b])
- (LIB) PyRunner add fields PathId, tid, name ([1fea7147])
- (LIB) pyrunner.rs simplify handling TID ([e2952681])
- (LIB) Python compileall -o012 ([96e36de0])
- (LIB) Python venv no --copies ([7fcd9015])
- (LIB) linereader.rs check max line length, return Error if line too long ([33015801])
- (LIB) JournalReader add stat journal_event_larget_processed,accepted ([79f13700])
- (LIB) journalreader.rs evtxreader.rs pyeventreader.rs 'fn path_id() -> PathId' ([b6ebdcad])
- (LIB) journalreader.rs smaller default buffer ([cf2e8ec8])
- (PROJECT) build.rs obey CARGO_TERM_QUIET CARGO_TERM_VERBOSE ([bebf6e9d])
- (PROJECT) rename profiles release_0X to release_0X_pa, build all of them ([7202b7db])
- (PROJECT) .vscode/settings.json set env OUT_DIR ([a469917c])
- (TOOLS) rename 'easy-install' -> 's4-easy-install' ([e6d8b93d])
- (TOOLS) s4-easy-install allow InstallDir INSTALL_DIR ([56338193])
- (TOOLS) s4-easy-install.ps1 -Version default 0.9.82 ([c23f7d1f])
- (TOOLS) s4-easy-install.sh allow IGNORE_CHECKSUM=1 ([86bb1ba7])
- (TOOLS) s4-easy-install.sh BOLD last message ([86befefa])
- (TOOLS) s4-easy-install.sh try 'mktemp -d' multiple way ([cdfbc9fe])
- (TOOLS) performance-plot.sh do not run for already existing output file ([545e2804])
- (TOOLS) performance-plot.sh small default BLOCKSZ_MAX ([5c92c761])
- (TOOLS) performance-plots.sh gen-99999-1-Motley_Crue.log.{bz2,gz,lz4,tar,xz} ([3c0a15e9])
- (TOOLS) performance-plots.sh UTF-32LE.log|100 ([5de3665a])
- (TOOLS) release-files.sh set PROGRAMS_S4_LISTING for compare-log-mergers ([6f48d443])
- (TOOLS) release-files.sh set S4_ALLOC_TRACKER_LINK ([ef4e2665])
- (TOOLS) release-profiles.sh compare all profiles ([1294a79f])
- (TOOLS) backup.sh ignore zip failure, test output file ([51d5e563])
- (TOOLS) build-all-profiles.sh check for renice ([87227cc4])
- (TOOLS) cargo-build-profiles-timeout.sh all profiles ([942ab54b])
- (TOOLS) compare-log-mergers.sh echo commands ([7193ca3c])
- (TOOLS) compare-log-mergers.sh readlink or continue ([d7183047])
- (TOOLS) compare-log-mergers track profile,platform,opt_level ([f9d1638d])
- (TOOLS) compressions tar no --verify ([fa0e54b9])
- (TOOLS) cross-builds.ps1 rustup override $MSRV ([917c29d4])
- (TOOLS) cross-builds.sh use Extended RE, reset rustup ([c63ff966])
- (TOOLS) flamegraphs.sh fallback to dated DIROUT ([8314462d])
- (TOOLS) log-files-time-update.txt add missing gen-99999-*log* ([11fa3eae])
- (TOOLS) log-files-time-update.txt gen-99999-1-Motley_Crue.tar ([8ba36163])
- (SUBPROJECT) ere-core replace BitVec with stack array [Issue #84] ([274d75da])
- (SUBPROJECT) lzma_rs clippy requires docstrings ([2b3081de])

### Fixed

- (BIN) fix early exit failures due to pipes: early cancel proc ret 0 ([e76f18b4])
- (CI) fix errant '\' in S4_TEST_FILES ([41e6294d])
- (CI) fix upload-artifact wrong name, rm S4_BLOCKSZ override ([1dcd9f64])
- (LIB) fix systemd_dlopen2.rs API declaration, impl JournalReader::Drop with fixed pointer ([c2dae0c7])
- (LIB) impl Drop for Readers ([b461b647])
- (TOOLS) fix SCRIPTD ([29edb226])
- (TOOLS) prefer 'readlink -f' to 'realpath' ([868d5c9a])
- (TOOLS) release-files.sh fix unset $TMPD ([ff09253a])
- (TOOLS) release-files.sh fix wrong path to 'tools' ([84a92911])
- (TOOLS) performance-plot.sh fix RAM Sz set ([e154f451])
- (TOOLS) performance-plot.sh fix x scale ([6602a97a])
- (SUBPROJECT) ere_benchmark.rs correct crate path ref ([7e94281e])

## 0.9.82

_Released 2026-06-24_

_MSRV 1.88.0_

[0.9.81..0.9.82]

- support all UTF encodings [Issue #16]
- process various _month/day/year_ patterns found in Windows logs
  [Issue #46] [Issue #88] [Issue #93] [Issue #94] [Issue #95] [Issue #96] [Issue #97] [Issue #98]
- allow shortened `--color` CLI option
- allow hexadecimal `\xhh` for CLI options `sepearator`
- add Claude `SKILL.md` for using `s4`

### New

- (BIN) fix --prepend, allow \xhh ([36601944])
- (BIN) allow shortened `--color` CLI option ([1f04b993])
- (BIN) print compiled features in --version ([5d64753c])
- (BIN) --version shows build profile ([d063dd17])
- (CI) step build for docs.rs ([3565b306])
- (LIB) add 2 more DTPD for m/d/y patterns, fix others and improve associated tests [Issue #46] [Issue #95] [Issue #93] [Issue #88] [Issue #97] [Issue #98] ([84ba43d0])
- (LIB) add 3 new Regex for various Windows logs [Issue #95] [Issue #93] [Issue #88] [Issue #97] [Issue #98] ([892bb61b])
- (LIB) add two ERE_REGEX_DATETIME for Month/Day/Year, found in Windows11Pro [Issue #95] [Issue #93] [Issue #88] [Issue #97] [Issue #98] ([84686ec1])
- (LIB) regex Windows 11 lsasetup.log [Issue #94] ([098d8438])
- (LIB) add regex for oddball Windows sammui.log [Issue #96] ([7ad9cc4a])
- (LIB) allow env. var. S4_BUILD_TIMETAMP to override buildtime datetime ([8ec6c9fa])
- (LIB) support text encodings UTF-16LE,BE UTF-32LE, BE with without BOM [Issue #16] ([cc3ee9db])
- (LIB) syslinereader.rs disable syslines_by_range during Stage3Streaming ([78325c0e])
- (LIB) filepreprocessor check for all 0xFF bytes ([f5c0bd9d])
- (PROJECT) add claude SKILL.md ([7592b273])

### Changed

- (CI) actions/checkout@v7 ([fc88b9f2])
- (CI) cargo clippy S4_BUILD_REGEX=1 ([e947a92f])
- (CI) consolidate S4_BUILD_PRINT S4_BUILD_REGEX_PRINT ([c5db71aa])
- (CI) easy-install test pass -x ([858a2276])
- (CI) fix blocksz too small ([31ef5757])
- (CI) git autocrlf=false ([cef0ea0b])
- (CI) job_code_coverage_llvm install lcov ([a9de9e04])
- (CI) job job_easy_install ([488ea8b9])
- (CI) run s4 job uses more UTF* logs ([08fbb653])
- (CI) step run llvm-cov build with S4_BUILD_REGEX=TEST ([f8657df1])
- (CI) test jobs set S4_BUILD_REGEX_PRINT ([a3f0232c])
- (CI) update actions setup-python, mlc, coverallsapp ([850efa34])
- (CI) windows_test mac_test set S4_BUILD_REGEX=TEST ([d19d64c8])
- (DOCS) README.md _building locally_ describe env. vars ([8c4a90d5])
- (DOCS) README.md simplify powershell install instructions ([5e7a4e84])
- (DOCS) subprojects/ere/README.md short description of subprojects/ere ([86a539fd])
- (LIB) allow S4_BUILD_PRINT to be any truthy value: 1, true, on, yes ([12a3b476])
- (LIB) bump bitvec 1.1.1 ([d8283448])
- (LIB) bump chrono 0.4.45 ([cdfa031f])
- (LIB) bump clap 4.6.1 ([1c97261e])
- (LIB,CI) bump flamegraph 0.6.13 ([229b25f5])
- (LIB) bump itertools 0.15.0 evtx 0.12.2 ([c9e6afa5])
- (LIB) bump lz4_flex 0.13.1 ([4e07ec70])
- (LIB) bump phf 0.14.0 ([7ce31155])
- (LIB) bump rand 0.10.1 ([d6223438])
- (LIB) bump syn 2.0.118 ([74d74c11])
- (LIB) bump tikv-jemallocator 0.7.0 ([103075e2])
- (LIB) Cargo.toml memchr 2.8.2 ([b2d4ff0f])
- (LIB) Cargo.toml mimalloc 0.1.52 ([56d147ee])
- (LIB) Cargo.toml regex 1.12.4 ([48ed1f56])
- (LIB) dependabot: bump ctrlc from 3.4.7 to 3.5.2 ([26bb3c8f])
- (LIB) dependabot: bump kinded from 0.4.0 to 0.5.0 ([b989a7e7])
- (LIB) dependabot: bump lru from 0.16.3 to 0.18.0 ([101d4e7a])
- (LIB) dependabot: bump memchr from 2.7.6 to 2.8.1 ([edfdee91])
- (LIB) dependabot: bump memchr from 2.8.1 to 2.8.2 ([09a54e45])
- (LIB) dependabot: bump mimalloc from 0.1.48 to 0.1.52 ([1b162a6c])
- (LIB) dependabot: bump numtoa from 0.3.1 to 1.0.0 ([f38900fe])
- (LIB) dependabot: bump regex from 1.12.3 to 1.12.4 ([df63898c])
- (LIB) ere_datetimes_impl allow S4_BUILD_REGEX=TEST for tests ([9aa48cb0])
- (LIB) prefer numtoa over to_string ([3c0a306c])
- (LIB) syslinereader.rs summary_stat more ([432b4d50])
- (LIB) systemd_libloading.rs extern "C" ([cb633edb])
- (LIB) type RegexId ([45bc7fe2])
- (LOGS) add dtf5-6b.* variations, Darwin25 ([6afdf1bf])
- (LOGS) add gen-1000-3-foobar.UTF-*.log ([3db6df30])
- (LOGS) add Windows11Pro/cv_debug ([ae78d857])
- (PROJECT) bump version 0.9.82 ([f76f5165])
- (PROJECT) kinded =0.5.0 ([78d26831])
- (RELEASE) releases/0.9.81/x86_64-unknown-linux-gnu add x2 .html ([9e7fdf3a])
- (SUBPROJECT) bump ere projecst to 0.10.2 ([c70cf1ba])
- (SUBPROJECT) bump ere_datetimes_impl 0.10.1 ([a14ac6a2])
- (SUBPROJECT) ere_datetimes_impl.rs mark more regex TEST 23 40 71 73 78 96 136 ([742e6f93])
- (SUBPROJECTS) Reword various README.md ([db12695f])
- (SUBPROJECT) ere_datetimes_impl exclude .sh from package ([9204a453])
- (TEST) compare-current-and-expected update ([f381e929])
- (TEST) printer_tests print to sink; do not clutter test output ([95385332])
- (TEST) printers_tests.rs test_PrinterLogMessage_print_sysline_UTF UTF-8 BOM ([8f29c2f8])
- (TEST) compare-current-and-expected add dtf5-6b.UTF*.log ([b9c196f8])
- (TEST) printers_tests.rs add test_PrinterLogMessage_print_sysline_UTF ([b3752d11])
- (TOOLS) cross-builds.sh try 'cargo build' with -Zbuild-std if cross fails ([1dc1ab26])
- (TOOLS) add iconv.sh ([bbfe27bc])
- (TOOLS) add mac-setup.sh ([8eb80483])
- (TOOLS) build-all-profiles.sh add cargo check ([7a3187bf])
- (TOOLS) build-all-profiles.sh only build 2 at time; avoid OOMs ([bbb1418a])
- (TOOLS) cargo-llvm-cov-combin-multiple.sh add UTF* logs ([9e27d9f0])
- (TOOLS) cargo-llvm-cov-combine-multiple.sh default S4_BUILD_REGEX=TEST ([e8890aaf])
- (TOOLS) cargo-test.sh set S4_BUILD_REGEX=TEST ([0f85b485])
- (TOOLS) compare-current-and-expected red color stdout ([e9dda18e])
- (TOOLS) compare-current-and-expected sleep times ([e7ce0712])
- (TOOLS) compare-current-and-expected/compare.sh sleep after diff ([6e0c5daf])
- (TOOLS) cross-builds.sh allow FILTER ([e73ffe75])
- (TOOLS) cross-builds.sh fix sed matching ([5f494207])
- (TOOLS) cross-builds.sh skip already built platforms ([789265aa])
- (TOOLS) cross-builds.sh support old column ([c348408e])
- (TOOLS) easy-install.* allow ABI, Test ([37af405d])
- (TOOLS) easy-install.sh rm '--', not supported on old sh ([7de7f5cd])
- (TOOLS) iconv.sh output GB18030 ([005f7fc3])
- (TOOLS) s4-easy-install.sh do not use readlink ([5c99c2c5])
- (TOOLS) flamegraphs.sh rename .svg ([329a2a17])
- (TOOLS) log-files-time-update.sh list not_found a second time ([e8b1a1db])
- (TOOLS) mdtohtml more often ([ca786de1])
- (TOOLS) performance-plot.sh fix FILE_NUM_MAX ([bfac49f8])
- (TOOLS) perormance-plot.sh name tweak MD_FINAL CSV_FINAL ([5d741156])
- (TOOLS) release-files.sh s4 --venv ([3d2b9431])
- (TOOLS) rust-workflow.sh call build-all-profiles.sh ([f8c1c7d2])
- (TOOLS) rust-workflow.sh run test UTF* files ([329f85cf])
- (TOOLS) rust-workflow.sh S4_BUILD_REGEX=1 cargo msrv verify ([bf604a33])
- (TOOLS) s4-alloc_trackers.sh add UTF* files ([37313cdd])
- (TOOLS) s4-alloc_trackers.sh rm non-existent default failes ([08d2e28d])
- (TOOLS) rename easy-install -> s4-easy-install ([b0d5d189])

### Fixed

- (BENCH) fix non-existent var import ([9b30dd2b])
- (LIB) build.rs fix for docs.rs build; refactor build.rs with messages ([f7fc2fad])
- (LIB) Regex #185 fix pattern, test cases ([2dbedd57])
- (PROJECT) fix dependabot merges that did not update Cargo.toml ([ba98940f])

## 0.9.81

_Released 2026-05-28_

_MSRV 1.88.0_

[0.8.80..0.9.81]

- Replace runtime `Regex` regular expressions with embedded subproject build-time `ere` regular expressions. [Issue #449]
  - Halves the time to process many syslog files.
  - Eliminates runtime regular expression compile.
  - Compile time is tens of minutes.
  - Complicates project and publishing.
- Add new installation methods:
  - `easy-install.sh` script
  - manual download from Releases page
  - `binstall` support [Issue #271]
  - `quickinstall` support

### New

- (BIN) allow args via environment, move --venv in --help ([e916a2d1])
- (BIN) allow more partial -a -b patterns, refactor matching ([bb02aeec])
- (BIN) --version print Git Commit ([1f0b90f3])
- (BIN LIB) allow env S4_LIBSYSTEMD to specify path libsystemd.so ([4f5175c0])
- (BIN) version print Regex count ([31948002])
- (BIN) --version print rust compiler version ([782abbf7])
- (BIN) print RUSTFLAGS OPT_LEVEL in --version/help ([293ba44b])
- (CI) add cross s390x-unknown-linux-gnu x86_64-unknown-freebsd x86_64-unknown-illumos ([2cbf5577])
- (CI) add .github/tools/install-libsystemd.sh simplify libsystemd installs ([91edf978])
- (CI) add profile release_Opt0, use in short jobs, improve step naming ([f126a04c])
- (CI) add step print default-target ([bdd4c333])
- (CI) add step 'test ere' ([31865ac8])
- (CI) add step test ere-core ([bb3ee6f9])
- (CI) add workflow_dispatch ([4a7e6798])
- (LIB) add 2 DTF patterns for claude/history.jsonl, add log, update tests ([30638861])
- (LIB) add ere_automator_procmacro ere_datetimes_impl [Issue #449] ([cd8a8c83])
- (LIB) add statistic line_parts_created ([87325d0c])
- (LIB) add subprojects/ere/ere from 2kai2kai2/ere [Issue #449] ([00846a55])
- (LIB) subpojects/ere/ere support regex parsing bytes ([dd4f4232])
- (LIB) subprojects/ere/ere update to robertream/ere branch add-ui-compile-fail-named-captures ([ad985fad])
- (LIB) subprojects/ere versioning and pkg matching ([c9df16b9])
- (LOGS) add dtf3-invalid-UTF8.log, dtf10.log, dtf1a.log ([286ce3c2])
- (LOGS) add samba/ rsyslog/ test_cases/ ([f65650f0])
- (LOGS) add test_data_file_H_dt4.log ([bd069b03])
- (PROJECT) add package.metadata.binstall [Issue #271] ([802166de])
- (RELEASE) add releases/0.9.81/x86_64-unknown-linux-gnu ([c442d822])
- (TOOLS) add build-all-profiles.sh ([41aebcd0])
- (TOOLS) add cargo-bloat.sh ([ffe8d68d])
- (TOOLS) add cargo-macro-stats.sh ([7887ee46])
- (TOOLS) add cargo-modules-structure.sh ([89182037])
- (TOOLS) add cargo-print-type-sizes.sh ([b63af4ed])
- (TOOLS) add cargo-size.sh ([a9b47992])
- (TOOLS) add compare-two-s4.sh ([97329587])
- (TOOLS) add cross-checks.sh ([12fcb256])
- (TOOLS) add download-released-s4.sh ([53558dab])
- (TOOLS) add mdtohtml.sh ([f472247e])
- (TOOLS) add performance-plots.sh ([ad96b358])
- (TOOLS) add s4-alloc_trackers.sh ([08017c8f])
- (TOOLS) add stack-size.sh ([a7b7012a])
- (TOOLS,DOCS) add easy-install.sh, update README with installation instructions ([cd58d05e])
- (TOOLS) easy-install do not require rust ([455a8941])
- (TOOLS) easy-install.sh checksum zip file ([a5d7efc9])
- (TOOLS) easy-install.sh POSIX compatible ([7087d4ff])
- (TOOLS) add easy-install.ps1 and README instructions ([e2d36cee])
- (TOOLS) performance-plots.sh add log file gen-99999-1-Motley_Crue.log, allow FILE_NUM_MAX ([2cf4a04c])
- (TOOLS) performance-plots.sh add more varying log files ([abb0706e])
- (TOOLS) performance-plots.sh allow FILES, save CSV of raw data ([98c7bbd9])
- (TOOLS) performance-plots.sh dtf5-6a.log ([3055d42e])

### Fixes

- (PYTHON) fix dep pytokens 0.4.0 ([2fd39187])

### Changes

- (AI) add prompts/sync-rust-platform-tiers.prompt.md ([3ac44ab1])
- (BENCHES) use std::hint::black_box ([17b04216])
- (BIN) alloc_tracker.rs consistent naming, clearer helper functions ([f8214de1])
- (BIN) alloc_tracker.rs fewer allocations in alloc ([175b74ee])
- (BIN) alloc_tracker.rs remove a clone, other printing adjustments ([6fdcc1ba])
- (BIN,LIB) add SystemDebug for specialized debugging ([3a17c26c])
- (BIN,LIB) alloc_tracker stores index to filename,function ([fd429436])
- (BIN,LIB) create src/s4/mod.rs, move relevant files, separate tests ([48ef3d88])
- (BIN,LIB) sysalloc_debug various improvements ([98c95a0a])
- (CI) alloc_tracker set RUST_MIN_STACK [Issue #467] ([59e39b73])
- (CI) allow step Publish Dry Run to fail ([6d9313f6])
- (CI) build matrix only run win+mac on MSRV ([879f3512])
- (CI) chmod 555 s4 ([9b64e57f])
- (CI) clippy no --verbose ([1a71232d])
- (CI) cross check --release ([dd224ee0])
- (CI) cross do not i686-unknown-linux-gnu ([428926d3])
- (CI) cross jobs set file S4_BUILD_REGEX ([7559d3e1])
- (CI) disable job_cross_targets_windows ([0608caba])
- (CI) flamegraph S4_BUILD_REGEX=1 ([0648f40c])
- (CI) FLAMEGRAPH_VERSION: 0.6.12 ([0f511e71])
- (CI) force S4_BUILD_REGEX via file ([fe51c9ab])
- (CI) job_cross_targets_macos uploads, add targets aarch64-apple-darwin arm64e-apple-darwin x86_64h-apple-darwin ([ef6c6d46])
- (CI) job_cross_targets_macos use BSD find ([919cc4fc])
- (CI) fix job_cross_targets_macos upload ([592cbba7])
- (CI) job_cross_targets runs-on ubuntu-latest ([9f194c5e])
- (CI) job_test_linux needs job_build_debug_release ([3c902662])
- (CI) matrix 1.94.1 ([db27f5c2])
- (CI) name steps with MSRV_UPLOAD ([495b31dd])
- (CI) no cross x86_64-unknown-redox ([3fe4262e])
- (CI) no duplicate cross builds ([0106aa75])
- (CI) on: subprojects/**.rs ([fa3bb518])
- (CI) remove cross aarch64-apple-ios x86_64-apple-ios ([be67657e])
- (CI) rm cross arm64e-apple-darwin ([4384a4a0])
- (CI) rm cross x86_64h-apple-darwin ([7d5de9b5])
- (CI) rm unnecessary bash --version, grep --version ([684d9043])
- (CI) run with S4_SYSALLOC_DEBUG_PRINT S4_SYSALLOC_DEBUG_TRACKING ([f1717a9b])
- (CI) rust.yml remove unused uploads ([179c84a7])
- (CI) rust.yml yamllint fixes ([3202c47d])
- (CI) simplify job_cross* needs ([b7078ffb])
- (CI) skip alloc_tracker on macos-latest ([13c3926c])
- (CI) step compare-debug-release chmod +x compare-two-s4.sh ([d0959042])
- (CI) step compare-debug-release.sh set PROGRAM_A PROGRAM_B ([07193f74])
- (CI) step flamegraph only builds it ([83819c8c])
- (CI) step venv S4_BUILD_REGEX=1 ([5de73699])
- (CI) test_windows use cargo test ([69149fd5])
- (CI,TOOLS) add log-files-time-update.ps1, use that in CI step ([e1fff225])
- (CI) update actions setup-python, upload-artifact, download-artifact ([e4b6a981])
- (CI) update cross targets for linux ([cad2b80e])
- (CI) use actions/checkout@v6,upload-artifact@v6 ([45c67a77])
- (CI) use s4_release to create --venv ([20b4b045])
- dependabot: bump anyhow from 1.0.100 to 1.0.102 ([dde421c2])
- dependabot: bump bstr from 1.12.0 to 1.12.1 ([b25ec234])
- dependabot: bump bytes from 1.11.0 to 1.11.1 ([3ad35f02])
- dependabot: bump cargo-msrv from 0.15.1 to 0.18.4 ([f31e2d99])
- dependabot: bump cfg-if from 1.0.3 to 1.0.4 ([9b08a080])
- dependabot: bump chrono from 0.4.42 to 0.4.44 ([9f355b27])
- dependabot: bump const_format from 0.2.35 to 0.2.36 ([e72a329e])
- dependabot: bump criterion from 0.5.1 to 0.8.2 ([01a7cce4])
- dependabot: bump filetime from 0.2.26 to 0.2.27 ([39d4b897])
- dependabot: bump flamegraph from 0.6.10 to 0.6.11 ([5e4e41da])
- dependabot: bump flamegraph from 0.6.11 to 0.6.12 ([6c23f866])
- dependabot: bump flate2 from 1.1.6 to 1.1.9 ([6dd2195a])
- dependabot: bump lz4_flex from 0.12.0 to 0.12.1 ([a58b1a9d])
- dependabot: bump once_cell from 1.21.3 to 1.21.4 ([c2487ce4])
- dependabot: bump opt-level from 1.0.0 to 1.0.1 ([eaf713ee])
- dependabot: bump rand from 0.9.2 to 0.9.3 ([bbb85cf8])
- dependabot: bump rangemap from 1.6.0 to 1.7.1 ([211ba869])
- dependabot: bump tar from 0.4.44 to 0.4.45 ([56f7b0bf])
- dependabot: bump tempfile from 3.23.0 to 3.24.0 ([ba4c049c])
- dependabot: bump tempfile from 3.24.0 to 3.27.0 ([17a4aa8e])
- dependabot: bump time from 0.3.45 to 0.3.47 ([a9df5a66])
- dependabot: bump unicode-width from 0.2.1 to 0.2.2 ([ee094b2d])
- (DOC) README embed lib.rs logo PNG data ([9d677c03])
- (DOC) README.md fix easy-install section, update performance comparison, misc ([5d2e4ae2])
- (DOC) README rearrange section allocators ([02be0545])
- (DOC) README replace some tag links to main ([a8adb983])
- (DOC,RELEASE) show compare-mem-mss.svg ([4f0b0c39])
- (DOCS) CHANGELOG update 0.8.80 ([f5903af7])
- (DOCS,SUBPROJECTS) commetn PROJECT VERSION ([87aa9759])
- (DOCS) update README with cross platforms ([92097a56])
- (LIB) Fix thread-safety: use thread-local counter for backtrace allocation snapshot ([fb70a847])
- (LIB) Add ALLOCATOR_ALLOCATED_TRACKING_OFF_BACKTRACE to track bytes during backtrace calls ([fdc69b7a])
- (LIB) Add S4_ALLOC_DEPTH env var to alloc_tracker (default 1, max 16) ([3c2b0dfe])
- (LIB) alloc_tracker print markdown ([34c117d1])
- (LIB) alloc_tracker.rs allow S4_ALLOC_TRACKER_OUTPUT ([0293baf2])
- (LIB) alloc_tracker.rs print_tracking_map better distinguish tracking internals ([be8f4865])
- (LIB) build.rs panic print info about bad value for S4_BUILD_REGEX ([9cdb6784])
- (LIB) bump MSRV 1.86.0 ([e0b96709])
- (LIB) bump MSRV 1.88.0 ([03a2c316])
- (LIB) cargo update ([6f355371])
- (LIB) clippy recommendations ([6798a5ba])
- (LIB) const_format feature rust_1_83 ([3daffcc5])
- (LIB) ere_automator_procmacro reject regex_id < 1 ([83a2d429])
- (LIB) evtx 0.11.2 ([a86b5708])
- (LIB) evtx::Evtx store the original record ([e2019f37])
- (LIB) fix bitvec missing dependency ([962e7228])
- (LIB) fix once_cell ver 1.21.4 in Cargo.toml ([297ca3d5])
- (LIB) journalreader.rs ENTRY_BUFFER_SZ=16 ([a7fe1de4])
- (LIB,LOG,TOOLS) rename Microsoft-Windows-Kernel-PnP%4Configuration -> Microsoft-Windows-Kernel-PnP__Configuration ([ee8101d6])
- (LIB) more const fn ([dd53fdc4])
- (LIB) mv slice_contains to slice_contains.rs ([365012d0])
- (LIB) pyeventreader.rs do not debug_panic over etl_parser_used ([4a27516a])
- (LIB,PY) python black wheel ([e889d5e0])
- (LIB) refactor regex; compare-current-and-expected/update.sh ([ef6e975a])
- (LIB) refactor regex to use ere [Issue ] ([7c65a387])
- (LIB) replace unmaintaned dotenv with dotenvy ([9b627bf2])
- (LIB) --summary adjust width space for datetime first/last ([e05286e8])
- (LIB) --summary out of order for syslogs ([6a4ef68e])
- (LIB) --summary print libsystemd path ([c95b376e])
- (LIB) syslinereader.rs use sort_unstable_by to avoid allocaction ([12ec8d9e])
- (LIB) sysline.rs const charsz count_lines ([ecc279c9])
- (LIB) upgrade tar 0.4.46 ([75afb738])
- (LIB) use explicit Arc::clone ([ff2a4a40])
- (LIB) use unit_prefix in alloc_tracker table ([77582527])
- (LIB) Print all captured frames per entry in print_tracking_map ([3b0aefe4])
- (LOGS) log-files-time-update.sh add ./tools/compare-log-mergers/gen-5000-1-faces*.log ([5e0ebce3])
- (PROJECT) add extensions.json ([671bbeb5])
- (PROJECT) adjust binstall template string ([1a115291])
- (PROJECT) allow setting file S4_BUILD_REGEX ([b269c9d7])
- (PROJECT) bump version 0.9.81 ([fb51c813])
- (PROJECT) define root workspace, workspaces for subprojects/ere/* projects ([903ee8aa])
- (PROJECT) exclude more from package ([379db5a6])
- (PROJECT) extensions.json markdownlint ([5918660c])
- (PROJECT) .gitignore /**/target/ ([3c01b315])
- (PROJECT) test-case downgrade 3.2.1 ([2edd326d])
- (PYTHON) Bump pygments from 2.19.2 to 2.20.0 in /src/python/s4_event_readers ([f8c42de5])
- (PYTHON) pip-tools-compile.sh use $PYTHON ([7fe0d4ea])
- (PYTHON) pip update requirements-dev.txt ([35b02055])
- (RELEASE) add 0.8.81rc1 ([8e46a515])
- (RELEASE) add releases/0.9.81rc6,0.9.81rc7 ([e921e671])
- (SUBPROJECT) ere name and rename package with leading 'super-speedy-syslog-searcher_' [Issue #449] ([f500cf73])
- (SUBPROJECT) ere use bitvec instead of Vec bool ([8da8ed5c])
- (TEST) ere/compile_fail.rs add feature tests-compile-fail ([6723d4f3])
- (TEST) ere fix module references in tests ([d1b7a07c])
- (TEST) fix pyrunner_tests on Windows; force ending '\n' ([f784a31a])
- (TEST) downgrade test-case to 3.2.1 ([febb71c7])
- (TEST) fix test_decompress_to_ntf_ok_some ([6fa05388])
- (TEST) fix test_PyEventReader_new_asl_odl, skip panic testing ([7c701ac8])
- (TEST) NEXTEST_TEST_THREADS=1 possibly fix CI failures ([8c7b2393])
- (TEST) syslinereader_tests.rs fix REGEX_ID_H_dt4 ([bd1ec3ec])
- (TEST) test_process_dt fix use of utc_now, use local_now ([7c94c990])
- (TOOLS) backup.sh .git/ ([f561976f])
- (TOOLS) backup.sh use zip ([0f93ff94])
- (TOOLS) cargo-bloat.sh --version ([4799f3c2])
- (TOOLS) cargo-call-stack.sh use DIROUT, nightly-2023-11-13 ([e83fdb5d])
- (TOOLS) cargo-test.sh NEXTEST_TEST_THREADS=1 default ([e2841e3b])
- (TOOLS) chmod +x magic-trace.sh ([7764150f])
- (TOOLS) chmod +x *.sh ([a7ba9e3c])
- (TOOLS) clean-file.sh be selective about sed expressions used ([03eea397])
- (TOOLS) compare-current-and-expected/update.sh ([e807cd11])
- (TOOLS) compare-debug-release.sh fix path default logs listing ([eb602d99])
- (TOOLS) compare-debug-release.sh use compare-two-s4.sh, more logs ([2f8bedc0])
- (TOOLS) compare-log-mergers.sh allow PROGRAM_S4_OPTIONAL, add version,allocator,platform ([9733f4b3])
- (TOOLS) compare-log-mergers.sh allow PROGRAM_S4_OPTIONAL ([fc176f0f])
- (TOOLS) compare-log-mergers.sh require PROGRAMS_S4_LISTING ([62a6f0c3])
- (TOOLS) compare-log-mergers.sh set platform Python for toolong ([1f8f24ee])
- (TOOLS) compare-mem.sh add TIME SVG, other improvements ([f764c6fb])
- (TOOLS) compare-mem.sh improve scaling, ticks, etc. for SVG ([83bc25eb])
- (TOOLS) compare-mem.sh improve svg with ticks, grids ([5624bb3a])
- (TOOLS) compare-mem.sh SVG add 2nd y-axis diffs ([51ba2e8b])
- (TOOLS) compare-mem.sh SVG title improve ([190ecbfb])
- (TOOLS) cross-builds.ps1 rustup install MSRV ([a951fb3d])
- (TOOLS) cross-builds.sh grep for MSRV ([368fc14c])
- (TOOLS) cross-checks -> cross-builds ([80e0b599])
- (TOOLS) cross-checks.ps1 tweak regex, note OutputType ([009ac596])
- (TOOLS) cross-checks.sh output as markdown, cp s4 to DIROUT ([513bb501])
- (TOOLS) cross-checks.sh refactor SHA256 creation ([d9788a97])
- (TOOLS) cross-checks.sh try all platforms, print nice table at end ([3865ae2f])
- (TOOLS) dependencies.apt add gnuplot ([e52abe58])
- (TOOLS) dependencies.apt cover compressions.sh ([3cd8f732])
- (TOOLS) easy-install.sh fix URL format ([49349571])
- (TOOLS) heaptrack.sh better name outputs ([89e91d6b])
- (TOOLS) heaptrack.sh rm unused, use DIROUT ([d1c548dd])
- (TOOLS) log-files-time-update.ps1 compatible with Powrshell 5 ([7f92acab])
- (TOOLS) log-files-time-update.sh tail -n2 missing file ([f9ceca94])
- (TOOLS) log-files-time-update.txt fix some unusual datetime formats ([1894f511])
- (TOOLS) magic-trace.sh find perf ([bbfa880a])
- (TOOLS) mdtohtml.sh embed CSS instead of file ref ([921db423])
- (TOOLS) mdtothtml.sh apply simple css ([440cbf9d])
- (TOOLS) mv compare-mem.sh -> performance-plot.sh ([260fcfc9])
- (TOOLS) mv s4-alloc_tracker.sh ([13038896])
- (TOOLS) osv-scanner.sh fix PATH add ([d92039a3])
- (TOOLS) osv-scanner.sh --version, forward user-passed args ([8a40accd])
- (TOOLS) PATH+=~/go/bin ([596f5b9e])
- (TOOLS) performance-plot.sh fix format, rearrange names, print git tag ([9806797c])
- (TOOLS) performance-plot.sh note BlockSz ([00530383])
- (TOOLS) performance-plot*sh replace FNUM_MAX with FILE_NUM ([64b76e7c])
- (TOOLS) print targets_built targets_failed ([7b973854])
- (TOOLS) refactor cross-checks.ps1 ([3ab938d5])
- (TOOLS) release-files s4-alloc_trackers.sh ([524e25e8])
- (TOOLS) release-files.sh build in parallel ([0f8109cc])
- (TOOLS) release-files.sh call heaptrack.sh ([408aac8c])
- (TOOLS) release-files.sh fewer parallel builds ([4e9d8eb7])
- (TOOLS) release-files.sh move flamegraphs to last ([9614463b])
- (TOOLS) release-files.sh performance-plots.sh osv-scanner.sh ([95498c64])
- (TOOLS) release-files.sh use clean-file.sh ([f5b42761])
- (TOOLS) rsync-project-to.sh exclude subprojects/ere/*/target ([770a0aee])
- (TOOLS) rust-workflow.sh allow --clean ([97bb2f00])
- (TOOLS) rust-workflow.sh allow --skip-clean --skip-build ([95aa7aa6])
- (TOOLS) rust-workflow.sh S4_BUILD_REGEX=1 cross-checks.sh ([103899b0])
- (TOOLS) s4-alloc_debug.sh --quiet ([4094fee8])
- (TOOLS) s4-alloc_debug.sh helper script ([d5b91372])
- (TOOLS) s4-alloc_tracker S4_ALLOC_TRACKER_DEPTH=1 ([6e14d696])
- (TOOLS) s4-alloc_tracker.sh S4_BUILD_REGEX_PRINT= ([7ceb8e85])
- (TOOLS) s4-alloc_trackers.sh add log file gen-99999-1-Motley_Crue.log ([92af83ad])
- (TOOLS) s4-alloc_trackers.sh insert javascript for sorting tables ([821707dd])
- (TOOLS) s4-alloc_trackers.sh mkdir $DIROUT ([ffbbfac2])
- (TOOLS) s4-alloc_trackers.sh PATH+=~/go/bin ([9a7502e5])
- (TOOLS) s4-alloc_trackers.sh title is LOGNAME ([5af3dbf7])
- (TOOLS) tmp startswith 's4-tmp' ([136852f5])
- (TOOLS) valgrind-massif.sh default profile valgrind/s4 ([3ab90e70])
- (TOOLS) venv-create.sh helper script ([3334fbf0])
- (TOOLS) +x cargo-bloat.sh ([9cb73644])
- (TOOLS) +x s4-alloc_tracker*.sh ([57bf4db5])
- (TOOLS) +x venv-create.sh ([96781a0a])

## 0.8.80

_Released 2026-01-11_

_MSRV 1.85.0_

[0.7.79..0.8.80]

### New

- (LIB) parsing .etl .odl using Python process [Issue #262] [Issue #99] ([4bd2d6db])
- (BIN) summary print ASL Events ([5d4beee6])
- (LIB) process .asl files [Issue #102] ([7ab4f3bb])
- (LIB) allocator tcmalloc ([db649b16])
- (LIB) allocator rpmalloc ([dc8f3631])
- (TOOLS) flamegraphs.sh asl etl odl ([03bbeb7a])

### Changes

- (BIN) clippy recm parse ([32bff443])
- (BIN) --version and --help agree ([8f8a3b5e])
- (BIN) --version print platform ([ccab6fc0])
- (BUILD) Cargo.toml exclude more published ([a66e2a95])
- (BUILD) bin,lib use default edition 2024 ([7673f395])
- (CI) job test_windows NEXTEST_TEST_THREADS=1 ([32017711])
- (CI) setup venv, test w .etl .odl ([7f01cdf0]) ([bb8a1d7d]) ([84ee6cca])
- (CI) test venv creation ([b09eac86])
- (CI) use flamegraph 0.6.8 ([916d7845])
- (DOC) README mention .asl .etl .odl support ([d9cf2120])
- (DOC) README improve allocator subsections ([ccc9e298])
- (DOC) tweak docstring license ([00827808])
- (LIB BIN) misc clippy recommendations ([a9955bc1])
- (LIB) dependabot: bump dlopen2 from 0.7.0 to 0.8.2 ([f8953e5a])
- (LIB) dependabot: bump lz4_flex from 0.11.3 to 0.12.0 ([6002cf1e])
- (LIB) dependabot: bump kinded from 0.3.0 to 0.4.0 ([9982f699])
- (LIB) dependabot: bump flamegraph from 0.6.8 to 0.6.10 ([1f97209d])
- (LIB) dependabot: bump tikv-jemallocator from 0.6.0 to 0.6.1 ([446970eb])
- (LIB) dependabot: bump lru from 0.16.1 to 0.16.3 ([570d4820])
- (LIB) blockreader.rs use specific error FileTooLarge, IsADirectory ([f54e5814])
- (LIB) bump const_format 0.2.35 ([e1216edf])
- (LIB) bump flamegraph=0.6.8 ([573c4c7a])
- (LIB) bump flate2 1.1.6 ([c24a5f09])
- (LIB) MSRV 1.83.0 ([aa1ece79])
- (LIB) bump MSRV 1.85.0 ([e94f9e81])
- (LIB) bump numtoa 0.3.1 ([6215abed])
- (LIB) Apply summary_stat! macro and docstring comments to linereader.rs [Issue #437] ([10622640])
- (LIB) filedecompressor.rs decompress_to_ntf replace '\0' ([9ef5f4bb])
- (LIB) filedecompressor.rs more explicit File open_options ([2215ce8f])
- (LIB) fixedstructreader.rs summary_stat! ([4b582114])
- (LIB) fixedstruct.rs const ll_line ll_host ([b7898f97])
- (LIB) journalreader.rs allow non_upper_case_globals ([8fd66f41])
- (LIB) journalreader.rs const mtime ([1171a278])
- (LIB) more const fn ([73c1ac58])
- (LIB) PipeStreamReader fix unbounded channel ([39bb00e2])
- (LIB) printers.rs helper fn print_colored_stderr ([af549c65])
- (LIB) pyrunner.rs channel::bounded(16) ([e8b786c5])
- (LIB) PyRunner stat channel_max ([ff46a3f3])
- (LIB) rm venv, odl_reader.py rm unused imports ([8ab4b909])
- (LIB) summary_stat! blockreader.rs evtxreader.rs ([c2d7cc41])
- (LIB) syslinereader.rs create index with_capacity ([8d6665b3])
- (LIB) syslogprocessor.rs rm redundant path blocksz ([d2a6d981])
- (LIB) systemd_dlopen2 const more fns ([2742cc8d])
- (LIB TEST) journalreader.rs use summary_stat! ([edc67891])
- (LIB TEST) pass recv_timeout to PyRunner, refactor tests for common venv ([e806aee6])
- (LIB TEST) syslinereader.rs summary_stat! ([947407b6])
- (LIB) update tracing-subscriber 0.3.20 ([b3320944])
- (LIB) --venv run s4_event_readers ([89aca0bc])
- (PROJECT) add licenses for copied code ([78eea85d])
- (PROJECT) add .taplo.toml ([fd481296])
- (PROJECT) edition 2024 ([883d0f01])
- (PROJECT) profile flamegraph set opt-level debug debuginfo ([adf89152])
- (PROJECT) .vscode/settings.json set rust-analyzer MSRV ([f3cf3672])
- (RELEASE) add releases/0.7.79/aarch64-unknown-linux-gnu ([53260fca])
- (RELEASE) add releases/0.8.80/x86_64-unknown-linux-gnu ([beac51ae])
- (RELEASE) bump version 0.8.80 ([d3e11249])
- (TEST) add cargo-test-each.sh, CI job job_test_linux_each ([a5856518])
- (TEST) clippy Error::other ([0770728e])
- (TEST) compare-current-and-expected update.sh export TZ=America/New_York ([d519d782])
- (TEST) fix test_PyEventReader_new_asl_odl_panic ([0f52dd97])
- (TEST) implement multi-platform inter-process mutex file for coordinating venv ([88fa09ba])
- (TEST) pyrunner_tests.rs always set print end ([1b9e4228])
- (TEST) pyrunner_tests.rs consolidate swap_bytes ([e804087b])
- (TEST) compare-current-and-expected/update.sh update for new --summary ([7a448d6d])
- (TEST) sd_journal_h.rs formally ignore unused tests ([1d8f57f7])
- (TEST TOOLS) compare-current-and-expected add Python files, venv, rm summary stats for Python, change dt_before ([77fc6038])
- (TEST) use std::hint::black_box ([68725bea])
- (TEST) venv_setup note PID ([f3584ab6])
- (TOOLS) add compare-versions ([ee2e1500])
- (TOOLS) add magic-trace.sh ([cdf70636])
- (TOOLS) add tcmalloc apt packages ([f298fc98])
- (TOOLS) add tools/dependencies.apt ([2ca50bf4])
- (TOOLS) do not compare 2023.10.26.asl [Issue #445] ([2e2730a7])
- (TOOLS) add compare-mem.sh ([49825e22])
- (TOOLS) backup.sh add missing files, all logs ([b94f74dc])
- (TOOLS) backup.sh *log,svg ([f6dd6e18])
- (TOOLS) build-versions patch blockreader.rs ([781fbebb])
- (TOOLS) build-versions.sh cargo clean later ([e7ef323c])
- (TOOLS) build-versions.sh fix summary table ([4c49260a])
- (TOOLS) build-versions.sh set rust-version ([f662d18a])
- (TOOLS) cargo-test-each $1 can be a filter ([c251114c])
- (TOOLS) cargo-test-each.sh do not rm during EXIT ([5431942b])
- (TOOLS) compare-current-and-expected add 2023.10.26.asl.stderr, fix TZ ([de2fe8c1])
- (TOOLS) compare-current-and-expected count line diff in summary of differences ([72ec4d9e])
- (TOOLS) compare-debug-release_logs.txt add .asl .etl .odl ([932986c4])
- (TOOLS) compare-debug-release.sh compare individually ([311b7f43])
- (TOOLS) compare-grep-sort.sh compare gen-5000-1-faces.log ([f045423e])
- (TOOLS) compare-log-mergers.sh more files to compare ([498d5f9e])
- (TOOLS) compare-versions grep patterns for versions older 0.7.75 ([13f65d1f])
- (TOOLS) dependencies.apt colordiff ([9556ac7b])
- (TOOLS) flamegraph.sh search for perf ([9848eab1])
- (TOOLS) flamegraph.sh trim version output in NOTES ([b7d11d37])
- (TOOLS) hyperfine --version ([d5a855e2])
- (TOOLS) rust-gdb.sh add --silent, pass args as-is ([9ad1616b])
- (TOOLS) rust-workflow --venv ([303b446f])
- (TOOLS) .vscode/launch.json add 3 Debug Run profiles ([bb32b517])
- (TOOLS) vscode/settings.json flake8 args ([7dea777e])
- (TOOLS) +x compare.sh ([53a3cabf])

### Fixes

- (LIB) summary.rs fix debug_assert and or ([9df010c8])

---

## 0.7.79

_Released 2025-09-28_

_MSRV 1.81.0_

[0.7.78..0.7.79]

### New

- (BIN) allow -a -b HH:MM:SS HH:MM [Issue #36] ([524988dc])
- (BIN) --help explains datetime filters without time date ([41f316ca])
- (BIN LIB) count namedtempfiles, print in summary ([8c5ca504])
- (BIN LIB) option --light-theme [Issue #261] ([e594436e])
- (TOOLS) add cargo-llvm-cov-combine-multiple.sh ([bfa153de])

### Changes

- (BIN) bump version 0.7.79 ([407fd870])
- (BIN) fix grammar oddity ([635ce62a])
- (BIN) --help separator of ending info ([3a6bbe84])
- (BIN) NFC bin.rs misc clippy tweaks ([56c0743c])
- (BIN) --summary tweak spacing in 'Processing Drops' subsection ([5627c971])
- (CI) add wtmp journal strace to S4_TEST_FILES ([4dc89cea])
- (CI) coveralls upload cargo-llvm-cov-combine-multiple.sh ([9b54b319])
- (DOCS) examples hightlight-a -b offsets ([de94d527])
- (DOCS) fix README table version description ([bf556e61])
- (DOCS) README add platforms supported ([0b781120])
- (DOCS) README comparisons note grep+sort dmesg ([33e795be])
- (DOCS) README more examples of -a -b usage ([3dc2d474])
- (DOCS) README note 0.7.79 ([a9d0009a])
- (DOCS) README tweak wording on comparison ([da32db19])
- (LIB) dependabot: bump lz4_flex from 0.11.3 to 0.11.5 [Issue #419] ([c734d635])
- (LIB) apply misc clippy suggestions ([5390f7fa])
- (LIB BIN) SUBPATH_SEP='\0' SUBPATH_SEP_DISPLAY ([835cdde6])
- (LIB) bump clap 4.5.48 ([eb02c426])
- (LIB) bump ctrlc 3.5.0, lz4_flex 0.11.3 ([01eb4c24])
- (LIB) pin ctrlc=3.4.7 ([f3f4f7d4])
- (LIB) bump rust-version 1.81.0 ([6b8d31e1])
- (LIB) cargo upgrade multiple packages ([332d8659])
- (LIB) NFC helpers.rs tweak docstrings ([612c4533])
- (LIB) NFC rename ResultS3 to ResultFind ([e8887e16])
- (LIB) remove redundant CStr::is_empty ([81446a51])
- (LIB) rm unused BlockReader::_subpath ([e75a099b])
- (LIB) rm unused PrinterLogMessage::_color_logmessage ([88285895])
- (RELEASE) add releases/0.7.79/x86_64-unknown-linux-gnu/ ([e6fdc373])
- (TEST) add helpers_tests, revise count_char_in_str ([be9e0dfd])
- (TOOLS) cargo fmt use nightly ([492188bd])
- (TOOLS) cargo-llvm-cov-run.sh tweak dt-before, --locked ([d1f63b14])
- (TOOLS) release-files rearrange last clean_file call ([47ab5f3e])
- (TOOLS) rust-workflow.sh call cargo-llvm-cov-combine-multiple.sh ([831f3420])
- (TOOLS) requirements logmerger==0.12.0 ([81cff5d9])
- (TOOLS) use logmerger 0.12.0 ([246addaa])

### Fixes

- (LIB) fix errant debug_panic during test_blockread ([d288501a])
- (LIB) fix errant debug_panic for wrong drop_block ([0d6cdb58])

---

## 0.7.78

_Released 2025-08-24_

_MSRV 1.73.0_

[0.7.77..0.7.78]

### Changes

- (DOCS) update comparison table of log mergers for 0.7.77 ([5b12554d])
- (RELEASE) add releases/0.7.78/aarch64-unknown-linux-gnu/ ([41009684])
- (TOOLS) compare-log-mergers.sh hyperfine -i ([4adfd641])

### Fixes

- (LIB) fix processing uptime logs [Issue #416] ([0addafbf])

---

## 0.7.77

_Released 2025-08-06_

_MSRV 1.73.0_

[0.7.76..0.7.77]

### New

- (BIN) --help print author, ver, lic, rep, msrv ([bf08883b])
- (LIB) add processing of dmesg format [Issue #4] [Issue #56] ([5cf38ef9])
- (LIB) add support lightdm.log format [Issue #110] ([7ce50613])

### Changes

- (BIN) change default allocator name to 'system' ([e4a97d99])
- (CI) enable x86_64-sun-solaris x86_64-pc-solaris [Issue #354] ([aeb3e899])
- (CI) disable cross x86_64-sun-solaris ([476443b1])
- (DOCS) README compare table not arch ([bbfb384c])
- (DOCS) README declare section table of results ([77cc4977])
- (DOCS) README example of compare logs ([5b42c314])
- (DOCS) README link to [Issue #128] ([7b3593c6])
- (DOCS) README.md mention X.org style logs ([e35c450d])
- (DOCS) README note dmesg in table ([53d9cbc7])
- (DOCS) README note list of ignored files ([ff7e7a74])
- (DOCS) README note rustc, link speed comparison ([b6aa5625])
- (DOCS) README update compare table 0.7.76 ([51f0a78b])
- (LIB) dependabot: bump anyhow from 1.0.97 to 1.0.98 ([755c269b])
- (LIB) dependabot: bump cfg-if from 1.0.0 to 1.0.1 ([8c1cd0b9])
- (LIB) dependabot: bump chrono from 0.4.40 to 0.4.41 [Issue #404] ([df5ff095])
- (LIB) dependabot: bump libc from 0.2.171 to 0.2.174 [Issue #411] ([05ca7b4d])
- (LIB) dependabot: bump lru from 0.13.0 to 0.16.0 ([53419617])
- (LIB) dependabot: bump lz4_flex from 0.11.3 to 0.11.5 ([9f942b0e])
- (LIB) dependabot: bump mimalloc from 0.1.45 to 0.1.47 [Issue #414] ([7a4558e9])
- (LIB) dependabot: bump nix from 0.29.0 to 0.30.1 [Issue #415] ([03b6e216])
- (LIB) dependabot: bump numtoa from 0.2.4 to 0.3.1 ([5914ce02])
- (LIB) downgrade numtoa to 0.2.4 to avoid rustc errors ([ecd0e724])
- (LIB) dependabot: bump phf from 0.11.3 to 0.12.1 ([a967d783])
- (LIB) dependabot: bump rangemap from 1.5.1 to 1.6.0 [Issue #412] ([2b62c402])
- (LIB) dependabot: bump unicode-width from 0.2.0 to 0.2.1 ([34cd9539])
- (LIB) specify extern fn as "C" ([a961e626])
- (LIB) Uptime is i32 ([437eeb96])
- (TOOLS) changelog-link-gen.sh link PR notation ([14d6f336])
- (TOOLS) compare-log-mergers use requirements.txt, link projects, update toolong ([9325865b])
- (TOOLS) fix build of mimalloc jemalloc ([8ab1f705])
- (TOOLS) prefer 'test' to 'nextest' ([2a1584e3])
- (TOOLS) rsync-to-project.sh allow passing RSYNC ([b566804d])

---

## 0.7.76

_Released 2025-07-06_

_MSRV 1.73.0_

[0.7.75..0.7.76]

### Changes

- (BIN) --help notes allocator, debug, test ([d4de1431])
- (BUILD) add rust-toolchain.toml ([20c469c9])
- (BUILD) rm rust-toolchain.toml ([9d60fcc7])
- (BUILD) Cargo.lock update dunce ([581c6eb5])
- (BUILD) MSRV 1.73.0 ([3a2f29f2])
- (CI) add job_deny ([5c811752])
- (CI) fix job_cross_targets ([2e60f5f7])
- (CI) fix target add ([91a42744])
- (CI) remove rust caching [Issue #360] ([1fd2eb2e])
- (CI) replace moonrepo/setup-rust with Swatinem/rust-cache [Issue #360] ([9b64afba])
- (CI) rust.yml cross loongarch64-unknown-linux-gnu ([245729f1])
- (CI) rust.yml replace moonrepo/setup-rust@v1 with actions-rust-lang/setup-rust-toolchain@v1 [Issue #360]([f563b7aa])
- (CI) rust.yml use cache-apt-pkgs-action for apt installs ([d00d725d])
- (CI) simplify profile build cmds ([702e54ad])
- (CI) skip cross x86_64-pc-solaris ([786b1543])
- (CI) skip cross x86_64-sun-solaris ([7eed267c])
- (DOCS) CHANGELOG NFC note 'RELEASE' ([709b5293])
- (DOCS) NFC README.md tweak tables format ([f76b06eb])
- (DOCS) NFC release 0.7.75 tweaks; README table simplify ([14f3cab1])
- (DOCS) NFC tweak precommit message category ([f47f89b7])
- (DOCS) README add lnav ([e4658658])
- (DOCS) README, Changelog add 0.7.75 ([2e80d2bf])
- (DOCS) README.md release 0.7.76 ([ca77ad21])
- (DOCS) README rm Benches, update program comparison ([08276667])
- (LIB) add more libsystemd.so file name variations to search ([d03812d6])
- (LIB) journalreaders.rs deprecated `from_i32` with `from_raw` ([3139418f])
- (LIB) misc code comments and docs wording fixes ([5218d4f8])
- (LIB) NFC TRACKING OnceLock::get_or_try_init ([66c753ab])
- (LIB) printers.rs line.rs prefer write_all ([1b3b8d34])
- (LIB) printers.rs NFC user write_to ([71d1a55a])
- (LIB) update bindings 0.72.0, generate new sd_journal_h.rs [Issue #401] ([8cd73fc7])
- (LIB) update hashbrown 0.14.2->0.14.5 0.15.0->0.15.2 ([132bcd74])
- (LIB) update subprojects/bindgen, sd_journal_h.rs using cargo 1.73.0, bindgen 0.53.1, libsystemd-dev 249.11 [Issue #401]([d62350d0])
- (LIB) bump anyhow 1.0.96, evtx 0.8.5, tempfile 3.17.1 ([44c1f3c1])
- (LIB) bump bstr 1.12.0 ([7d618a89])
- (LIB) bump libc=0.2.167 tempfile=3.14.0 termcolor=1.4.1 stringzilla=3.10.11 bstr=1.11.0 ([2bed8dca])
- (LIB) bump rand 0.9.0 ([4d14dbfc])
- (LIB) bump cargo update wasm-bindgen=0.2.95 crc-catalog=2.4.0 futures-util=0.3.31 bytes=1.8.0 ([07d25370])
- (LIB) bump crossbeam-channel 0.5.15 ([222152bf])
- (LIB) dependabot: bump anyhow from 1.0.86 to 1.0.91 ([4fc3720a])
- (LIB) dependabot: bump anyhow from 1.0.91 to 1.0.93 [PR #343] ([add02632])
- (LIB) dependabot: bump anyhow from 1.0.93 to 1.0.94 [PR #358] ([837d6f50])
- (LIB) dependabot: bump anyhow from 1.0.94 to 1.0.95 [PR #365] ([b3bfd4d9])
- (LIB) dependabot: bump anyhow from 1.0.96 to 1.0.97 [PR #381] ([7d9f16f8])
- (LIB) dependabot: bump bstr from 1.11.0 to 1.11.3 [PR #368] ([d12f1c15])
- (LIB) dependabot: bump bstr from 1.9.1 to 1.10.0 [PR #315] ([b95aded3])
- (LIB) dependabot: bump chrono from 0.4.38 to 0.4.39 [PR #359] ([103b4d23])
- (LIB) dependabot: bump chrono from 0.4.39 to 0.4.40 [PR #382] ([f1a7dd70])
- (LIB) dependabot: bump const_format from 0.2.32 to 0.2.33 [PR #330] ([21d83759])
- (LIB) dependabot: bump const_format from 0.2.33 to 0.2.34 [PR #357] ([6ad1d90b])
- (LIB) dependabot: bump crossbeam-channel from 0.5.13 to 0.5.14 [PR #361] ([4f3992cf])
- (LIB) dependabot: bump ctrlc from 3.4.4 to 3.4.5 [PR #329] ([7fab005a])
- (LIB) dependabot: bump ctrlc from 3.4.5 to 3.4.7 ([f858a3df])
- (LIB) dependabot: bump encoding_rs from 0.8.34 to 0.8.35 [PR #333] ([b2e639c0])
- (LIB) dependabot: bump evtx from 0.8.2 to 0.8.4 [PR #349] ([93db291b])
- (LIB) dependabot: bump filepath from 0.1.2 to 0.2.0 [PR #371] ([4a7a8d56])
- (LIB) dependabot: bump filetime from 0.2.23 to 0.2.25 [PR #332] ([9f9d7c3d])
- (LIB) dependabot: bump flate2 from 1.0.30 to 1.0.34 [PR #323] ([2008c9a6]) (jayvdb/main)
- (LIB) dependabot: bump flate2 from 1.0.34 to 1.0.35 [PR #346] ([0807ecf2])
- (LIB) dependabot: bump flate2 from 1.0.35 to 1.1.0 [PR #378] ([3b0e67c6])
- (LIB) dependabot: bump flate2 from 1.1.0 to 1.1.2 ([6def544d])
- (LIB) dependabot: bump itertools from 0.13.0 to 0.14.0 [PR #376] ([573b7739])
- (LIB) dependabot: bump libc from 0.2.167 to 0.2.168 [PR #356] ([967869ba])
- (LIB) dependabot: bump libc from 0.2.168 to 0.2.169 [PR #366] ([557301ae])
- (LIB) dependabot: bump libc from 0.2.169 to 0.2.170 [PR #380] ([e6048fb3])
- (LIB) dependabot: bump libc from 0.2.170 to 0.2.171 [PR #385] ([4c8fdcde])
- (LIB) dependabot: bump lru from 0.12.3 to 0.12.5 [PR #326] ([dbb4e4f3])
- (LIB) dependabot: bump lru from 0.12.5 to 0.13.0 [PR #372] ([69c80568])
- (LIB) dependabot: bump mimalloc from 0.1.43 to 0.1.44 [PR #389] ([1936fd62])
- (LIB) dependabot: bump mimalloc from 0.1.44 to 0.1.45 [PR #391] ([45390157])
- (LIB) dependabot: bump nix from 0.27.1 to 0.29.0 [PR #335] ([a0fb0c93])
- (LIB) dependabot: bump once_cell from 1.19.0 to 1.20.2 [PR #328] ([64055794])
- (LIB) dependabot: bump once_cell from 1.20.2 to 1.20.3 [PR #374] ([336a3d96])
- (LIB) dependabot: bump once_cell from 1.20.3 to 1.21.1 [PR #386] ([6f722ae7])
- (LIB) dependabot: bump once_cell from 1.21.1 to 1.21.3 [PR #388] ([e23c849b])
- (LIB) dependabot: bump openssl from 0.10.64 to 0.10.66 [PR #314] ([927d57b4])
- (LIB) dependabot: bump openssl from 0.10.66 to 0.10.70 [PR #369] ([b57cfb21])
- (LIB) dependabot: bump openssl from 0.10.70 to 0.10.72 [PR #390] ([86209a0a])
- (LIB) dependabot: bump phf from 0.11.2 to 0.11.3 [PR #375] ([b1bffe27])
- (LIB) dependabot: bump regex from 1.10.5 to 1.11.0 [PR #324] ([1620f7e7])
- (LIB) dependabot: bump regex from 1.11.0 to 1.11.1 [PR #336] ([646d8335])
- (LIB) dependabot: bump stringzilla from 3.10.11 to 3.11.0 [PR #355] ([797ecd1c])
- (LIB) dependabot: bump stringzilla from 3.10.7 to 3.10.10 [PR #348] ([8dcf0dcd])
- (LIB) dependabot: bump stringzilla from 3.11.0 to 3.11.3 [PR #367] ([7c335c2c])
- (LIB) dependabot: bump stringzilla from 3.11.3 to 3.12.3 [PR #383] ([9d245cad])
- (LIB) dependabot: bump stringzilla from 3.8.4 to 3.10.7 [PR #338] ([7cc45a6e])
- (LIB) dependabot: bump tar from 0.4.41 to 0.4.42 [PR #327] ([bc386776])
- (LIB) dependabot: bump tar from 0.4.42 to 0.4.43 [PR #337] ([afad2413])
- (LIB) dependabot: bump tar from 0.4.43 to 0.4.44 [PR #384] ([738bd507])
- (LIB) dependabot: bump tempfile from 3.10.1 to 3.13.0 [PR #325] ([63f7d592])
- (LIB) dependabot: bump tempfile from 3.14.0 to 3.16.0 [PR #370] ([72bb96ee])
- (LIB) dependabot: bump tempfile from 3.16.0 to 3.17.0 [PR #377] ([0b42a8a1])
- (LIB) dependabot: bump tempfile from 3.17.1 to 3.19.0 [PR #387] ([0ee66989])
- (LIB) dependabot: bump tikv-jemallocator from 0.5.4 to 0.6.0 [PR #313] ([988e563d])
- (LIB) dependabot: bump tokio from 1.38.0 to 1.44.2 ([dc35dfd3])
- (LIB) dependabot: bump unicode-width from 0.1.13 to 0.2.0 [PR #340] ([cb267457])
- (LIB) dependabot: bump zerovec-derive from 0.10.2 to 0.10.3 [PR #312] ([42169f49])
- (LIB) dependabot: bump zerovec from 0.10.2 to 0.10.4 [PR #311] ([250e9dee])
- (LOGS) add dtf7-20-LEVELS.log.zip ([aa0990ac])
- (RELEASE) bump version 0.7.76 ([cf3b3e66])
- (PROJECT) add .gitattributes ([9f0de805])
- (PROJECT) add .git_hooks/ ([d4ca06c1])
- (PROJECT) add .vscode/settings.json,launch.json ([19d3013d])
- (PROJECT) Cargo.toml profiles jemalloc mimalloc ([5dba3193])
- (PROJECT) update  releases/0.7.76/x86_64-unknown-linux-gnu ([c9852d2a])
- (RELEASE) add 0.7.76/aarch64-unknown-linux-gnu run Rasp Pi 3B ([ae811c9e])
- (RELEASE) add 0.7.76/x86_64-unknown-linux-gnu on Ubuntu 22 WSL ([d9c0b1bd])
- (RELEASE) update releases/0.7.76/x86_64-unknown-linux-gnu/ ([7b2037ef])
- (TOOLS) add cargo-deny.sh ([22852ea3])
- (TOOLS) add osv-scanner.sh ([9bbc405b])
- (TOOLS) cargo-test.sh only --version to check for nextest ([0b6faff2])
- (TOOLS) cargo-upgrade tweaks ([bdb9fb26])
- (TOOLS) compare-grep-sort remove perf measurements ([494e7732])
- (TOOLS) compare-log-mergers allow skipping toolong ([361e3bc1])
- (TOOLS) compare-log-mergers call lnav, refactor markdown table ([58289d1d])
- (TOOLS) compare-log-mergers.sh fix git log, check for lnav ([40775be4])
- (TOOLS) compare-log-mergers.sh hyperfine --warmup ([e1bfd835])
- (TOOLS) flamegraph.sh also try to search for 'perf' ([3a0ae4ba])
- (TOOLS) flamegraph.sh SVG has rust host target ([5fa013ec])
- (TOOLS) release-files.sh build by profile ([6745fb1d])
- (TOOLS) release-files.sh cache sudo creds ([c3058eb9])
- (TOOLS) release-files.sh NFC header comment ([b3db4313])
- (TOOLS) rust-workflow.sh update build mimalloc jemalloc ([fc872faf])

### Fixes

- (TEST) sd_journal_h.rs comment out all tests [Issue #401] ([c5a231bf])

## 0.7.75

_Released 2024-07-07_

_MSRV 1.70.0_

[0.7.74..0.7.75]

### Changes

- (BIN) --help mention journalctl datetimes different [Issue #101] ([20818ce5])
- (DOCS) README comparison table includes mimalloc, jemalloc ([21e28b30])
- (DOCS) README note HACK .tar file ([c27311e7])
- (DOCS) README note mimalloc Bus Error ([c463f6d8])
- (DOCS) README simplify section 'Building locally' ([d2973817])
- (DOCS) releases 0.7.73 bench heaptrack-syslog ([98b32b5d])
- (LIB) blockreader.rs read_block_FileGz simplify copying bytes ([a1a29564])
- (LIB) const more functions ([e5828040])
- (LIB) default blocksz 0x10000 ([81bb423f])
- (LIB) update mimalloc, tikv-jemallocator ([746530e4])
- (LIB) use slice_contains_X_2_memchr ([55b8777a])
- (LIB) dependabot: bump lazy_static from 1.4.0 to 1.5.0 [(#307)] ([98b366b5])
- (LIB) dependabot: bump memchr from 2.7.2 to 2.7.4 [(#308)] ([a1f18eab])
- (LOGS) add simple-12.*, dtf7-20-LEVELS.log.lz4 ([2cd25f56])
- (PROJECT) add releases/0.7.74/criterion/, README links ([35e678e9])
- (TOOLS) add compare-cat.sh ([e01919ff])
- (TOOLS) backup.sh check for 7z ([f4c8eccd])
- (TOOLS) compare-grep compare-log-mergers build s4 alloc variations ([b42e0f54])
- (TOOLS) release-files cargo bench --locked ([f22620fa])

### Fixes

- (LIB) handle gz bz2 stuttering decompression [Issue #305] ([9503d81f])
- (LIB) add bool drop_data [Issue #201] [Issue #305] ([09e71bb9])
- (TOOLS) fix undeclared $md3 ([3af9e554])

## 0.7.74

_Released 2024-06-22_

_MSRV 1.70.0_

[0.7.73..0.7.74]

### New

- (BENCH) refactor bench_slice_contains.rs to test stringzilla, jetscii, memchr ([2f87bf06])
- (BIN) Allocator per OS, summary allocator ([e8fec2c8])
- (BIN) use mimalloc and jemallocator ([86c59498])
- (BIN) skip build REGEX_DUR_OFFSET if passed empty String ([11d17c67])
- (BIN) --help explain tie-breaker logic ([53f8b909])
- (BIN) --help note decompression, naming req ([57d70052])
- (CI) add build 1.79.0 ([2bfbe631])
- (CI) add job_markup_link_checker ([eb212f72])
- (CI) add .mlc.toml, slow CI checks ([5eea0947])
- (CI) all commands used --locked ([b5433fd4])
- (CI) Windows NEXTEST_TEST_THREADS=2 [Issue #218] ([34fbb7d2])
- (LIB) add DTFSS_YmdHMSf pattern for setupacct.log [Issue #91] ([89dd70d5])
- (LIB) datetime.rs add regex patterns for DTFSS_BdHMSYZ YEAR + TZ variations ([10f4a0d3])
- (LIB) add stringzilla impl of 'slice_contains' funcs ([8f38523f])
- (LIB) allocators are features 'jemalloc' 'mimalloc' ([4be561af])
- (LIB) buffered writes, statistic flushed ([154c40d6])
- (LOGS) add gen-200-2-no-2.log ([e023b80a])
- (LOGS) add gen-200-2-yes-2.log ([6a91fb78])
- (LOGS) add Ubuntu16/kern.log.2.gz ([fbad9a40])
- (LOGS) add user.log.1 and variations ([2d7f5e25])
- (LOGS) modify FreeBSD13.1/devd.log ([a34a071d])
- (PROJECT) bench jetascii, memchr, stringzilla ([43d57d9e])
- (TEST) add test_print_summary_empty ([88519a06])

### Changes

- (BIN) use thread_local! instead of lazy_static! ([5b4d35c6])
- (CI) cross check targets *msvc on windows-latest ([666c3cf2])
- (CI) job_runs breakup into more steps ([ac1136b1])
- (CI) run cargo bench with bench-specific features ([bd49e68c])
- (CI) rust.yml rm x86_64-uwp-windows-msvc ([72aa6c8b])
- (DOCS) simplify docstrings, NFC adjust some comments ([4ecaaa46])
- (DOCS) README note more build requirements ([d36e9a2b])
- (DOCS) update docstring HTTP links to chrono version 0.4.38 ([3c5b0de8])
- (LIB) dependabot: bump tar from 0.4.40 to 0.4.41 [(#302)] ([1250c119])
- (LIB) dependabot: bump unicode-width from 0.1.12 to 0.1.13 [(#301)] ([ea7f856e])
- (LIB) bump regex 1.10.5 ([248718fd])
- (LIB) mv bindgen -> subprojects/bindgen ([74e0ee80])
- (LIB) summary print current_platform ([e43b6879])
- (LIB) use loop_unroll not stringzilla, leave stringzilla code as bench ([35d3c2eb])
- (TOOLS) add dev-dependencies cargo-msrv, cross ([e456f50c])
- (TOOLS) add files to compare-debug-release_logs.txt ([f00cf12c])
- (TOOLS) add profile valgrind, tweak profile flamegraph, call dot and gprof2dot.py ([84b0d3c9])
- (TOOLS) add release-files.sh, other tools adjustments ([c0c12172])
- (TOOLS) call markup link checker ([eeeeb164])
- (TOOLS) compare-grep-sort use hyperfine if available, dump markdown results ([c246c067])
- (TOOLS) compare-log-mergers.sh NFC header comment ([336ee321])
- (TOOLS) compressions.sh touch tars to reference ([93857c74])
- (TOOLS) flamegraph change RUSTFLAGS, check PROGRAM --version ([e8df9ba1])
- (TOOLS) flamegraph smaller text ([683cf870])
- (TOOLS) flamegraphs.sh add empty.log ([e64e0e47])
- (TOOLS) release-files.sh also compare-grep-sort compare-log-mergers ([b3c6f4d5])
- (TOOLS) rust-workflow adjust cross TARGETs ([123e5551])
- (TOOLS) rust-workflow cross targtets, profiles flamegraph valgrind ([47d40b49])
- (TOOLS) valgrind-callgrind also rm OUTSVG ([566b1b62])
- (TOOLS) valigrind-callgrind.sh use installed gprof2dot ([7c0f4bb1])

### Fixes

- (BIN) rm src/bin/mod.rs ([732f8e65])
- (CI) flamegraph.sh call with --preserve-env=PATH ([86342232])
- (DEBUG) fix debug in profiles flamegraph, valgrind ([ac0ce998])
- (DOCS) README fix missing link, simplify sentence ([5b4d5fc8])
- (LIB) datetime.rs constrain epoch range [Issue #304] ([459c931b])
- (LIB) tighten matches around CGP_YEAR [Issue #304] ([247a6338])
- (LIB) printers.rs fix print_journalentry not resetting color at end ([e3126e4d])

---

## 0.7.73

_Released 2024-06-02_

_MSRV 1.70.0_

[0.7.72..0.7.73]

### New

- (LIB) process .bz2 files [Issue #40] ([d28b437f])
- (TEST) printers.rs add test_PrinterLogMessage_print_journal, impl LoadLibraryError is_ok, is_err, is_prev_err ([43dccc0c])
- (TOOLS) add tools/compression.sh ([822f58cf])

### Changes

- (BIN) s4.rs FileErr print path ([19a1dd17])
- (BIN) s4.rs do not print error for FileErrNotSupported ([753c5c3a])
- (BIN) summary.rs tweak summary 'Run Time' print ([b1926711])
- (DOCS) common.rs fix Lz4 docstring ([1f0d2698])
- (DOCS) CHANGELOG fix missing links for long short hashes ([3a500a4a])
- (DOCS) README.md add badge CHANGELOG ([e8ae1ae5])
- (DOCS) src/ update docstrings printer/ filedecompressor.rs ([6ab5a2a5])
- (LIB) blockreader.rs larger buf for BZ2 LZ4 data extract ([b76c05d3])
- (PROJECT) add log dtf7-20-LEVELS-outoforder.log ([801ab5c8])
- (TOOLS) flamegraphs.sh use log gen-1000-3-foobar.log.bz2, journal.bz2 ([af851e2d])
- (TOOLS) flamegraphs.sh simplify to a for loop ([46d58621])
- (TOOLS) flamegraph.sh rm perf help ([4b115c29])

### Fixes

- (DOCS) changelog-link-gen.sh fix missing 7,8 char short hashes ([56844a29])
- (LIB) remove temporary files during early exit [Issue #297] ([ca375c23])

## 0.7.72

_Released 2024-05-30_

_MSRV 1.70.0_

[0.6.71..0.7.72]

### New

- (BIN) s4.rs check for FileEmpty FileTooSmall sooner [Issue #270] ([2160556d])
- (BIN) s4.rs -h README.md describe file name guessing, directory filter ([a7d3a394])
- (BIN) s4.rs summary includes run time, threads spawned, thread errors ([c3a4d2a8])
- (BIN) summary.rs print canonical path for archived files ([962de8e3])
- (LIB) src/ allow processing journal, evtx that are compressed, arch [Issue #291] [Issue #284] ([f2e5b4d1])
- (LIB) src/ add strace output datetime pattern ([13f1ef40])
- (LIB) datetime.rs match cv_debug.log [Issue #260] ([132a18a8])
- (LIB) journal,evtx tempfile when gz,lz4,tar,xz [Issue #284] ([cea1c6f8])
- (LIB) evtx,journal,readers.rs Modifed Time always Unix Epoch [Issue #284] ([6947cb9f])
- (LIB) src support lz4, refactor processing linear syslogs [Issue #128] [Issue #201] [Issue #283] [Issue #291] ([a223eeb4])
- (LIB) blockreader.rs syslinereader.rs add is_streamed_file() [Issue #182] [Issue #283] ([8a6af558])
- (LIB) filepreprocessor.rs use jwalk for WalkDir ([219e2cfc])
- (PROJECT) add logs simple-*log ([b5dd71e8])

### Changes

- (BIN) bin/ move --summary printing to src/printer/summary.rs ([35fd92ff])
- (BIN) s4.rs NFC more debug logs ([9d3ff3ec])
- (BIN) s4.rs summary prints bytes for blocks ([25060e8b])
- (BIN) s4.rs truncate summary for empty files ([e5c4c6ed])
- (BIN) summary.rs empty files do not print full --summary ([40172427])
- (CI) rust-workflow.sh cargo nextest ([93129e27])
- (CI) rust.yml add 10 cross targets ([5dd008a2])
- (CI) rust.yml add job_flamegraph ([2e95d616])
- (CI) rust.yml force flamegraph to succeed ([b875e10f])
- (CI) rust.yml job_cross_targets_macos swap moonrepo/setup-rust@v1 for Swatinem/rust-cache@v2 ([dfd5f6b8])
- (CI) rust.yml rm builds stable beta nightly ([d13dffa8])
- (CI) rust.yml set log file Modified Times before tests ([e30fd625])
- (CI) rust.yml update cross targets for MSRV 1.70.0 ([a9902ebf])
- (DOCS) tweak tagline 's4 aims...' ([b371ef11])
- (DOCS) README remove 'wtmp' hyperlink ([67f779c3])
- (DOCS) README.md note Windows VSC, simplify some formatting ([bc5ea85c])
- (DOCS) README.md NFC move build badges ([58d6ec9e])
- (DOCS) README tweak wording ([abdd5bac])
- (DOCS) README add badge lib.rs, badge downloads per version, rearrange badges ([145c45be])
- (DOCS) README update wiki link ([c98bf252])
- (DOCS) README tweak opening wording ([01c11d89])
- (DOCS) README tweak wording 'On Windows' ([2b7a60b9])
- (DOCS) README note error too many files open [Issue #270] ([e6d03e4d])
- (DOCS) README NFC tweak syntax per linter ([5eaa48ff])
- (DOCS) README add more example commands; utmp files, Windows logs [Issue #16] ([8ec6e240])
- (DOCS) README update Issue links ([b4e9cf24])
- (DOCS) README simplify wording in sections 'software examples' ([ea56625a])
- (DOCS) README tweak wording at 3 ([236802fa])
- (DOCS) README opening paragraph... again! ([81959bd1])
- (DOCS) README tweak opening paragraph ([ee9d6c0f])
- (DOCS) README shield downloads highlight 'Downloads all time' ([518514e9])
- (LIB) blockreader.rs add read_block_last [Issue #283] ([6d892736])
- (LIB) blockreader.rs attempt to parse more of the XZ header [Issue #12] [Issue #283] ([5750cf34])
- (LIB) blockreader.rs NFC comment note Issue #293 [Issue #293] ([4236304b])
- (LIB) filepreprocessor.rs handle empty archive file sooner, add tests ([8265bff2])
- (LIB) filepreprocessor_tests.rs NFC prefer macro e_err! ([a8d5902d])
- (LIB) syslinereader.rs add file_offset_last [Issue #283] ([f5d9f894])
- (LIB) refactor `enum FileType` and file type guesstimating, remove Mimeguess [Issue #15] [Issue #257] [Issue #285] ([6eff5c55])
- (LIB) refactor path_to_filetype for archivetype for all file types [Issue #257] [Issue #258] ([dd3637a7])
- (LIB) filepreprocessor.rs refactor process_path_tar [Issue #7] [Issue #14] [Issue #16] [Issue #285] ([2c70ee35])
- (LIB) blockreader.rs bad subpath returns InvalidInput ([2ff222ea])
- (LIB) Cargo.lock update libc=0.2.155 ([3ada5b0a])
- (LIB) Cargo.toml bump MSRV 1.70.0 ([29a55582])
- (LIB) common.rs NFC simplify some match FileType statements ([da392e6d])
- (LIB) datetime.rs NFC add TODO for more DTPD! with RP_LEVELS ([0522f1eb])
- (LIB) datetime.rs regex RP_LEVELS also match 'PANIC' ([ecbdca19])
- (LIB) filepreprocessor.rs map PermissionDenied to FileErrNoPermissions ([7ca59ef4])
- (LIB) src/ NFC debug print lazy_static init ([a0697763])
- (LIB) src/ NFC use common FIXEDOFFSET0 ([ae5bbaec])
- (LIB) src/ replace ::static_assertions::const_assert! with ::const_format::assertcp! ([016e8b73])
- (LIB) dependabot: bump crossbeam-channel from 0.5.12 to 0.5.13 [(#295)] ([26979a36])
- (LIB) dependabot: bump flate2 from 1.0.28 to 1.0.30 [(#289)] ([bc58af34])
- (LIB) dependabot: bump itertools from 0.12.1 to 0.13.0 [(#296)] ([c2a2fc73])
- (LIB) dependabot: bump libc from 0.2.153 to 0.2.154 [(#292)] ([b63e8480])
- (LIB) dependabot: bump unicode-width from 0.1.11 to 0.1.12 [(#290)] ([a2ebe40b])
- (PROJECT) bump version 0.6.72 ([a64be8e9])
- (PROJECT) bump version 0.7.72, overwrites 0.6.62 ([8d6eed86])
- (PROJECT) .gitignore .virtualenv/ ([43d4cca5])
- (PROJECT) add archive tars of wtmp and journal ([c6221743])
- (PROJECT) add log files dtf7_\* empty\* many variations ([fb822555])
- (PROJECT) Cargo.toml defalt-run=s4 ([7bdfa4df])
- (PROJECT) logs/ add MacOS13 logs, compressed variations ([808ef682])
- (TEST) filepreprocessor_tests.rs add more tests of oddities and archive combos ([91d33d02])
- (TEST) src/ add test test_new_*Reader_no_file_permissions ([867a6fe2])
- (TOOLS) compare-current-and-expected update after bc972fb9 ([2fef7441])
- (TOOLS) compare-current-and-expected update.sh always run ./tools/log-files-time-update.sh ([7e422759])
- (TOOLS) flamegraph.sh NFC $PERF dump more info ([f6b28c8b])
- (TOOLS) flamegraph.sh title small, add version flamegraph rustc ([e8e7c9ca])
- (TOOLS) log-files-time-update.sh MacOS tools compatibility ([57a149c6])
- (TOOLS) cargo-test.sh debug print nextest list ([4a79d15d])
- (TOOLS) compare-current-and-expected add all variations of log file dtf2-2 ([6f5862e3])
- (TOOLS) compare-current-and-expected improve summary of diffs ([8633d667])
- (TOOLS) compare-grep-sort.sh allow user set $FILES ([cb9a91ed])
- (TOOLS) compare-log-mergers force install to expected version, update versions ([0f822c54]) (origin/main, origin/HEAD)
- (TOOLS) compare-log-mergers.sh note git commit ([a8a89bd2])
- (TOOLS) flamegraphs.sh add flamegraph-syslog-no-matches.svg ([b8a8ea75])
- (TOOLS) flamegraphs.sh bump FREQ, echo more ([6f4a2557])
- (TOOLS) rust-workflow call log-files-time-update.sh ([3d962df8])
- (TOOLS) compare-debug-release add more files ([d3313e4c])
- (TOOLS) compare-current-and-expected/update.sh, add various empty and small logs ([54b7d896])

### Fixes

- (BIN) s4.rs minor --help fixes ([aeed87f6])
- (BIN) summary.rs fix summary first line printed in wrong color ([6eb17c7c])
- (BIN) summary.rs datetime consistent printing ([d4060a99])
- (DOCS) src/ various docstring fixes and tweaks ([8e46d08e])
- (DOCS) README fix typo in link ([dc170648])
- (DOCS) README fix logmerge does support pcap ([5239fb75])
- (TEST) datetime_tests.rs fix test_DATETIME_PARSE_DATAS_test_cases missing tests ([d9c3bf3b])

---

## 0.6.71

_Released 2024-04-16_

_MSRV 1.67.1_

[0.6.70..0.6.71]

### New

- (BUILD) Cargo.toml categories add 'date-and-time' ([7d9bf20b])
- (LIB) compile regex on-demand [Issue #84] ([5ae0f033])
- (LIB) datetime.rs add common JSONL patterns Timestamp|Datetime ([75d48b0e])
- (PROJECT) add logs/MacOS12.6/ ([995bfb07])
- (PROJECT) add CrowdStrike logs ([fd1a1da0])

### Changes

- (BIN) bin.rs change --help 'syslog' to 'log' ([ba75da98])
- (CI) chmod +x ./tool/cargo-publish,outdated ([bf7596e3])
- (DOCS) README move badges up ([6d67cdbe])
- (DOCS) README user accounting link to docs.rs, not code ([91df1d01])
- (DOCS) README link to comparison table instead of Job ([a6099bc3])
- (DOCS) README add section stargazers ([13b6039f])
- (DOCS) README add section commercial logs Mac OS 12.6 ([b715be55])
- (DOCS) README badge use MSRV badge, link to Cargo.toml ([b199791b])
- (DOCS) README oxford commas in opening ([0da5f9d5])
- (DOCS) README install advice uses '--locked' ([d6caa25b])
- (LIB) datetime.rs do not panic if failed u8_to_str ([7a22f9f4])
- (LIB) dependabot: bump bstr from 1.8.0 to 1.9.1 [(#279)] ([475529ee])
- (LIB) dependabot: bump chrono from 0.4.35 to 0.4.37 [(#275)] ([2f16d924])
- (LIB) dependabot: bump chrono from 0.4.37 to 0.4.38 [(#281)] ([54c3e09c])
- (LIB) dependabot: bump crossbeam-channel from 0.5.8 to 0.5.12 ([d23220a2])
- (LIB) dependabot: bump dlopen2 from 0.6.1 to 0.7.0 [(#274)] ([42ccefd3])
- (LIB) dependabot: bump encoding_rs from 0.8.33 to 0.8.34 [(#282)] ([25447169])
- (LIB) dependabot: bump evtx from 0.8.1 to 0.8.2 ([84abef39])
- (LIB) dependabot: bump filetime from 0.2.22 to 0.2.23 ([b1acadcc])
- (LIB) dependabot: bump memchr from 2.6.4 to 2.7.2 [(#272)] ([5de9a326])
- (LIB) dependabot: bump memoffset from 0.9.0 to 0.9.1 [(#280)] ([9ee5e796])
- (LIB) dependabot: bump regex from 1.10.2 to 1.10.4 ([6408589c])
- (LIB) dependabot: bump tempfile from 3.8.1 to 3.10.1 [(#278)] ([0664eebc])
- (LIB) dependabot: bump test-case from 3.2.1 to 3.3.1 ([8821b40c])
- (LIB) dependabot: bump walkdir from 2.4.0 to 2.5.0 ([8bf2c812])
- (PROJECT) add releases/0.6.71 ([d8af716b])
- (PROJECT) add MacOS11 log and crash files ([145efc2f])
- (TEST) datetime.rs add 5 tests using different brackets ([2340084f])
- (TOOLS) backup.sh fix flamegraph file listed twice ([12871b74])
- (TOOLS) compare-current-and-expected fix stderr differing commands ([ab11dae7])
- (TOOLS) compare-current-and-expected fix stderr comparison to not truncate entire file, run update.sh ([d7d264ba])
- (TOOLS) flamegraphs.sh add flamegraph-help.svg ([9e2c2533])
- (TOOLS) flamegraph.sh --no-inline ([966c3366])

### Fixes

- (LIB) printers.rs fix unset color on first print [Issue #258] ([567394fb])

---

## 0.6.70

_Released 2024-03-24_

_MSRV 1.67.1_

[0.6.69..0.6.70]

### New

- (CI) rust.yml cargo package ([cd6584d7])
- (CI) rust.yml add summary step for job_rust_msrv_os ([a0690573])
- (LIB) datetime.rs add pattern 'year then timezone' DTFSS_BdHMSYZ ([91c3cfb2])
- (LIB) datetime.rs add flask web server datetime pattern ([7f700e64])
- (PROJECT) add MacOS11/asl/*.asl files ([8ba2106c])
- (PROJECT) add logs/Windows11Pro/{Local,Temp} ([e790e681])
- (PROJECT) add log flask/server.log ([85bb57fb])
- (PROJECT) add releases/0.6.70/ files ([5f8a63e4])
- (TOOLS) add tools/cargo-publish.sh ([aa4cf9d3])

### Changes

- (BIN) rename bin.rs -> s4.rs ([ad557228])
- (LIB) printers.rs improve colors for dark background ([cd52d9d6])
- (LIB) bump chrono 0.4.35 ([89209f92])
- (LIB) dependabot: bump rangemap from 1.4.0 to 1.5.1 (#254) ([8d487876])
- (LIB) dependabot: bump lru from 0.12.0 to 0.12.3 (#252) ([fada0fc4])
- (LIB) dependabot: bump utf8_iter from 1.0.3 to 1.0.4 (#253) ([f271285d])
- (TOOLS) rust-workflow.sh clean first ([f05b395b])

---

## 0.6.69

_Released 2024-03-21_

[0.6.68..0.6.69]

_MSRV 1.67.1_

### New

- (CI) rust.yml enable cross targets ([8bee504d])
- (DOCS) syslogprocessor.rs docstring about "syslog file" ([c618a199])
- (DOCS) datetime.rs NFC docstring Result_Filter_DateTime1 ([4c7b6f1a])
- (LIB) refactor for fixedstruct [Issue #100] [Issue #108] [Issue #109] [Issue #121] [Issue #171] [Issue #217] ([1aed8624])
- (LIB) replace uapi for utmpx processing [Issue #100] [Issue #108] [Issue #109] [Issue #121] [Issue #171] [Issue #217] ([e8406c56])
- (LIB) datetime.rs,bin add DateTimeParseDatasCompiledCount summary stat ([9a238499])
- (PROJECT) add log CT-Log 2023-09-04 05-44-16.csv ([48c5a5e4])
- (PROJECT) add log ./logs/other/tests/dtf9c-23-12x2.log.xz ([0c1f5655])
- (PROJECT) add logs MacOS11 MacOS12.6 NetBSD9.3 FreeBSD13.1 OpenBSD7.2 ([7d3305cc])
- (PROJECT) add logs/Android9 ([c9a5de8f])
- (PROJECT) add logs gen-1000-3-foobar.log.{tar,xz} ([8ec4d012])
- (PROJECT) add logRFC-5424-Z.log ([a08cc1de])
- (PROJECT) add log ISO8601-YYYY-DD-MMTHH-MM-SS_2.log ([2d1541c4])
- (PROJECT) add releases/0.6.69/flamegraph*,compare-log-mergers.txt ([34ef7167])
- (PROJECT) add requirements.txt ([bd29d11b])
- (TEST) blockreader_tests.rs mark should_panic [Issue #201] ([a746b065])
- (TEST) add datetime pattern dtf14a ([70a30a2b])
- (TEST) datetime_tests.rs add test cases to test_slice_contains_X_2 ([a468ffad])
- (TOOLS) add tools/flamegraphs.sh ([b0ade003])
- (TOOLS) add tools/cargo-udeps.sh ([df2bcdef])

### Changes

- (BIN) bin.rs NFC debug print syslogprocessor stages more clearly ([9bfb3ca7c5eeeaa20a9a5e6071206a98a3e7fa17]
- (BIN) bin.rs ordered invalid results ([e75daad2])
- (BIN) bin.rs do not print after printing error [Issue #241] [Issue #250] ([157d54a4])
- (CI) rust.yml NFC move log-files-time-update.sh ([25b35ee8])
- (CI) rust.yml build 1.76.0, NFC vertical declaration ([baff6da9])
- (CI) bump MSRV 1.67.1 ([505280af])
- (CI) rust.yml pin yamllint==1.35.1 ([f32d7650])
- (CI) rust.yml clippy uses default profile ([89f67f30])
- (CI) rust.yml use moonrepo/setup-rust@v1 ([cef66e50])
- (CI) rust.yml add job_test_wasm ([aec8cebc])
- (CI) rust.yml comment job_test_wasm [Issue #171] ([da6f91fa])
- (CI) rust.yml use download-artifact@v4 ([8c23fd05])
- (CI) rust.yml NFC adjust S4_TEST_FILES indent ([92b97540])
- (CI) rust.yml update setup-python@v5 ([f46be6a0])
- (CI) rust.yml update upload-artifact@v4 ([e4f8059e])
- (CI) rust.yml add more S4_TEST_FILES ([8f8e8b7a])
- (CI) rust.yml update checkout@v4 ([7432b7c5])
- (CI) rust.yml run for Cargo.lock ([6a327d18])
- (CI) rust.yml correctly override rust version ([068a0a28])
- (CI) rust.yml add job_cross_targets ([d1dfe28a])
- (CI) rust.yml remove tar logs.tgz upload ([ef7d59a6])
- (CI) rust.yml test on Windows, Mac OS, binstall nextest [Issue #202] [Issue #219] [Issue #218] ([659c824e])
- (CI) rust.yml job grcov uses rust 1.67.0 [Issue #170] ([78579a91])
- (CI) rust.yml uploaded artifacts have specific name ([007f752f])
- (CI) rust.yml remove job_grcov ([b2a0ab53])
- (CI) rust.yml use script compare-debug-release.sh ([35914891])
- (CI) rust.yml tweak `on` for `pull_request` ([616bef16])
- (CI) rust.yml use newer rust for job llvm_cov ([f363682c])
- (CI) rust.yml use script compare-debug-release.sh ([231fcc46])
- (CI) rust.yml tweak `on` for `pull_request` ([24c21a15])
- (CI) rust.yml msrv add 1.72.0, drop 1.67.1 ([f968b462])
- (CI) rust.yml use newer rust for job llvm_cov ([69767ad6])
- (CI) rust.yml remove call cargo-llvm-cov-run.sh ([2ff0d197])
- (CI) rust.yml remove tar logs.tgz upload ([9fbc6318])
- (CI) rust.yml test on Windows, Mac OS, binstall nextest ([2e10dd1c])
- (CI) rust.yml NFC move log-files-time-update.sh ([00171bbd])
- (DOCS) README add more sections to examples in "logging chaos" ([071536e7a77bc4fefcbba6874040d0a4c77ce4b])
- (DOCS) README fix missing link to local file ([c13d0820])
- (DOCS) README update section HACKS ([c4f86ec5])
- (LIB) update chrono 0.4.28 ([0021f057])
- (LIB) bump chrono 0.4.27, add MINUS SIGN tests ([498ad5f4])
- (LIB) bump MSRV 1.67.0, lock clap 4.1.0 ([4db00567])
- (LIB) Cargo.toml criterion 0.5.1 ([2f1fc58e])
- (LIB) Cargo.toml clap downgrade 4.1.0 to support MSRV 1.66.0 ([afc0dab5])
- (LIB) Cargo.toml remove dependency `backtrace` ([b339c881])
- (LIB) Cargo remove flamegraph as a dependency ([f08dd9c4])
- (LIB) dependabot: bump bstr from 1.6.0 to 1.7.0 [(#199)] ([cff93366])
- (LIB) dependabot: bump bstr from 1.7.0 to 1.9.0 [(#237)] ([a09e9f66])
- (LIB) dependabot: bump clap from 4.3.12 to 4.3.19 ([2c1a38ea])
- (LIB) dependabot: bump clap from 4.3.19 to 4.3.21 [(#172)] ([1c6ccb18])
- (LIB) dependabot: bump clap from 4.3.21 to 4.3.23 [(#175)] ([0f4521cd])
- (LIB) dependabot: bump clap from 4.3.23 to 4.4.6 [(#193)] ([50e3c523])
- (LIB) dependabot: bump dlopen2 from 0.5.0 to 0.6.1 [(#166)] ([f4203933])
- (LIB) dependabot: bump encoding_rs from 0.8.32 to 0.8.33 [(#177)] ([bdafe96f])
- (LIB) dependabot: bump filetime from 0.2.21 to 0.2.22 ([0c6ba914])
- (LIB) dependabot: bump flate2 from 1.0.26 to 1.0.27 [(#176)] ([53e8a759])
- (LIB) dependabot: bump itertools from 0.11.0 to 0.12.1 [(#246)] ([e3755777])
- (LIB) dependabot: bump libc from 0.2.149 to 0.2.153 [(#247)] ([24691784])
- (LIB) dependabot: bump lru from 0.11.0 to 0.12.0 [(#197)] ([08fed12c])
- (LIB) dependabot: bump memchr from 2.5.0 to 2.6.4 [(#195)] ([8123ef18])
- (LIB) dependabot: bump nix from 0.26.2 to 0.27.1 [(#178)] ([68583d84])
- (LIB) dependabot: bump regex from 1.9.1 to 1.9.3 [(#169)] ([ddc2a803])
- (LIB) dependabot: bump regex from 1.9.3 to 1.9.4 [(#181)] ([eb5a1c33])
- (LIB) dependabot: bump regex from 1.9.4 to 1.10.2 [(#203)] ([a01e57d2])
- (LIB) dependabot: bump rustix from 0.36.9 to 0.36.16 [(#200)] ([3e70a605])
- (LIB) dependabot: bump tar from 0.4.39 to 0.4.40 [(#173)] ([e3b58f92])
- (LIB) dependabot: bump tempfile from 3.6.0 to 3.7.0 ([5543ff72])
- (LIB) dependabot: bump tempfile from 3.7.0 to 3.7.1 [(#168)] ([46c1502f])
- (LIB) dependabot: bump tempfile from 3.7.1 to 3.8.0 [(#174)] ([5d5ce99c])
- (LIB) dependabot: bump webpki from 0.22.0 to 0.22.4 [(#204)] ([6671e40e])
- (LIB) dependabot: bump zerocopy from 0.7.25 to 0.7.32 [(#249)] ([f3749f5b])
- (LIB) src/ remove debug prints with addresses [Issue #213] ([fdc1899a])
- (LIB) fixedstruct.rs swap numtoa for lexical ([4e32f5ce])
- (LIB) fixedstruct.rs use i64 to satisfy cross targets ([6c1954e7])
- (LIB) src/ NFC code comment updates, docstring links ([bd9b494e])
- (LIB) read gz files sequentially [Issue #182] ([02261be4])
- (LIB) blockreader.rs docstring tweaks BlockReader ([4b80fa51])
- (LIB) blockreader.rs don't panic for missing Block, return Error ([7f9535da])
- (LIB) blockreader.rs NFC debug print mtime() [Issue #245] ([daded23c])
- (LIB) datetime.rs remove to_byte_array; use built-in const bytes ([699015e0])
- (LIB) syslinereader.rs InvalidInput error tells short line length ([6c825065])
- (LIB) syslogprocess.rs stage runtime check is debug-only ([463d93f3])
- (LIB) syslogprocessor.rs Year min_diff is global static ([1e552fe9])
- (LIB) syslinereader.rs use stable feature `pop_last` ([4268bc6b])
- (LIB) syslinereader.rs fix mixup summary regex_captures_attempted get_boxptrs_singleptr ([f94d2a0d])
- (LIB) syslogprocessor.rs utmpxreader.rs set_error prints error ([4370adae])
- (LIB) src/ cargo fmt suggestions ([8fc41818])
- (LIB) journalreader.rs explicit static ([97b69b23])
- (LIB) codespell fixes ([8f0eee74])
- (LIB) src/ tweak drop_block return value logic ([c240b82c])
- (LIB) sd_journal_h.rs allow(clippy::all) ([487138fc])
- (LIB) line.rs sysline.rs debug print no address ([e083e73e])
- (LIB) src/ refactor error messages: include paths ([c83e433c])
- (PROJECT) logs truncate large logs, rm very large .journal ([9c4e8e9b])
- (TEST) datetime_tests.rs use test_case::test_matrix ([a5ad717d])
- (TEST) tests strategic skip journalreader utmpx on macos windows ([5618d051])
- (TEST) syslinereader_tests.rs fix test ntf_gz_8byte_fpath ([58d9d79a])
- (TEST) src/ fix code and tests that fail on target_os="windows" [Issue #202] ([a7cfd10e])
- (TEST) tests/mod.rs utmpxreader_tests, utmpx_tests not on "macos" [Issue #217] ([2c8bf0f1])
- (TOOLS) log-files-clean*sh less aggressive cleaning ([2c34a47f])
- (TOOLS) flamegraph.sh modify SVG title with more info ([04558a30])
- (TOOLS) flamegraph.sh xmllint ([2a2d372e])
- (TOOLS) compare-*.sh allow colordiff ([1d6bc01b])
- (TOOLS) hexdump.py fix offset option, allow hex ([fcfc7294])
- (TOOLS) backup.sh allow exported BACKUPDIR ([50cc3c2d])
- (TOOLS) backup.sh backup flamegraph* releases/ ([4a43a7b3])
- (TOOLS) compare-current-and-expected use individual logs [Issue #213] ([d0736743])
- (TOOLS) compare-current-and-expected check file hashes, fix stdin [Issue #206] ([34b75c5b])
- (TOOLS) compare-current-and-expected comment "ERROR: " [Issue #224] ([c7ec4ebf])

## 0.6.68

_Released 2023-07-22_

[0.6.67..0.6.68]

_MSRV 1.67.0_

### New

- (BIN) bin.rs parse -a -b fractional seconds %6f [Issue #145] ([54b08603])
- (BIN) bin.rs better pattern coverage for -a -b ([05d974c6])

### Changes

- (LIB) dependabot: bump clap from 4.3.10 to 4.3.12 ([844c1e06])
- (LIB) dependabot: bump tar from 0.4.38 to 0.4.39 ([26daee68])
- (LIB) dependabot: bump const-str from 0.5.5 to 0.5.6 ([2f9a434c])
- (LIB) dependabot: bump lru from 0.10.0 to 0.11.0 ([1e3e789b])
- (LIB) dependabot: bump bstr from 1.5.0 to 1.6.0 ([6f36d21e])
- (LIB) dependabot: bump itertools from 0.10.5 to 0.11.0 ([d205c419])
- (LIB) dependabot: bump regex from 1.8.4 to 1.9.1 ([a0edb157])
- (LIB) bump si_trace_print to 0.3.10 ([0997ecdc])
- (BIN) bin.rs adjust --help wording, spacing ([93a58cc0])
- (DOCS) README update --help ([9948f0a9])
- (DOCS) README add color to some other logs ([3805df7d])
- (DOCS) README coveralls.io badge ([7d97c64e])
- (CI) rust.yml nextest all early on ([b8a8c34b])
- (CI) rust.yml consistent toolchains, use MSRV ([7daa34c8])
- (CI) rust.yml add coverage llvm-cov to coveralls ([d0f4166b])
- (BIN) bin.rs NFC clippy recommendations ([3aaddac4])
- (BIN) bin.rs NFC fixup test case names ([4ac84307])

## 0.6.67

_Released 2023-07-08_

[0.6.66..0.6.67]

_MSRV 1.66.0_

### New

- (BIN) bin.rs parse -a -b fractional seconds %3f [Issue #145] ([4b328c5a])

### Changes

- (LIB) dependabot: bump bstr from 1.4.0 to 1.5.0 ([f5bd4e3a])
- (LIB) dependabot: bump chrono from 0.4.24 to 0.4.25 ([bea60aae])
- (LIB) dependabot: bump chrono from 0.4.25 to 0.4.26 ([9522093d])
- (LIB) dependabot: bump clap from 4.2.7 to 4.3.0 ([87a884bb])
- (LIB) dependabot: bump clap from 4.3.0 to 4.3.8 ([7cc5fbcc])
- (LIB) dependabot: bump clap from 4.3.8 to 4.3.10 ([d340bd29])
- (LIB) dependabot: bump const-str from 0.4.3 to 0.5.4 ([56adf881])
- (LIB) dependabot: bump const-str from 0.5.4 to 0.5.5 ([2030a739])
- (LIB) dependabot: bump const_format from 0.2.30 to 0.2.31 ([861be671])
- (LIB) dependabot: bump dlopen2 from 0.4.1 to 0.5.0 ([eeb3632b])
- (LIB) dependabot: bump filetime from 0.2.20 to 0.2.21 ([11c9d0bb])
- (LIB) dependabot: bump flamegraph from 0.6.2 to 0.6.3 ([addbc642])
- (LIB) dependabot: bump libc from 0.2.144 to 0.2.147 ([53596672])
- (LIB) dependabot: bump lzma-rs from 0.2.0 to 0.3.0 ([d31c1456])
- (LIB) dependabot: bump phf from 0.11.1 to 0.11.2 ([ebf4fd3f])
- (LIB) dependabot: bump regex from 1.8.1 to 1.8.3 ([f4cdd62e])
- (LIB) dependabot: bump regex from 1.8.3 to 1.8.4 ([ee4c9d0c])
- (LIB) dependabot: bump tempfile from 3.5.0 to 3.6.0 ([ff0f4695])
- (LIB) dependabot: bump test-case from 2.2.2 to 3.1.0 ([2e772970])
- (TOOLS) flamegraph.sh use `--profile flamegraph` ([d4f5e0af])
- (TEST) bin.rs add 3 more test cases for datetime parsing ([943619be])
- (CI) rust.yml update grcov flags ([707d4729])
- (CI) rust.yml upload binary for all platforms at MSRV [Issue #152] ([6ff633cc])
- (CI) rust.yml expand version matrix ([597c0807])
- (CI) rust.yml NFC reword job name ([1677328e])
- (CI) rust.yml add job codecov.yml validation ([a4eabe11])
- (CI) rust.yml fix upload if statements ([a7e93c60])
- (CI) rust.yml update action rust-toolchain@stable ([40e428d6])

### Fixes

- (BIN) bin.rs fix parsing of example input strings ([99b8c469])
- (LIB) summary.rs clippy fix unused warning BLOCKSZ_MIN ([e317c87e])
- (LIB) src/ cargo clippy fix missing ymdhmsl ([b50ce993])
- (CI) rust.yml relax yamllint line-length [Issue #120] ([01b80ffa])

---

## 0.6.66

_Released 2023-05-13_

[0.6.65..0.6.66]

_MSRV 1.66.0_

### Changes

- (LIB) (TESTS) (TOOLS) improve tests and logs for `BlockReader::mtime()` and files that are `FileType::Unknown` ([b2530a58]) ([f5abc7a1])

### Fixes

- (LIB) blockreader.rs fix panic on `FileType::Unknown` in mtime() ([307c86c2])

---

## 0.6.65

_Released 2023-05-11_

[0.6.64..0.6.65]

_MSRV 1.66.0_

### New

- (CI) rust.yml add job_yamllint [Issue #120] ([8bdeafdd]) ([6e688085])
- (TEST) add logs dtf13a.log dtf13b.log dtf13c.log dtf13d.log ([c3d0621b])

### Changes

- (LIB) datetime.rs allow lenient matching timezone ([fe422d64])
- (TOOLS) compare-current-and-expected.sh --prepend-dt-format='%Y%m%dT%H%M%S.%9f' ([e5e7f45a])

---

## 0.6.64

_Released 2023-05-09_

[0.6.63..0.6.64]

_MSRV 1.66.0_

### New

- (BIN) bin.rs print datetimes as UTC dimmed ([e51c30f1])
- (LIB) parse Red Hat Audit logs, parse epochs [Issue #112] ([0fceba27]) ([69ef9f7b])
- (BIN) (LIB) src/ Summary statistics for regex capture attempts ([281adc0d])
- (TOOLS) yamlllint.yml add rules for yamllint.sh ([fa5ff732])

### Changes

- (LIB) dependabot: bump clap from 4.2.1 to 4.2.7  ([33447dd1])
- (LIB) dependabot: bump crossbeam-channel from 0.5.7 to 0.5.8 ([06640e32])
- (LIB) dependabot: bump libc from 0.2.141 to 0.2.144 ([8e98a8f3])
- (LIB) dependabot: bump lru from 0.8.1 to 0.10.0 ([75f7c9fa])
- (LIB) dependabot: bump regex from 1.7.1 to 1.8.1 ([66414e9d])
- (LIB) dependabot: bump tempfile from 3.4.0 to 3.5.0  ([210f01c3])
- (BIN) bin.rs refactor channel data passing  [Issue #104] [Issue #60] ([0ea897a7])
- (LIB) journalreader.rs efficient key tracking in `next_short` [Issue #84] ([78106320])
- (LIB) (BIN) miscellaneous codepspell fixes ([524e269e]) ([0c6af5d6]) ([5bb8a5d1]) ([860b213f]) ([af93d662])
- (LIB) datetime.rs remove duplicate enum `DTFS_Hour::Hs` ([cc1cb8aa])
- (LIB) syslogprocessor.rs add `blockzero_analysis_bytes` ([cdd64dfe])
- (TEST) datetime_test.rs test builtins using slice ([b8989f3f])
- (TOOLS) compare-current-and-expected common args, more args ([d395d94c])
- (TOOLS) compare-debug-release.sh pass more args to s4 ([dfab1e70])
- (CI) github add `dependabot.yml` ([877177bc])

### Fixes

- (LIB) syslinereader.rs fix sort of indexes ([f1baa4d5])
- (LIB) datetime.rs fix too short slice recommendation ([2af24cbf])

---

## 0.6.63

_Released 2023-05-01_

[0.6.62..0.6.63]

_MSRV 1.66.0_

### New

- (LIB) datetime.rs match single-digit days and hours [Issue #98] ([830dbbd5])

### Changes

- (LIB) datetime.rs use compile-time map timezone names to values [Issue #84] ([98ebe687])
- (LIB) syslinreader.rs pre-create FixedOffset strings [Issue #84] ([3b950014])
- (LIB) datetime.rs support RFC 2822 timezone "UT" ([dd8248c3])

### Fixes

- (LIB) datetime.rs fix missing and out-of-order timezones ([cf9153bc])
- (LIB) src/ allow `FileType::Unknown`, fix panic on Unknown ([2cb0412d])

---

## 0.6.62

_Released 2023-04-27_

[0.6.61..0.6.62]

_MSRV 1.66.0_

### Fixes

- (LIB) fails to build on Debian 11 aarch64 [Issue #108] ([67cb45a4])

---

## 0.6.61

_Released 2023-04-23_

[0.6.60...0.6.61]

_MSRV 1.66.0_

### New

- (LIB) filepreprocessor.rs handle trailing junk like "~" ([23dfeb32])
- (TEST) add `summary()` tests for various readers ([57e2a4d7]) ([8be5e30f]) ([efb694df])
- (PROJECT) add logs `SIH.20230422.034724.362.1.etl` `FreeBSD13_utx.lastlogin.utmp` ([ce8518ea]) ([ccd4f0c9])

### Changes

- (TOOLS) journal_print.py allow user passing fields ([3c5a18a4])

### Fixes

- (LIB) s4 panics on some .etl files [Issue #105] ([5f77a0f7])

---

## 0.6.60

_Released 2023-04-22_

[0.5.59...0.6.60]

_MSRV 1.66.0_

### New

- (LIB) (BIN) (DOCS) (TOOLS) (PROJECT) (BUILD) systemd journal parsing [Issue #17] ([3a6eac6b])

---

## 0.5.59

_Released 2023-03-31_

[0.5.58...0.5.59]

_MSRV 1.66.0_

### Changes

- (LIB) Efficiency hack EZCHECK12D2 ([6f7831f1]) ([08738c41])
- (BIN) bin.rs summary spacing, linerize About section ([dc7b7c27])
- (TOOLS) compare-current-and-expected-update checks stdout and stderr ([f5f2be2d]) ([cf91c1d2]) ([85d5ba25]) ([f5bf771e]) ([52777c1e]) ([3df00ac9]) ([05f04e30]) ([81f94b8b])
- (CI) split up jobs into more parallel jobs [Issue #63] ([2edda450]) ([c1262d43])

### Fixes

- (LIB) datetime.rs fix one-off error in slice_contiains_X_2 ([d159553e])
- (DOCS) src/ fix various docs, pub some functions ([476ed604])

---

## 0.5.58

_Released 2023-03-29_

[0.4.57...0.5.58]

_MSRV 1.66.0_

### New

- (BIN) Allow user-passed timezone for prepended datetime [Issue #27] ([630b8ce9])
- (PROJECT) add logs/programs/{AWS,Microsoft IIS,apache} ([ee4515f1])
- (LIB) Parse RFC 2822 [Issue #29] ([38d1c473])

### Changes

- (TEST) add test test_PrinterLogMessage_print_evtx ([e6931ed9])
- (BUILD) clap 0.4.21 ([f8f977d1]) ([8f750916])

---

## 0.4.57

_Released 2023-03-26_

[0.4.56...0.4.57]

_MSRV 1.66.0_

### Changes

- (LIB) Print evtx files in chronological order [Issue #86] ([e42d021c])
- (BIN) bin.rs summary linerize more ([cebf2818])
- (LIB) datetime.rs NFC refactor dt_pass_filters ([bbdb2cb6])
- (PROJECT) CHANGLOG revise h2 naming ([ef80a0d5])

### Fixes

- (BIN) bin.rs tweak --help wording ([3c34d099])

---

## 0.4.56

_Released 2023-03-24_

[0.3.55...0.4.56]

_MSRV 1.66.0_

### New

- (LIB) process Windows Event Log evtx files [Issue #86] [Issue #87] ([368eba9b])

### Changes

- (TEST) add various `Summary` tests ([7f3911b0]) ([9f5391ba]) ([5b2b6f80]) ([0923408b]) ([d03737cd])

### Fixes

- (BIN) bin.rs fix -a "" (passing empty string) ([d4ed03e0])
- (BIN) bin.rs fix panic for multiple non-existent paths ([da980d8b])

---

## 0.3.55

_Released 2023-03-18_

[0.3.54...0.3.55]

_MSRV 1.66.0_

### New

- (LIB) syslinereader.rs track EZCHECK12 use ([29072ac5])
- (LIB) datetime.rs add patterns derived from hawkeye.log  ([d091a792])
- (TOOLS) add tools/cargo-outdated.sh ([44fa812a])
- (TOOLS) add tools/cargo-upgrade.sh ([cb698ec9])

### Changes

- (LIB) datetime.rs use slice as ref TryInto ([b723fed8])
- (BUILD) Cargo cargo update and upgrade ([f2199b30])

### Fixes

- (BIN) bin.rs fix --separator, use for utmpx ([24f00e77])
- (LIB) utmpxreader.rs fix errant println! ([50fec201])

---

## 0.3.54

_Released 2023-03-17_

[0.2.53...0.3.54]

_MSRV 1.66.0_

### New

- (BIN) use shrink_to_fit on maps [Issue #84] ([e9b501fc])
- (BUILD) Cargo.toml improve release optimizations ([06e500f1])

### Fixes

- (BIN) bin.rs user-passed %Z for filters [Issue #85]([98c4b362])
- (LIB) fix FreeBSD compile of `uapi` and define `umptx` ([62e89e29])

---

## 0.3.53

_Released 2023-03-10_

[0.2.52...0.3.53]

_MSRV 1.66.0_

### New

- (LIB) add support for utmpx login records (major refactoring) [Issue #70] ([b227f531])

### Changes

- (BUILD) MSRV 1.66.0

---

## 0.2.52

_Released 2023-02-15_

[0.2.51...0.2.52]

_MSRV 1.64.0_

### New

- (LIB) datetime.rs add format catalina apache access  [Issue #82] ([5337dd90]) ([997a365d])

---

## 0.2.51

_Released 2023-02-09_

[0.2.50...0.2.51]

_MSRV 1.64.0_

### New

- (BIN) bin.rs option --sysline-separator [Issue #80] ([b6d359fe])
- (BIN) print bytes counts in hex ([e46b1f94])
- (LIB) datetime pattern for tomcat catalina [Issue #81] ([8def2f69])

---

## 0.2.50

_Released 2023-01-29_

[0.2.49...0.2.50]

_MSRV 1.64.0_

### New

- (BUILD) github code coverage --all-targets [Issue #77] ([6baae7cc])
- (TEST) printers_tests.rs add initial tests for printers.rs ([5cabf7b9])
- (TEST) syslinereader_tests.rs add basic tests SyslineReader::new ([0bee4492])
- (TEST) tests/common.rs add eprint_file_blocks ([361e9867])
- (TOOLS) add valgrind-massif.sh ([84f30592])
- (TOOLS) add heaptrack.sh ([749f8ce7])
- (TOOLS) add s4-wait.sh ([e1dfb0a2])

### Changes

- (BUILD) bump MSRV to 1.64.0 ([ac5749d3])
- (BUILD) rust.yml remove build 1.68.0 ([aee27e45])
- (LIB) bump si_trace_print = "0.3.9" ([90556122])
- (LIB) syslinereader.rs add ezcheck12_min ([713bb735])

### Fixes

- (BIN) bin.rs bounded channel queue [Issue #76] ([ac509a2c])

---

## 0.2.49

_Released 2023-01-26_

[0.2.48...0.2.49]

_MSRV 1.64.0_

### Changes

- (LIB) src/ refactor find_line_in_block find_sysline_in_block partial ([cda6e991])
- (LIB) rust.yml remove build 1.68.0 ([aee27e45])
- (LIB) syslinereader.rs add ezcheck12_min ([713bb735])
- (LIB) debug/printers.rs improve buffer_to_string_noraw improve `buffer_to_string_noraw` to be more robust ([0c7efef5])
- (LIB) src/ refactor patterns analysis [Issue #74] [Issue #75] ([8575cd87])
- (LIB) src/ revise all si_trace_prints calls ([df628a72])
- (TEST) debug/helpers.rs add create_temp_file_data ([e3c0e0a4])
- (TEST) syslogprocessor_tests.rs add tests for short files ([d48099c0])

### Fixes

- (LIB) syslinereader.rs fix one-off error in get_boxptr ([619da415])
- (LIB) syslogprocessor.rs fix expected sysline count in blockzero ([e3ca0e22])
- (TEST) common.rs fix PartialEq for FileProcessingResult ([85d51b6b])

---

## 0.2.48

_Released 2023-01-15_

_MSRV 1.61.0_

### New

- (BUILD) rust.yml build on more versions of rust ([c80a0449])
- (LIB) datetime.rs add more general purpose matches ([d6bb2d1d])
- (LOGS) add logs/FreeBSD12.3 ([9128a71a])
- (LOGS) add logs dtf12-*.log ([715cff55])
- (LOGS) add logs/programs/ntp/ ([8fbb9f8e])
- (TEST) syslinereader_tests.rs add basic tests SyslineReader::new ([0bee4492])
- (TEST) syslogprocessor_tests.rs add tests for short files ([d48099c0])
- (TOOLS) add cargo-call-stack.sh ([6b47b9c1])

### Changes

- (BUILD) rust.yml explicit shell, print version more often ([9ba41e35])
- (DOCS) README fix shields link for github ([08d198ae])
- (DOCS) blockreader.rs doc string links ([7d8d35aa])
- (LIB) src/ revise all si_trace_prints calls ([df628a72])
- (LIB) debug/printers.rs improve buffer_to_string_noraw ([0c7efef5])
- (LIB) syslinereader.rs debug print DTPI attempts ([35fbb1da])
- (LIB) datetime.rs add test for a DTPD ([d9f70ce8])
- (LIB) datetime.rs refactor DTFSSet::fmt::Debug ([0d9d80be])
- (LIB) datetime.rs more precise syslog matching, tests, notes ([7db097f3])
- (LIB) syslinereader.rs manual pop_last ([c225eb65])
- (TEST) tests/common.rs add eprint_file_blocks ([361e9867])
- (TEST) blockreader_tests.rs add basic new BlockReader tests [Issue #22] ([d3f723ed])
- (TEST) linereader.rs add basic new LineReader tests ([b7a25d09])
- (TEST) debug/helpers.rs add create_temp_file_data ([e3c0e0a4])
- (TOOLS) cargo-test.sh remove --test-threads=1 ([308628cc])

### Fixes

- (LIB) src/ refactor patterns analysis [Issue #75] [Issue #74] ([8575cd87])
- (LIB) src/ refactor find_line_in_block find_sysline_in_block partial [Issue #22] ([cda6e991])
- (LIB) common.rs fix PartialEq for FileProcessingResult ([85d51b6b])
- (LIB) syslogprocessor.rs break when fo_prev >= fo_prev_prev [Issue #75] ([82259968])
- (LIB) syslinereader.rs fix one-off error in get_boxptr ([619da415])
- (LIB) syslogprocessor.rs fix expected sysline count in blockzero ([e3ca0e22])

---

## 0.2.47

_Released 2023-01-09_

[0.2.46...0.2.47]

_MSRV 1.61.0_

- (BIN) bin.rs fix typo in clap help ([b03da488])
- (DOCS) README update --help ([cdaad462])

---

## 0.2.46

_Released 2023-01-09_

[0.1.45...0.2.46]

_MSRV 1.61.0_

### New

- (BIN) bin.rs print canonical path ([9ceb5b48])
- (BIN) bin.rs use verbatim_doc_comment for all args ([a88c6625])
- (LIB) datetime.rs refactor day matching [Issue #58] [Issue #42] [Issue #47] ([1e58094e])
- (LIB) datetime.rs support synoreport.log [Issue #45] ([7d6d9f2d])
- (LIB) datetime add pattern for pacman.log [Issue #41] ([989ecdd9])
- (LIB) datetime pattern for explicit syslog format ([ef73bd5c])
- (LIB) datetime.rs add Windows 10 ReportingEvents.log, fix missing tests ([50870c1b]) ([c4c7f301])
- (CI) rust-workflow.sh add MSRV verify ([6e284ff0])
- (CI) github add task compare-current-and-expected.sh ([133cb5c7]) ([82255c0c]) ([26ec11b7]) ([b1dc6f92]) ([ee95b636])
- (CI) rust-workflow.sh run compare scripts ([3ac5374e])
- (CI) github run log-files-time-update.sh ([705dd66a]) ([69abc77c])
- (TOOLS) add tools/compare-debug-release.sh ([acc9b5b8]) ([5e975901]) ([b1dc6f92]) ([8514bb9e])
- (TOOLS) tools add cargo-msrv.sh ([e43d48bf])
- (TOOLS) add tools/compare-current-and-expected ([f0146fc0]) ([8c9a919d]) ([c87ceff1]) ([dd63214e]) ([80d06ddf])
- (TEST) add logs Windows10Pro/Panther/* ([2975c9af])
- (TEST) add log MPDetection-12162017-091732.log ([8e6fc80b])
- (TEST) add logs Orbi/tmp ([19adf7ec])
- (TEST) add busybox logs ([e1e46068])

### Changes

- (BIN) filepreprocessor.rs allow crossing filesystems [Issue #71] ([c6f18991])
- (BIN) bin.rs refactor summary printing ([5f5606de])
- (BUILD) Cargo downgrade const-str for MSRV support ([dfd2898d])
- (LIB) datetime refactor ([1e58094e])
- (LIB) src/ refactor process_path ([e80df386])
- (LIB) blockreader.rs change tar path separator char ([9a37f841])
- (LIB) src/ store path in Summary ([34320a79])
- (LIB) filepreprocessor.rs fix unreachable match ([70dcb6e4])
- (LIB) cargo fmt, cargo clippy warnings ([09a885de]) ([a2569929]) ([bab5ee53]) ([8a50df10]) ([a8a5f364])
- (PROJECT) add tools/.gitignore ([44bd6b10])

### Fixes

- (CI) github fix MSRV check ([58d2f205])
- (PROJECT) .gitignore add leading path for dirs ([f355964d])
- (TOOLS) hexdump.py flush stderr, stdout ([bbe3b006])

---

## 0.1.45

_Released 2023-01-01_

[0.1.44...0.1.45]

_MSRV 1.61.0_

### New

- (LIB) datetime.rs add pattern for apport.log [Issue #55] ([7557a59e])
- (LIB) datetime.rs add pattern for openftp.log ([Issue #48]) ([fda61f8f])

### Changes

- (LIB) be more sure of matching year ([fda61f8f])
- (TEST) more stringent and precise DTPD check ([fda61f8f]) ([60aa5d1c])

---

## 0.1.44

_Released 2022-12-29_

[0.1.43...0.1.44]

_MSRV 1.61.0_

### New

- (LIB) datetime.rs add 6 DTPD! entries ([d3f5d8a4])

### Changes

- (BIN) print summary information for files with bad permissions [Issue #69]([3ee20b9c])
- (BIN) better align summary output ([f6a72ff1])
- (CI) github do more runs of s4 with default blocksz ([48bd3aa9])
- (LIB) src/ refactor FileType ([d882f968])
- (LIB) src/ add ProcessPathResult::FileErrNotExist,FileErrNotParseable ([e9ea121a])
- (TOOLS) tools add rust-workflow.sh ([8e3b72a7])

### Fixes

- (LIB) syslinereader do not panic in case of unexpected Done [Issue #67] ([2da33982])

---

## 0.1.43

_Released 2022-12-26_

[0.1.42...0.1.43]

_MSRV 1.61.0_

### New

- (CI) add codecov.yml ([bd49cdc8])

### Changes

- (BUILD) adjust "filetime" dependency ([ff2cd81b])
- (DOCS) update README ([cb8ed82e])
- (BIN) update --help for `--blocksz` ([21578610])

---

## 0.1.42

_Released 2022-12-19_

[0.1.41...0.1.42]

_MSRV 1.61.0_

### Changes

- (BIN) (BUILD) update clap from 3 to 4 ([f58f506f])
- (BUILD) cargo update ([41bb25a1])

---

## 0.1.41

_Released 2022-12-18_

[0.1.40...0.1.41]

_MSRV 1.61.0_

### Changes

- (TOOLS) gen-log.sh add option for extra lines ([610785f3])
- (BIN) re-arrange trailing summary print of datetimes ([a80facf0])
- (LIB) for logs without year, skip processing outside filters [Issue #65] ([33418d03])
- (PROJECT) add gen-20-2-2-faces.log ([8a47ad83])

### Fixes

- (LIB)(BIN) fix drop, add more summary stats, tests [Issue #66] ([916259ba])

---

## 0.1.40

_Released 2022-11-22_

[0.1.39...0.1.40]

_MSRV 1.61.0_

### New

- (BIN) add CLI option `--prepend-separator` ([467b14db])

### Changes

- (BIN) add summary _syslines stored high_ ([d1f5895f1e5a55cbbcbfc4072bbde53a7a85fc])

---

## 0.1.39

_Released 2022-10-19_

[0.1.38...0.1.39]

_MSRV 1.61.0_

### Changes

- (LIB) Cargo.toml rust MSRV 1.61.0 ([3c4e8b1b]) 
- (BUILD) rust.yml add job_rust_versions, jo_rust_msrv_os 1.61.0 ([94d6862e])
- (LIB) const-str downgrade to 0.4.3 ([7cdaa4b6])
- (DOCS) README add more Windows examples, wording ([db5b6a5f])
- (LIB) (BIN) codespell fixes ([b9d4c2c2]) ([aaaf78e1])

---

## 0.1.38

_Released 2022-10-16_

[0.0.37...0.1.38]

_MSRV 1.62.0_

### New

- (BIN) bin.rs --summary print -a -b as UTC ([186c7472])
- (BIN) bin.rs allow relative offset datetimes [Issue #35] ([4eef6221])

### Changes

- (DOCS) README add Windows log snippets, tweak wording ([65c00784])

---

## 0.0.37

_Released 2022-10-12_

[0.0.36...0.0.37]

_MSRV 1.62.0_

### New

- (LIB) datetime.rs patterns for Windows compsetup.log mrt.log ([0f225cee])

### Changes

- (LIB) blockreader.rs eprintln path ([3e1607f0])
- (TEST) add logs Windows10Pro ([1c746c24])

---

## 0.0.36

_Released 2022-10-10_

[0.0.35...0.0.36]

_MSRV 1.62.0_

### New

- (LIB) datetime.rs parsing nginx apache logs [Issue #53] ([003b29ba])

### Changes

- (BIN) bin.rs --help tweak, comment cleanup ([3963e070])
- (TEST) add more logs ([9ddaaeed]) ([effffe87]) ([66eea98e]) ([0743e415]) ([e736a714])
- (DOCS) README changes, docstring changes ([65794851]) ([001f0c3d]) ([4b51b30d]) ([fda30e59]) ([8f143748]) ([44291c74])

### Fixes

- (LIB) datetime.rs add RP_NOALPHA after CGP_TZZ ([1cfc72e9])

---

## 0.0.35

_Released 2022-10-09_

[0.0.34...0.0.35]

_MSRV 1.62.0_

### New

- (LIB) datetime.rs handle format with syslog levels [Issue #57] ([d75fdfc0])
- (LIB) datetime.rs add RP_NOALPHA after CGP_TZZ ([1cfc72e9])
- (DOCS) rustdocs improvements ([1de420a5]) ([1b88a1e3])
- (PROJECT) README add section "syslog definition chaos" ([ec82a500])
- (BUILD) Cargo update to latest dependencies ([7185ba47])
- (BUILD) Cargo.toml exclude more READMEs ([bd44896a])

### Fixes

- (TEST) cargo-test.sh fix --version ([2a1b1085])

---

## 0.0.34

_Released 2022-10-07_

[0.0.33...0.0.34]

_MSRV 1.62.0_

### New

- (BIN) bin.rs allow user opt dt format string [Issue #28] ([6660f686])
- (TEST) add logs ISO8601\*log RFC3164\*log RFC3339\*log ([3980d5b6])
- (TEST, TOOLS) log files add systems, cleaning scripts, touch set ([55a1e55e])
- (LIB) datetime.rs allow Unicode "minus sign" [Issue #38] ([fc2a8379])
- (PROJECT) README updated Features, Limitations ([0ca431ce])
- (PROJECT) README README fill section About ([aa3992cc])

### Fixes

- (TEST) cargo-test.sh `rm -f` to avoid possible error during exit_ ([2343d263])

---

## 0.0.33

_Released 2022-09-21_

[0.0.32...0.0.33]

_MSRV 1.62.0_

### New

- (BIN) bin.rs allow user opt dt format string [Issue #28] ([6660f686])
- (DEBUG) use `si_trace_print` 0.2.5 ([fc548235])

---

## 0.0.32

_Released 2022-09-20_

[0.0.31...0.0.32]

_MSRV 1.62.0_

### New

- (TEST) datetime_tests.rs add tests cases datetime_parse_from_str ([c9bc19ec])

### Fixes

- (LIB) datetime.rs fix copying fractional [Issue #23] ([764689fe])

---

## 0.0.31

_Released 2022-09-19_

[0.0.30...0.0.31]

_MSRV 1.62.0_

### New

- (DOCS) improved README badges ([30553b79]) ([b5d4d917]) ([17cd4973])

---

## 0.0.30

_Released 2022-09-18_

[0.0.29...0.0.30]

_MSRV 1.62.0_

### New

- (LIB) allow -t %Z or %z, allow Zulu ([a71c5e81])

### Changes

- (TEST) add log dtf11-A.log ([56078e8b])
- (BIN) bin.rs stack_offset_set only debug builds ([d2158ee2])

---

## 0.0.29

_Released 2022-09-17_

[0.0.28...0.0.29]

### Changes

- (DOCS) README update install and run instructions ([01f39590])
- (BUILD) Cargo.toml remove unused bench, exclude unused benches ([b088be72])
- (LIB) Cargo.toml rust-version 1.62 ([aaf976b8])
- (BUILD) rust.yml add "Check Publish" ([7f751c12])
- (TEST) add logs gen-100-10. gz xz tar tgz ([33a492b9])
- (TOOLS) valgrind-dhat.sh add more logs to parse ([6d64fd6d])
- (LIB) Cargo update dev-dependencies ([6805e2b9])

---

## 0.0.28

_Released 2022-09-17_

[0.0.27...0.0.28]

### New

- (BUILD) Cargo.toml rust-version 1.62 ([aaf976b8])
- (BUILD) rust.yml add "Check Publish" ([7f751c12])
- (BUILD) Cargo update dev-dependencies ([6805e2b9])
- (BUILD) Cargo.toml ready to publish ([d97f0ab7])
- (CI) Check clippy ([e7172e45]) ([ac8d29bb])
- (CI) rust.yml add grcov job ([fcf91c96])
- (CI) rust.yml add step for "cargo check" ([17f89020])
- (TOOLS) add cargo-clippy.sh ([676633a7])

### Changes

- (TEST) add new test logs
- (LIB) src/ clippy recommendations ([84cc63c2])
- (LIB) src/ mv src/printer_debug -> src/debug ([d70104fb])

### Fixes

- (TOOLS) tools chmod 100755 ([a1378662])

---

## 0.0.27

_Released 2022-09-16_

[0.0.26...0.0.27]

### New

- (LIB) datetime.rs allow lowercase named timezone ([cae98770])
- (TOOLS) add cargo-test-cov-tarpaulin.sh ([5f93e4ad])
- (TOOLS) add cargo-test-cov-llvm.sh ([97cf45e9])
- (TOOLS) add cargo-test-cov-grcov.sh ([55113bc5])
- (TOOLS) add changelog-link-gen.sh ([0579522f])

### Changes

- (BIN) bin.rs prepend only files with Syslines [Issue #21] ([e189fd21])
- (LIB) src/ cargo fmt ([4fda60f2]) ([53892a3a]) ([a1e1a680]) ([dff29276]) ([4b784a72]) ([78581dba])
- (LIB) use crate si_trace_print, remove local impl ([f49fb33d])
- (TOOLS) rename compare-cat.sh ([25e4bf65])
- (TOOLS) rename cargo-test.sh ([0a5ce1e0])
- (TOOLS) rename rust-gdb.sh ([f00ebc4c])
- (TOOLS) rename cargo-doc.sh ([9957cc56])
- (TOOLS) rename compare-grep-sort.sh ([acc34edc])
- (TESTS) datetime.rs add begin,end indexes to test cases ([5dfc932d])
- (LIB) Cargo.* update dependencies ([7fb6abe6]) ([d8faf4fd])
- (LIB) datetime.rs improve precision of patterns, var names ([238df6c7])
- (LIB) datetime.rs add x4 DTPI for a pattern lie `VERBOSE Tuesday Jun 28 2022 01:51:12 +1230` ([34595fe2])
- (LIB) datetime.rs match named pattern `<dayIgnore>` ([07214abd])

---

## 0.0.26

_Released 2022-08-12_

[0.0.25...0.0.26]

### New

- (DOCS) rustdocs
- (LIB) add datetime patterns for history.log ([6955a7b5])
- (BIN) bin.rs allow user to pass only dates ([0a46b5ae])
- (TOOLS) add rust-doc.sh ([f56045aa])
- (TOOLS) add valgrind-dhat-extract.sh ([aa5bdbbc])

### Changes

- (BIN) refactor logic for process return code ([09df0b65])
- (CI) github tests build documentation ([f7b4533f])
- (BIN) remove unsupported `%Z` in `--help` [Issue #20] ([b55341ec])
- (BIN) bin.rs simplify check for printing error ([79c1ea1e])
- (BIN) bin.rs print trailing `'\n'` if needed ([3c7984d4])
- (BIN) bin.rs remove panics in processing_loop ([dc30ca63])
- (LIB) line.rs printers.rs refactor some slice statements ([aab71b50])
- (DOCS) blockreader.rs NFC docstring about read_block_FileTar [Issue #13] ([a503554d])
- (LIB) filepreprocessor.rs NFC comment [Issue #15] ([ed3d1feb])
- (LIB) src/ NFC comment [Issue #16] ([c35066cd])
- (LIB) blockreader.rs NFC comment [Issue #13] ([1c58a778])
- (LIB) blockreader.rs NFC comment [Issue #12] ([bc411286])
- (LIB) blockreader.rs NFC comment [Issue #11] ([908b2f59])
- (LIB) blockreader.rs NFC comment [Issue #10] ([c9dc70a5])
- (LIB) blockreader.rs NFC comment [Issue #9] ([8cd40b52])
- (LIB) blockreader.rs NFC comment [Issue #8] ([c8328f3a])
- (LIB) blockreader.rs NFC comment [Issue #7] ([943ad625])
- (LIB) datetime.rs NFC comment [Issue #6] ([af468519])
- (LIB) datetime.rs NFC add commented [Issue #4] ([d8d56414])
- (BIN) bin.rs NFC link [Issue #5] ([81c437b0])
- (LIB) improve supplementary filetype matching in `path_to_filetype` ([48687e8a])
- (BIN) bin.rs consistent eprintln for failed parsing ([880e35ae])

### Fixes

- (TOOLS) tools chmod 100755 ([a1378662])
- (LIB) src/blockreader* fix handling zero byte gz, xz ([f708e15e]) ([21745ee9])
- (BIN) bin.rs fix -a -b parsing ([f6b52fc2])
- (LIB) Massive amount of code cleanup
- (TEST) Many more tests

---

## 0.0.25

_Released 2022-07-28_

[0.0.24...0.0.25]

### New

- (LIB) add handling for missing year in datetime ([ab579207])

### Changes

- (BIN) Fix flood of error messages due to printing failure ([c332a733])

### Fixes

- (LIB) Fix handling GZIP XZ ([8d5e6860])
- (DEBUG) Fix debug print Sysline String with unknown glyph ([b2d6de50])
- (DEBUG) Fix debug print Line String with unknown glyph ([ed5c04ad])
- (TEST) Many more tests

---


## 0.0.24

_Released 2022-07-20_

[0.0.23...0.0.24]

### New

- (LIB) handle tar files ([adf40070])

### Changes

- (LIB) datetime transforms `%b` `%B` to `%m` ([22980abf])

---

## 0.0.23

_Released 2022-07-12_

[0.0.22...0.0.23]

### New

- (LIB) WIP handle tar files ([b8deef34])
- (PROJECT) add CHANGELOG.md ([ca1c967a])

### Changes

- (LIB) add enum_BoxPtrs::DoublePtr ([61f15e13]) ([cb74da32])
- (LIB) refactor to use `regex::bytes::Regex` instead of `str`-based `regex::Regex` ([dfd60d4b]) ([3d78b0d0])
- (LIB) refactor name `enum_BoxPtrs` to `LinePartPtrs` ([b5505730])
- (TOOLS) rust-test.sh use nextest if available ([1bf27841])
- (TEST) faster tests reuse single `NamedTempFile` more often ([db2e8f3c])
- (CI) github run args change ([a82e25b5]) ([c8fc525d]) ([febfd00d])

### Fixes

- (BIN) fix --blocksz minimum check ([07baf6df])
- (LIB) printers.rs fix macro `print_color_highlight_dt` ([66595090])
- (DEBUG) line.rs impl LinePart::count_bytes,len ([9d9179cf])
  Fix `LinePart::count_bytes`
- (DEBUG) printers.rs fix `char_to_char_noraw` ([ced4667f])
- (DEBUG) line.rs fix `_to_String_raw` ([d5af77de])

---

## 0.0.22

_Released 2022-07-10_

[0.0.21...0.0.22]

### New

- (LIB) refactor datetime string matching ([3562638d])
  refactor datetime string matching within a `Line` to use regex.
- (TOOLS) add hexdump.py ([031434f4])
- (LIB) printers.rs highlight datetime in call cases ([a4fd91f4])
- (LIB) printer.rs all color lines highlight dt ([9c5fa576])
- (LIB) filepreprocessor also check supplement with ext removed ([0f4ac9ae])
  During filetype search, also call supplmentary check based on name
  using a path with file extension removed. Allows matching files like
  `kern.log.gz.1`.

### Changes

- (BUILD) remove crate chain-cmp ([7109c46d])
- (LIB) set `const` for funcs `slice_contains...` ([eeb20bb8])

### Fixes

- (LIB) fix errant �� printed at block edges ([5caa8dd6])

---

## 0.0.21

_Released 2022-06-24_

[0.0.1...0.0.21]

### New

- (LIB) add XZ support ([607a23c0])
- (BIN) bin.rs set default -t in help message ([e346e184])

### Fixes

- (LIB) src/ print summary invalid, fix dir walk error ([09a04c14])
  Print summary of each invalid file and it's MimeGuess and FileType result. This helps the use understand why a file was not parsed.
  (LIB) Fix directory walk in `process_path` only checking the root directoy and giving errant `FileType` to files.

---

<!--
All notable changes to this project will be documented in this file.

This somewhat project adheres to [Semantic Versioning](http://semver.org/) and [Keep a Changelog](http://keepachangelog.com/).
-->

This document template generated by [changelog-cli](https://pypi.org/project/changelog-cli/).

<!--
TODO per release: update links (run `tools/changelog-link-gen.sh`)

DO NOT CHANGE THE FOLLOWING COMMENT
EVERYTHING AFTER THE FOLLOWING COMMENT WILL BE DELETED AND REPLACED BY `tools/changelog-link-gen.sh`
-->

<!-- LINKS BEGIN -->

[Issue #10]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/10
[Issue #100]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/100
[Issue #101]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/101
[Issue #102]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/102
[Issue #104]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/104
[Issue #105]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/105
[Issue #108]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/108
[Issue #109]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/109
[Issue #11]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/11
[Issue #110]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/110
[Issue #112]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/112
[Issue #12]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/12
[Issue #120]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/120
[Issue #121]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/121
[Issue #128]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/128
[Issue #13]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/13
[Issue #14]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/14
[Issue #145]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/145
[Issue #15]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/15
[Issue #152]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/152
[Issue #16]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/16
[Issue #17]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/17
[Issue #170]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/170
[Issue #171]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/171
[Issue #182]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/182
[Issue #20]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/20
[Issue #201]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/201
[Issue #202]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/202
[Issue #206]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/206
[Issue #21]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/21
[Issue #213]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/213
[Issue #217]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/217
[Issue #218]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/218
[Issue #219]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/219
[Issue #22]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/22
[Issue #224]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/224
[Issue #23]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/23
[Issue #241]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/241
[Issue #245]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/245
[Issue #250]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/250
[Issue #257]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/257
[Issue #258]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/258
[Issue #260]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/260
[Issue #261]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/261
[Issue #262]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/262
[Issue #27]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/27
[Issue #270]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/270
[Issue #271]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/271
[Issue #28]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/28
[Issue #283]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/283
[Issue #284]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/284
[Issue #285]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/285
[Issue #29]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/29
[Issue #291]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/291
[Issue #293]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/293
[Issue #297]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/297
[Issue #304]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/304
[Issue #305]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/305
[Issue #35]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/35
[Issue #354]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/354
[Issue #36]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/36
[Issue #360]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/360
[Issue #38]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/38
[Issue #4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/4
[Issue #40]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/40
[Issue #401]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/401
[Issue #404]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/404
[Issue #41]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/41
[Issue #411]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/411
[Issue #412]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/412
[Issue #414]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/414
[Issue #415]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/415
[Issue #416]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/416
[Issue #419]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/419
[Issue #42]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/42
[Issue #437]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/437
[Issue #445]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/445
[Issue #449]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/449
[Issue #45]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/45
[Issue #46]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/46
[Issue #467]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/467
[Issue #47]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/47
[Issue #48]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/48
[Issue #5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/5
[Issue #53]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/53
[Issue #55]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/55
[Issue #56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/56
[Issue #57]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/57
[Issue #58]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/58
[Issue #6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/6
[Issue #60]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/60
[Issue #63]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/63
[Issue #65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/65
[Issue #66]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/66
[Issue #67]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/67
[Issue #69]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/69
[Issue #7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/7
[Issue #70]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/70
[Issue #71]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/71
[Issue #74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/74
[Issue #75]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/75
[Issue #76]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/76
[Issue #77]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/77
[Issue #8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/8
[Issue #80]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/80
[Issue #81]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/81
[Issue #82]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/82
[Issue #84]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/84
[Issue #85]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/85
[Issue #86]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/86
[Issue #87]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/87
[Issue #88]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/88
[Issue #9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/9
[Issue #91]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/91
[Issue #93]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/93
[Issue #94]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/94
[Issue #95]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/95
[Issue #96]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/96
[Issue #97]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/97
[Issue #98]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/98
[Issue #99]: https://github.com/jtmoon79/super-speedy-syslog-searcher/issues/99
[PR #311]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/311
[PR #312]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/312
[PR #313]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/313
[PR #314]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/314
[PR #315]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/315
[PR #323]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/323
[PR #324]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/324
[PR #325]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/325
[PR #326]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/326
[PR #327]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/327
[PR #328]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/328
[PR #329]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/329
[PR #330]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/330
[PR #332]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/332
[PR #333]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/333
[PR #335]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/335
[PR #336]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/336
[PR #337]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/337
[PR #338]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/338
[PR #340]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/340
[PR #343]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/343
[PR #346]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/346
[PR #348]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/348
[PR #349]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/349
[PR #355]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/355
[PR #356]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/356
[PR #357]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/357
[PR #358]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/358
[PR #359]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/359
[PR #361]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/361
[PR #365]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/365
[PR #366]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/366
[PR #367]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/367
[PR #368]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/368
[PR #369]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/369
[PR #370]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/370
[PR #371]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/371
[PR #372]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/372
[PR #374]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/374
[PR #375]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/375
[PR #376]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/376
[PR #377]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/377
[PR #378]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/378
[PR #380]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/380
[PR #381]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/381
[PR #382]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/382
[PR #383]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/383
[PR #384]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/384
[PR #385]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/385
[PR #386]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/386
[PR #387]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/387
[PR #388]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/388
[PR #389]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/389
[PR #390]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/390
[PR #391]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/391
[(#166)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/166
[(#168)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/168
[(#169)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/169
[(#172)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/172
[(#173)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/173
[(#174)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/174
[(#175)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/175
[(#176)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/176
[(#177)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/177
[(#178)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/178
[(#181)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/181
[(#193)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/193
[(#195)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/195
[(#197)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/197
[(#199)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/199
[(#200)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/200
[(#203)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/203
[(#204)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/204
[(#237)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/237
[(#246)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/246
[(#247)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/247
[(#249)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/249
[(#272)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/272
[(#274)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/274
[(#275)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/275
[(#278)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/278
[(#279)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/279
[(#280)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/280
[(#281)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/281
[(#282)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/282
[(#289)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/289
[(#290)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/290
[(#292)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/292
[(#295)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/295
[(#296)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/296
[(#301)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/301
[(#302)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/302
[(#307)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/307
[(#308)]: https://github.com/jtmoon79/super-speedy-syslog-searcher/pull/308
[0.0.1...0.0.21]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.1...0.0.21
[0.0.21...0.0.22]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.21...0.0.22
[0.0.22...0.0.23]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.22...0.0.23
[0.0.23...0.0.24]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.23...0.0.24
[0.0.24...0.0.25]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.24...0.0.25
[0.0.25...0.0.26]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.25...0.0.26
[0.0.26...0.0.27]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.26...0.0.27
[0.0.27...0.0.28]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.27...0.0.28
[0.0.28...0.0.29]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.28...0.0.29
[0.0.29...0.0.30]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.29...0.0.30
[0.0.30...0.0.31]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.30...0.0.31
[0.0.31...0.0.32]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.31...0.0.32
[0.0.32...0.0.33]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.32...0.0.33
[0.0.33...0.0.34]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.33...0.0.34
[0.0.34...0.0.35]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.34...0.0.35
[0.0.35...0.0.36]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.35...0.0.36
[0.0.36...0.0.37]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.36...0.0.37
[0.0.37...0.1.38]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.0.37...0.1.38
[0.1.38...0.1.39]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.38...0.1.39
[0.1.39...0.1.40]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.39...0.1.40
[0.1.40...0.1.41]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.40...0.1.41
[0.1.41...0.1.42]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.41...0.1.42
[0.1.42...0.1.43]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.42...0.1.43
[0.1.43...0.1.44]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.43...0.1.44
[0.1.44...0.1.45]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.44...0.1.45
[0.1.45...0.2.46]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.1.45...0.2.46
[0.2.46...0.2.47]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.2.46...0.2.47
[0.2.48...0.2.49]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.2.48...0.2.49
[0.2.49...0.2.50]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.2.49...0.2.50
[0.2.50...0.2.51]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.2.50...0.2.51
[0.2.51...0.2.52]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.2.51...0.2.52
[0.2.52...0.3.53]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.2.52...0.3.53
[0.2.53...0.3.54]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.2.53...0.3.54
[0.3.54...0.3.55]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.3.54...0.3.55
[0.3.55...0.4.56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.3.55...0.4.56
[0.4.56...0.4.57]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.4.56...0.4.57
[0.4.57...0.5.58]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.4.57...0.5.58
[0.5.58...0.5.59]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.5.58...0.5.59
[0.5.59...0.6.60]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.5.59...0.6.60
[0.6.60...0.6.61]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.60...0.6.61
[0.6.61..0.6.62]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.61..0.6.62
[0.6.62..0.6.63]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.62..0.6.63
[0.6.63..0.6.64]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.63..0.6.64
[0.6.64..0.6.65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.64..0.6.65
[0.6.65..0.6.66]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.65..0.6.66
[0.6.66..0.6.67]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.66..0.6.67
[0.6.67..0.6.68]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.67..0.6.68
[0.6.68..0.6.69]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.68..0.6.69
[0.6.69..0.6.70]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.69..0.6.70
[0.6.70..0.6.71]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.70..0.6.71
[0.6.71..0.7.72]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.6.71..0.7.72
[0.7.72..0.7.73]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.72..0.7.73
[0.7.73..0.7.74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.73..0.7.74
[0.7.74..0.7.75]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.74..0.7.75
[0.7.75..0.7.76]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.75..0.7.76
[0.7.76..0.7.77]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.76..0.7.77
[0.7.77..0.7.78]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.77..0.7.78
[0.7.78..0.7.79]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.78..0.7.79
[0.7.79..0.8.80]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.7.79..0.8.80
[0.8.80..0.9.81]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.8.80..0.9.81
[0.9.81..0.9.82]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.9.81..0.9.82
[0.9.82..0.10.83]: https://github.com/jtmoon79/super-speedy-syslog-searcher/compare/0.9.82..0.10.83
[00171bbd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/00171bbdf238fd9c1ba6d89fa29a730318332d7e
[001f0c3d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/001f0c3db2c5751a35946f572aca6bf07c9efcaf
[0021f057]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0021f0576d0d629c72028443f2a266f957e5b084
[003b29ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/003b29bab508b32750cb303c70db9dc75cc04eab
[00530383]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/005303834eb662b2993d913eff8a8b1ea9fcd03c
[005f7fc3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/005f7fc363ae09a7e7d908e6ef815f77550ac348
[007f752f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/007f752fdd69eb37df38171a7e485f5ae026ec6c
[00827808]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/00827808a32126ce7e404ec49d0bdf46a8991547
[00846a55]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/00846a550af3c2e0bb397defeec68b29cc00cad2
[009ac596]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/009ac596a4ef2617d80d1f3643041b88379bcacb
[00a92328]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/00a92328417a01dbb07eb9d5bcde0b51b36c5209
[0106aa75]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0106aa75111a81214e479a84b9bf2719e2a02af7
[016e8b73]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/016e8b73852789c607b467c02e44c1ccf7933da3
[01a7cce4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/01a7cce46d38eedf65f937dd1214f3ae6fdb697b
[01b80ffa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/01b80ffaf666111674a7c11f33d913f8a0118d19
[01c11d89]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/01c11d89d6d5b3f329462b009c950f53c56a30e8
[01eb4c24]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/01eb4c24d48245839d6c11b612493608e2ceaf03
[01f39590]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/01f395903cff248be11ecf6f12974a3951aa7e92
[02261be4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/02261be4a6779a73ee08a3075bccb5effc31818f
[0293baf2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0293baf215a4c88159ecebe4a6c57a28d48e3900
[02be0545]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/02be0545aa3289bdcbe2e7cb53b900d9d07d79e7
[031434f4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/031434f4d9dfb4e0f8190a720f8db57a3772e3a2
[03a2c316]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/03a2c316c112badec32140aec400089035215fe0
[03b6e216]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/03b6e2165655f1cc8159c3ca1d267e3fa4019752
[03bbeb7a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/03bbeb7a51556361bbbb8ba40045543322bedc49
[03eea397]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/03eea397b60ebb1f1ddaf8d3747db1517bd6b9b1
[04558a30]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/04558a3054bc365543fcc40e33123248cd49f66e
[0522f1eb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0522f1eb34453c68c8f1abca0efeb5dfac5c2bf9
[0579522f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0579522ff7609e22c14b33aa6c6a70cec6372226
[05ca7b4d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/05ca7b4d70a8bddc3e505953df9f5d0ce2d28b21
[05d974c6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/05d974c6c4ced7b380343cbff1710e99a2a2ce28
[05f04e30]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/05f04e30dbf5985f01dabc1daa2fa36d10e900a1
[0608caba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0608caba55eccb75691b626ae8582683c42fbc90
[0648f40c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0648f40c5ec863b69071ba256c56bd479cf2078a
[06640e32]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/06640e3218bbbe8bdf97c9a54907fcb1a9491876
[0664eebc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0664eebccafedc636d0c03f67e5098bcad95b99a
[068a0a28]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/068a0a2893f6007a51aa9e65c997b9e08e72c3a5
[06e500f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/06e500f1d0148e0f9b50ab5907d7f6103533d5f7
[07193f74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/07193f74a6f308eac71969b3d0afea740cb1a2fe
[07214abd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/07214abde6479431cc1a9f87f50f3b713e5ea503
[0743e415]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0743e4157daa108569d99746d8a6314cfe6e0248
[0770728e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0770728e689105c4354ffe3260093d40b92e4ada
[07baf6df]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/07baf6df44ec3ccd2da43f3c5cb9f5ef30a6b0e8
[07d25370]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/07d25370ff0197fdacd36f92140d5ac5c3b2b7e3
[08005574]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08005574bef903fa30ab41c07d482014401dc3e0
[08017c8f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08017c8fecd684def6043848f80e1584938392de
[0807ecf2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0807ecf2c83240771f15113c9238b304c02b16c6
[08276667]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0827666779e549927d58c54c6afe6234c8b97f63
[0855ed09]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0855ed091f8ec12502f77d8a1c06518a0d897c8a
[08738c41]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08738c41a371749b9aac26c0ab319129d8be0c9f
[08d198ae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08d198ae57fc5b97013bdda5e883d7df383755f9
[08d1c801]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08d1c801cdbc6a908ee1dca543289cd6ccccea7f
[08d2e28d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08d2e28d04cfd075729f9884e96da2eea5ee04b9
[08fbb653]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08fbb65302ca13d5afa2e08ce4adb31fcb75970f
[08fed12c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/08fed12cf9f2e7a4003a02d2a3e3efecddf49c80
[0923408b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0923408bff8036c1b1c37bfba0a71012845c0935
[098d8438]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/098d84382b636b181aa853cd1147870c1d965d19
[0997ecdc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0997ecdc607501cf45e1e8c043210660a290646e
[09a04c14]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/09a04c14146af1916aeda14e8134d02baf088d5d
[09a54e45]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/09a54e45520e1c2ab48540bceaf0a8ba59b4b0ea
[09a885de]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/09a885de20cffeabbfaae72f2d597e007c9b6593
[09df0b65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/09df0b6551fec2ea22cee7dca2cd308cf11b531a
[09e71bb9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/09e71bb9ae655a0528c5a074dadfd5e9d71743c7
[0a1e78ce]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0a1e78cec7ad0cdcaf09d5e7d26f4d68e477bd6a
[0a46b5ae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0a46b5aee7eb99e19a9a2a91ed81d759978b6024
[0a5ce1e0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0a5ce1e0011920909cfa5bc022f95b3a502ff244
[0addafbf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0addafbf19cbae50139a2a2a70ccb21546166879
[0b42a8a1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0b42a8a18e424d18f431a7336a2840441bc9af95
[0b6faff2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0b6faff2b763edeba1bc2792ed936c44e8c302d9
[0b781120]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0b781120aeedae4d4d566508979b7c1d8a51b2fa
[0bee4492]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0bee4492533b7a88dfb43a9965b9026bcdefc705
[0c1f5655]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0c1f5655a108865ec3fbccc964d59d455f7e27a1
[0c6af5d6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0c6af5d6d031fd90fd472452bd42ddffab313da4
[0c6ba914]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0c6ba914d48380fae289077ea08b282484e075b5
[0c7efef5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0c7efef500543e3176b1538c90065cad3d624c50
[0ca431ce]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0ca431ce8b510b6714420a8954f587eccd84a01d
[0d01efed]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0d01efed1a01c64371ea4a055e82db7f02d90262
[0d6cdb58]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0d6cdb585e84651c45ab654ce096cdbaf2818f83
[0d9d80be]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0d9d80be29fc5051429cf53924d4a7ac3f6010a7
[0da5f9d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0da5f9d5a4001a7055e96061d485075e2a9a5cdb
[0e55b5df]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0e55b5dfbc15a5977145811bbe73e95d1f7eb3d6
[0ea897a7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0ea897a7665eff58d9c148ee53559504301e4a52
[0ee66989]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0ee66989091e9ed2482abe0e6f1aaa4596904eea
[0f225cee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f225cee04b5443a58369b95bc8e6f10ed3f6401
[0f4521cd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f4521cd7cfe059fddab74cfd29f7920d6070ad7
[0f4ac9ae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f4ac9ae4cb4d11247a40cf1a3c09f78a9a42399
[0f511e71]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f511e718c2b8b971cbec0ea6ba1eeb60772e1ed
[0f52dd97]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f52dd97344e7b25979625372f8f0b2f52f6a6a2
[0f8109cc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f8109cc2ca3900ca27f833d372a60fa704e3605
[0f822c54]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f822c54ef4959aba65b4bd24969dbf44e19c6ea
[0f85b485]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f85b485e12a1e744833ffa22baac44f2dc0fd5b
[0f93ff94]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0f93ff94e2b5aa133594cdfcac49abf4770c13f7
[0fceba27]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/0fceba274b8dbefb01ed890d3c211fd85211822b
[101d4e7a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/101d4e7ab997813ca3c92a9dff3d360f232a6d0b
[103075e2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/103075e21f87d2f05b9dd83d010475e7ce80d98e
[103899b0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/103899b047d72a66a6f105075bd6461e49c9dc8d
[103b4d23]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/103b4d23da43e410ea813fa506a54c1a3f479769
[10622640]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/10622640e0139bbfe6715fdb9e144579ceaaffe8
[10f4a0d3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/10f4a0d3c4c3440869b4c651af42918e06b5711e
[1164b0d0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1164b0d0265885beeac2db3f31516997db5b5b48
[1171a278]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1171a278280a6fc7a3550e5e006450f1229d4c3c
[1186b43f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1186b43f620a587c94c8464ba05de91c28df9018
[11c9d0bb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/11c9d0bbe8651fb8e057e88166afb450534d03f4
[11d17c67]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/11d17c6717f136dd7ce606fb91d421d15d33cc40
[11fa3eae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/11fa3eaebf1bb72ac1727a20672d520ef9e5cc1b
[12217e26]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/12217e26349a4c5a7fec8bf39c4d9e0e13ac34c3
[123e5551]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/123e5551781c98edf298977e2b4c4c89cd7f127e
[1250c119]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1250c119189b2799394ac64e7d2be4e9307b1c06
[12871b74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/12871b740007754071764243b290be6cc4fe272b
[1294a79f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1294a79f9c7196d934c0f37d7fc1a617a3cd3be4
[12a3b476]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/12a3b476c99451f1f00d3fea37df77fc549c2bf4
[12ec8d9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/12ec8d9ebeb874a0425a791591ff733b8178c8c7
[12fcb256]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/12fcb256adba82190120d466a46cd421838b025c
[13038896]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/130388963fd4f4d83dc2de5c69a4c9ba03d4b471
[132a18a8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/132a18a8de9131a5d101f24f35a5a7d773beb8ef
[132bcd74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/132bcd746979ad6ee467c497d783700eb7b6353a
[133cb5c7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/133cb5c7dcab6f018c0422bde1f8ee6f9a304258
[136852f5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/136852f557d9fb3cff20674c24397c62baab10bd
[13b6039f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/13b6039f771dc97e6a2792cce81d0752a6cf1806
[13c3926c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/13c3926c2a45e4c1e5b70457ebb7d67e69155617
[13f1ef40]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/13f1ef40476078a0a6904e1056dcf0153a9b1b43
[13f65d1f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/13f65d1f0e2198cdd91f457e39e4614eae7ad137
[145c45be]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/145c45be9c3e02027211c02d5b97c63f9ef98944
[145efc2f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/145efc2f581de83aa8c639adbd49ad926f392be7
[14d6f336]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/14d6f336dce90a80c9516fbe45b4c8bca77a71af
[14f3cab1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/14f3cab13d9e48eba306d78058bef4bf3a85028f
[15491d06]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/15491d062547a20b3e38b3d2841f987c7d7ba62c
[154c40d6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/154c40d67646724a703f32957a342c4e27af815b
[157d54a4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/157d54a4dda13b1f0b4743daa55b77d20887e82a
[1620f7e7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1620f7e7fe735f758925b0a235427259ea0dad26
[1677328e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1677328e42072057bcac2622726bd973255477c5
[175b74ee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/175b74eeae5dfc832ea714134b96cc61647d9dd8
[179c84a7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/179c84a7e56b403222021de529dab66d21ece572
[17a4aa8e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/17a4aa8eacef102cd2c03c0d824cb4bcd94ef315
[17b04216]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/17b042163f12e1785faa114e5278e61a2f1d5a8a
[17cd4973]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/17cd497307d04f3d8a9b058a72e3ea415a9a9f89
[17f89020]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/17f89020870b8bc8ad8322e314c187b6e0836226
[18098f47]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/18098f47f2f263a2090bccf2c216fad2a8314a0f
[186c7472]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/186c74720db2b33e5c0df17ee690eddcdee360a7
[1894f511]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1894f51143b54761534c84c894aa91b774c4e687
[190ecbfb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/190ecbfb012529a5308e32948e2029128477bdd2
[1936fd62]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1936fd62b2714a54cd68cb4ba0418290f5598d10
[19a1dd17]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/19a1dd176d606ca55386d5b5e7ecd456e2a88c9a
[19adf7ec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/19adf7ec9e2a687b6df19d2e3121c2683f3fc840
[19d3013d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/19d3013da3f60c9032663a971abb3ac1f6f348e7
[1a115291]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1a115291205aaecfb9fd2b0fa7a73d7f08a9165a
[1a71232d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1a71232d58a70fe8ca9dff970c9c6c6391fec4ea
[1aed8624]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1aed86246f06e1e3f68f692d81b45c2e22be60b5
[1b162a6c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1b162a6ca9741f72b6796da56e08d9700a9c259a
[1b3b8d34]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1b3b8d344c83a24a68e21be0d699eb988de7bb50
[1b88a1e3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1b88a1e35a66004ea5016525bcbb1e125aa64db9
[1b9e4228]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1b9e4228c72e5a8cabe7414c478e8043c87e98e2
[1bf27841]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1bf2784185df479a3a17975f773e3a505f735e26
[1c02a824]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1c02a82419566264acd9cdeaa69e8d3f8d693a68
[1c58a778]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1c58a778dc5bd05e455ea25af60e8600b8b72857
[1c6ccb18]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1c6ccb188129272267fd14d0f16fff42f67d81c6
[1c746c24]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1c746c24b7e0ad7e7481cce626fb6488eb0076d6
[1c97261e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1c97261ee7ea2789c30c3f0f13a4dda80887d636
[1cfc72e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1cfc72e99382ab47b55c9410ab531c0baf8ac46e
[1d6bc01b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1d6bc01b3f26c8362f08a4adc73c24ae5b968d8e
[1d8f57f7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1d8f57f78fe4abfeec3b8f0adc171501e60c9ebd
[1dc1ab26]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1dc1ab26d5e40cb745bd6d5bacb844d49b72dcc6
[1dcd9f64]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1dcd9f64cefd4e8120b4d44781a936bf65b716a0
[1de420a5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1de420a5907cf62ae91a06732a8ef43e01f17598
[1e390bc5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1e390bc55c459145ced3c5e900ce3392e3415231
[1e3e789b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1e3e789ba02d8378d590f61487c0beff5bb39d4f
[1e552fe9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1e552fe9a673dc759b583ff1c434b00385015025
[1e58094e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1e58094eafae95c9c09b35c63aa000a0edfd5845
[1f04b993]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1f04b993e429f5bb280942c9e6ca9be30d22f793
[1f0b90f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1f0b90f317c95173bec723e906aa25f109d98887
[1f0d2698]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1f0d269897d712259f7dd53f5f970e5261cf47fd
[1f8f24ee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1f8f24ee058b3cfa0ffe4effd364b4bd23003589
[1f97209d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1f97209dd3468105bc1aef3d449000ab56589c15
[1fb92f06]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1fb92f062761cdbab6c9180b1c2197919e0250d6
[1fd2eb2e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1fd2eb2e535ac2cf7de3e15e99bf103434a2c20a
[1fea7147]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/1fea7147a4f2780f4c50238798a52a1489b17a90
[2008c9a6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2008c9a69b228a45cde4e91b3bf814a791d77c23
[2030a739]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2030a7392e792e00727f80f9a2d83257b851f519
[20818ce5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/20818ce549b633c20b26d701440a68adf5c30ded
[20b4b045]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/20b4b04541520afd5f0610af4232e3e1c02bb8c5
[20c469c9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/20c469c93e3ad1247fd40ac5a9315ede7837eae6
[210f01c3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/210f01c36f0e7b8415ae595fbda857cff44277fb
[211ba869]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/211ba8694b8a2388ee23563be2c719a8df8d8698
[21578610]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2157861027eff2cade51aa950a6a4300e86a1e50
[2160556d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2160556dd15491acfa5e13d9de69a8db4af224d5
[21745ee9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/21745ee99eb04a4204164825ca5c50e6f8b34fee
[21904d9b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/21904d9bbafabb256c67d1c9e1e33777ca9616cc
[219e2cfc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/219e2cfcf9124054da2d34732a1290387daa4344
[21d83759]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/21d837591e65f6ad6fb8b01058d7cfa5c4048472
[21e28b30]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/21e28b30d7ac8b60a14694607b2f8ad4aebdd94c
[21f11792]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/21f11792630d3ede70227a19e07e8cc6d5887650
[2215ce8f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2215ce8fdfac87eb4bfa2d9c838609829868134c
[222152bf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/222152bf5c2755777c756c6ad332429422594c85
[22852ea3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/22852ea3bf55b059c3bc3512f0f654f1b5c6a301
[22980abf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/22980abf582aa61c5e4c9ce94d8298997fb5bbbc
[229b25f5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/229b25f5d33ec94f84ecff742a65be7883cde7d4
[231fcc46]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/231fcc46d921f781a1cdaa188c9f7e189e2709cd
[2340084f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2340084f30e25df0e81af73ef2d06f60f42963aa
[2343d263]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2343d26300c5a139066081648054e5e299eb8a80
[236802fa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/236802fa5ca3266119e20d4d1dd968a6e5f80a97
[238df6c7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/238df6c7b1b569f724778c85bfead20cb14be59d
[23dfeb32]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/23dfeb32d0a9d8a7b272ef748fca9b8556b5b0c1
[245729f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/245729f12d39983a09bfbe4f54088ee646fdad68
[24691784]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/24691784f79d33a5dd5497f53e064302dfb161d3
[246addaa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/246addaab8cca45c18a592cc29ac6670f2b7009e
[247a6338]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/247a633873e93dd624c35397ef4ce9134e4d0222
[248718fd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/248718fd6a57e9fa44db71d7b096af12cd539ca2
[24c21a15]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/24c21a15728132fcbcc6c3ce9724ed8ff19ec8a1
[24f00e77]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/24f00e77839701e01123b61e4d7daefcab264a9b
[25060e8b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/25060e8bba5a158a6cfa000babdbb67d758f2ddc
[250e9dee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/250e9dee204333647dfbe913c3edc40ae0bfc70f
[25447169]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/25447169bfe6c6236b8b87e6b3f451a209c7d9d5
[25b35ee8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/25b35ee8e1f7b7ed7d5119bca73becc6ad721617
[25e4bf65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/25e4bf65d9d5af300b99092e189f0caea3164f5f
[260fcfc9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/260fcfc924c71e53b568ed13f9339109bda94f71
[26979a36]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/26979a36cbbaeaf0f4266c2717757eab58662a80
[26bb3c8f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/26bb3c8fa87a571cc73c4efb5cc55238812d339d
[26daee68]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/26daee68627b16262717b7091fb192a029896cf5
[26ec11b7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/26ec11b7fff8c478b4aa48ed1a4cec01b683a318
[2742cc8d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2742cc8d6476edc32037f973e5f0a2bf33308652
[274d75da]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/274d75da985e6ca4c6db15b0f316abcb9a32c456
[281adc0d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/281adc0d2ebea05a6f47fca2ccabffe865295c16
[286ce3c2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/286ce3c2f37b781919012eb673aaa5355d4819d2
[29072ac5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/29072ac5c184215f8c10547e5019bf1845864296
[293ba44b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/293ba44b2a355175af4e0b10f43057e1038fd764
[2975c9af]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2975c9af59b515ee71824cd156c0b3b1bfba3f7d
[297ca3d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/297ca3d533e676b099dadf0de40b0b1d8d27638c
[29a55582]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/29a555822f24429491a5e5586dac8c23c93057e7
[29edb226]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/29edb22603caa19636bcfb0d738e61ad1a26fb2b
[2a1b1085]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2a1b10859a31649a7ef31db9474e3a6ed526c9a4
[2a2d372e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2a2d372e0c7bccb98ae3577a256caa710bff2e7d
[2af24cbf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2af24cbfbb1645e2cd364a9ab4434e0892619939
[2b18f59b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2b18f59b0b42f489adb9bc4aeefa30002982fc1c
[2b3081de]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2b3081de54c987e41d366085beff73d4a9a46c2d
[2b62c402]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2b62c40255bea2201a65fbd8daec74dede6a3b98
[2b7a60b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2b7a60b90bbedf5f1c21e40832f16d131dcac7f5
[2bed8dca]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2bed8dcab816dc990f7b2de5256f46a8c767c875
[2bfbe631]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2bfbe63137fbea60804d085a15760b9aaffac2f8
[2c1a38ea]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2c1a38eaca7a66c54938c66abd046fc21e34b58e
[2c34a47f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2c34a47fc1adc5d59ce6edd35b377659084f4819
[2c397230]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2c39723084839f1b93962c0c97ed45104533ce37
[2c70ee35]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2c70ee35718d8c03435139516bcb5e95e5af9f43
[2c8bf0f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2c8bf0f1bf9c7237b20849b195c52f926c0e43ff
[2ca50bf4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2ca50bf4b6209f806f5da44ac7033c6544c44df4
[2cb0412d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2cb0412d714078b17402d5bcfa2b1175f4f71bb3
[2cbf5577]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2cbf5577920afcb90ae78f91277b11603c42ce6a
[2cd25f56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2cd25f56b9c2f6e2372d8139d9f506bd52b72557
[2cf4a04c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2cf4a04cac543004d7cb2e1cf11324e6abcbf25d
[2d14cc1f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2d14cc1fe52ee4f1812bc26b139c7fe51d15bd62
[2d1541c4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2d1541c4efefde26182963fb4c82e546ab2f36a4
[2d7f5e25]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2d7f5e25032ddeb48c29af3bbe8dfb9fc564bfd9
[2da33982]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2da339822a4f62266149b8d53925840c0860c9a2
[2dbedd57]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2dbedd57c6800558964aa8aaac8d83c3b3652061
[2dce79d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2dce79d546e4b6455f1d368cdc8af39051e4b876
[2e10dd1c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2e10dd1cab1dd3a68fa66207f03d66e4e2e72c0c
[2e2730a7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2e2730a7e305cda9f975d714f8f25707e5609bf6
[2e60f5f7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2e60f5f739db8b42c9bd1fcea663b4e392a176d7
[2e772970]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2e772970ab86a9541ad56a1702b4a219412ea88b
[2e80d2bf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2e80d2bfb1ba72ddb9fc968e9d5bb6f0ccdb7ce3
[2e95d616]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2e95d616dc5d6cdc9b907935ccd006cdf6e88f0c
[2edd326d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2edd326d590f3077179df4f2ef7433fa9205d9fa
[2edda450]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2edda45071e3593c83d16514bcfa2a81192a6d35
[2f16d924]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2f16d924762ee070ae1095f97e2bb0583125e27b
[2f1fc58e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2f1fc58e6727c42cfc83298c0e421743f63899af
[2f87bf06]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2f87bf0659d10a96a00886a022533150c75dc134
[2f8bedc0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2f8bedc0b5ed5011e63b06044cf3ab6ecb93a94f
[2f9a434c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2f9a434cd13200d95d8ce5e5f0a3f8af1b822a92
[2fd39187]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2fd39187ffa225f0d431bca0c4ed3a67711fa09c
[2fef7441]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2fef7441f3c7db1b558f451f1233919ab7b05414
[2ff0d197]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2ff0d197842c39450275d6d09bdd7ed06db1e735
[2ff222ea]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/2ff222eaf061cbefe372cb88accd2be39b141817
[303b446f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/303b446ff38219ee6df42e97dd1c81a2e3db1851
[3042b50a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3042b50a04da47bd2548c120d8f8ed54c464839b
[30553b79]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/30553b7989b55c802704c42deefe9424347092ee
[3055d42e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3055d42ebb9f85985186ed6c89c6bd75008f8cab
[30638861]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/30638861f40e9c735aa91fa2d1f2efa68bb45f70
[307c86c2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/307c86c22c96ca90ca5456e8dcaf6a83534efbf6
[308628cc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/308628ccfa8cef32aa093817b78983739f52548f
[311b7f43]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/311b7f43f0d50acd79b5c39aec78b9b4bbd4bbbb
[3139418f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3139418f31dea35046a0a34faffbda18993e3eec
[31865ac8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/31865ac80893aa6407270386a50fdf9e823789bb
[31948002]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/319480028edac53f43f1bded7b2a76da9d330400
[31a77692]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/31a7769248144d65da9a67ea0aa1bdda61fb3fd8
[31ef5757]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/31ef5757d16e6e70cbdc6abc7e87c25c2f25be94
[32017711]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/32017711dcbcd3d3f4389d6bbd3fbcb6c6203ad2
[3202c47d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3202c47d3f12cdee31e00bcb529d2cb5cd26de84
[329a2a17]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/329a2a17b6d77199a2c952f073baeaa419fbef14
[329f85cf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/329f85cfbd83c4f9292202e3435afc1b2275830f
[32bff443]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/32bff443ae94f1618a24da34a27d51988cc3c73b
[32c361f4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/32c361f49d70ed1c5c6408c45e958bc26dff31f2
[33015801]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/330158013b8c93ba40f0816e0f16fb0d235b8696
[332d8659]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/332d8659ccdf6ae391dc1856bb68c920f632ea3f
[3334fbf0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3334fbf0dc24e49383427c9387cc9195dd2582b6
[33418d03]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/33418d0311fc75fa7fda97ac621ddf2da493c128
[33447dd1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/33447dd116c091bd968eedf78675dc8c94b46982
[336a3d96]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/336a3d961213baf7cc33ab8aab73aa842225d906
[336ee321]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/336ee3218e27578abbefaf6549df64ec84b99276
[33a492b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/33a492b9c01c57a71191d7f1b46d457d5ff67059
[33e795be]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/33e795be325e9e0a2af1c46f3512f2a4962c39fd
[34320a79]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/34320a79819fceba1810067606990ab35bcf45b0
[34595fe2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/34595fe2693385b0cdff69ecf6306071d058b638
[34b75c5b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/34b75c5ba38708d8ed2f63b3135c0b42b57ab065
[34c117d1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/34c117d148f64acca022411f346570b8eb70adb5
[34cd9539]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/34cd953985ba4284988a6ada11fbce177e49baa9
[34ef7167]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/34ef7167ac032e42c020aaae405efcf3e3e26ad1
[34fbb7d2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/34fbb7d23cb372f1628f69a99b34d89c63305cd1
[3514cb99]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3514cb9974cf90b85518225c10632f8e3015402a
[3562638d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3562638d37272b2befa7f9007307fd4088cdd00c
[3565b306]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3565b306973dc70b0f04a715a3ba9922dcd26487
[35914891]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/35914891bd90cfadbe17867c370c65429883e879
[35b02055]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/35b0205589e316eb9604c31f53c3fe0511c7f62b
[35d3c2eb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/35d3c2ebd561f22cd1903ceb6d5f171d30bd191e
[35e678e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/35e678e97a63317ef3da1936fd54af87e27c88bb
[35fbb1da]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/35fbb1dade0bbfd40042b5154430df5754caa92e
[35fd92ff]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/35fd92ff3c1195eb1e549aa990fa6ef70183c5da
[361a6549]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/361a65495bc89376b23471472b4fe000b6221143
[361e3bc1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/361e3bc103917a3470bf57e2a693b873d05b441b
[361e9867]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/361e986710d8c97932b87bffc096e6af122ef58e
[365012d0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/365012d0165c8a45ad0bec6148804c26274e2000
[36601944]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3660194496ef9ea519720cea59319a84698b5703
[368eba9b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/368eba9b473b0c31ebd232bd89bc2aabd5a15d53
[368fc14c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/368fc14c74d87b771314bdb96763069a7173b076
[37313cdd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/37313cdd74523b6bfa9216494cbf47adf9fe43fc
[379db5a6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/379db5a63e89c4285aaf89b94e4ef4102d3d8ca2
[37a30267]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/37a30267795e98d186745c83446058d6d8e3ec67
[37af405d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/37af405da67bbd27550a8df624db09273b454912
[3805df7d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3805df7db5cc3090568a8ae2316b136f758dc962
[3865ae2f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3865ae2ff184e69f2f1983766a9539fd517b2538
[38d1c473]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/38d1c47305125d9bd4e9275ef99d9767af3f1380
[3911166e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3911166ec66d3f4b8123f3712d1a8171c7e7b096
[39201ec7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/39201ec7556ddeae1f9fcfe71c2898a3a96c3764
[3963e070]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3963e070fd8849ce327d9cdb4ef7bbbe52d0d7e2
[3980d5b6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3980d5b67bbd371d84cbb313f51e950dae436d54
[39bb00e2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/39bb00e232e48e71fc7acc1d6d608ce590bdc733
[39d4b897]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/39d4b897358e38143a054f34c7d6e7aee644e0d7
[3a0ae4ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3a0ae4ba086dcd9c40dc8ad1fb58921150bcf09b
[3a17c26c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3a17c26c094a1da46018dc3dccb2acd348711ce6
[3a2f29f2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3a2f29f2c32f771e365559fc630dd404ff1f17e8
[3a500a4a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3a500a4a9b957189b0298b8f6e579db7c608e27d
[3a6bbe84]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3a6bbe84934da4cafe87eab04c3f1f2271d8cc9a
[3a6eac6b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3a6eac6bab6e45b5cb413176a614cb329c4d3f67
[3aaddac4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3aaddac4d39967807fa2156e11fe5ef31dac8bc8
[3ab52452]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ab52452fb53f978708ae03dda7c0447d759dafd
[3ab90e70]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ab90e709c7059dfe8f17ef2b4c0fb43c0382062
[3ab938d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ab938d5f495d601fa8928a70a2f35502e843a12
[3ac44ab1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ac44ab180b1344b84570ee633a5152df5b66d33
[3ac5374e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ac5374edd67a53e0c1492e487db90e9d36a91fd
[3acad09e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3acad09e8ce7857ccd41b3e771eac3e817988102
[3ad35f02]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ad35f025ebf265c8fe1f4fa0267510d2b163de9
[3ada5b0a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ada5b0ad71dcbdf9fc67aa1a9976ea54f0960ba
[3af9e554]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3af9e5547fdfd6c20a318c7746521eccbd5ede38
[3b0aefe4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3b0aefe474e5936178a9def30799cd4abcf0d263
[3b0e67c6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3b0e67c6337346e3c294c05352b8b28c55bdb438
[3b950014]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3b950014411d743e3e5527f652e5a2d4aff9a847
[3c01b315]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c01b315f0f07d5d88de6992a24725d4b9e8967b
[3c0a15e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c0a15e916605bf02a31935c84474e4306642ba2
[3c0a306c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c0a306c3725b35175a7740396b1371b0b7505be
[3c2b0dfe]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c2b0dfe766fbc2a7ababb303e302d7c8043a5b7
[3c34d099]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c34d099f162ee65423dbee77946622b391955a3
[3c4e8b1b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c4e8b1b37415ad0662019d1792525ab0b00a8f9
[3c5a18a4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c5a18a47f168dfc463411e81b07f3250ba68df0
[3c5b0de8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c5b0de8e1e35b0be1f119f94c6d87bf82087f87
[3c679fda]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c679fda1f4da3de70104d2cc06c322cf7faac31
[3c7984d4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c7984d49df0d91037729a45c24a2a7b5a109687
[3c902662]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3c9026629af4aeb47f7057b0a15cdaa03457d2da
[3cd8f732]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3cd8f732aa09ff81035e320237c1bacc2aa6d8d6
[3ce67599]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ce6759955856eb432c7a02f6223e9fd15a3ac25
[3d2b9431]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3d2b9431b7738b16486f534f07a018d5a4f41a01
[3d4c952b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3d4c952b52e82732e6ab540c2b94553a5ca2614f
[3d78b0d0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3d78b0d0b6918dab784bbe2332b3a26928bb8f90
[3d962df8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3d962df81366bee4e970af8522364826359db7c8
[3daffcc5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3daffcc5f5098e3620985782faed6f6d76c6c52a
[3db6df30]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3db6df3070332db4b294a2dff6ada5c8def9afe9
[3dc2d474]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3dc2d474ce41b8bd872fcd0ef556f5166286f19d
[3df00ac9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3df00ac9e826042b31d9617d81f54df998525031
[3e1607f0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3e1607f076afe7a6e10578776a07d3feb0a2b9a8
[3e70a605]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3e70a605e7642596337bc49deda5a542c75aaddf
[3ee20b9c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3ee20b9c743ac1ab72652b4ea4ab61bd722d8a16
[3fe4262e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/3fe4262ecb7022fb2e80607f3b4d12bc4d9aa72c
[40172427]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/40172427877f7a020afebd8aa772810feeba85b3
[40775be4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/40775be44fab0f09e140e9acc8f7d1b1fdefa3b2
[407fd870]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/407fd8708d2740e76e160e72dfbf4b54425da180
[408aac8c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/408aac8c55b4a7e997c88205eef8f6c4859aa44f
[4094fee8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4094fee8952c99573245d93917c1d0db9aab31c3
[40ad3c39]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/40ad3c39e3531c2e51b34821c27e5120604cf67a
[40e428d6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/40e428d6022a4b800d96c72b57f46263d3bf3212
[41009684]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/410096845dee22b72d8c00929caf05edf44e817e
[41aebcd0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/41aebcd0076289c9b5d61352f96406ac54937581
[41bb25a1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/41bb25a10b5bc70c228f9f5930d4f0aaba9eafbd
[41e6294d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/41e6294d0967ab5c1e56697cd7e320a9538fde19
[41f316ca]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/41f316ca1f282bd5026434292922ca48fc82ee5b
[42169f49]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/42169f49a89756775d1e37c16130bdc2bf503599
[4236304b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4236304bb6d74f76ebc62959b0a5decb5f96ffb7
[4268bc6b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4268bc6bc4657036534627f43966754a4a419ed5
[428926d3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/428926d3c0bbb8ab0dda1285b7d9b81f3e3da998
[42ccefd3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/42ccefd392bfdc1aed8036f948aa74371308a8df
[432b4d50]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/432b4d5038409e00850d679ee9837b57d0bbba46
[4370adae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4370adae232ecac190d46a6828e6a7661cc0d96e
[4371f19f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4371f19f1710a50138b223f07d10fa34baa3bb6c
[437eeb96]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/437eeb964b96a82d569c1293e13fb687a373ab6d
[4384a4a0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4384a4a0b7bbddb8593e60a38bab9ea12709fc98
[43d4cca5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/43d4cca5446981ff9aa3bb5d5b5653c83e75e530
[43d57d9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/43d57d9e9fc8f85eda37eea882932b02ca11f77a
[43dccc0c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/43dccc0c5d1997fec802fbcab8245deb48b6a7d6
[440cbf9d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/440cbf9d744f288a78dfec48479bb7e3ef3ba4eb
[44291c74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/44291c749bfae647cf130fdc298dd2cc5d1876ba
[446970eb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/446970eb203f0522d23cbc6147b10a4c8c06b154
[44bd6b10]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/44bd6b10290eaa4e9ede11765d25eba4b171cbe2
[44c1f3c1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/44c1f3c115fedae121233aea83927d84b20affa0
[44fa812a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/44fa812ad1f50f90cf5fcf88603fad3a44d09783
[45390157]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/45390157c9bb47e2e0506499dbb540046de0786b
[455a8941]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/455a8941a603d30d5018b908865405484bc51729
[459c931b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/459c931b29b6f850c1379a4d9fb51748bad1905b
[45bc7fe2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/45bc7fe285196ec448ee981c547675bc21551545
[45c67a77]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/45c67a77ebf22f8598d3a71ac8678d3416bc0e5e
[45e88462]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/45e88462e33c69998adbc0a3072ab1644da940a2
[463d93f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/463d93f309ab8a320b53711f67a52072566de69c
[467b14db]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/467b14dbc59a60a808e7a71a1083f2490cf31d48
[46bf32e3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/46bf32e3f48d826c9b4fc7f597d04f7d87cd42d8
[46c1502f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/46c1502f07d0481c3aaab5e05899296f25c6ea13
[46d58621]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/46d586215b6f2f2a6357043793df880c3928a2be
[475529ee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/475529ee30fe2a2e8587ffee57a35190626570bd
[476443b1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/476443b17dfaffac3574fb0eea829935bdf88226
[476ed604]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/476ed604e7b4201efe5b6e5f7c4a588c3abaa157
[4772bae1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4772bae173e5c5c3c6c9a6db851297eecb62cb8d
[4799f3c2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4799f3c2a57c0f7e8e96cf97eb52745d2586b61d
[47ab5f3e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/47ab5f3ead22594e8971e3d95f733292a311bc7a
[47d40b49]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/47d40b4983cb14440ea6a6f02416862c5fec9fd5
[48687e8a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/48687e8a65af56cf9c6279702ccaa6a66c127a06
[487138fc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/487138fca7e5cac02347e8597f90b9e1237bd531
[488ea8b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/488ea8b95e76bd58fcaf89c2ebbf30e9ed20b52e
[48bd3aa9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/48bd3aa9c24f5420f54ffdddabd061ec5a25d55b
[48c5a5e4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/48c5a5e40fcf29e581fb329dd8c2e778a4e08b92
[48ed1f56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/48ed1f561cff4dc9a71d186d9a675bcd0bc2b1cc
[48ef3d88]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/48ef3d884fe81ac3be47c7c10089623997b6c259
[492188bd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/492188bdc4240caed845f35a0ca06dd97ab22e55
[49349571]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/49349571ebf016a24036d07cd12f330b285782cf
[494e7732]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/494e77321a94d2e7d2ce56c220baa245636d2ce4
[495b31dd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/495b31dded216aebb2ba050e6ffd7b20e013b866
[49825e22]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/49825e223c11efef97c34fdc2e48692183f807da
[498ad5f4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/498ad5f4cc0af31ac552e7aa6f9ee1b7ef030e13
[498d5f9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/498d5f9e608a3ef6638451119d152da80cafeea3
[4a27516a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4a27516aafe37a4bb60d2417a1755d6b62d8e6cf
[4a43a7b3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4a43a7b33b59744e5b070498613ec1fb61e440e2
[4a79d15d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4a79d15d303a92515b7ee970ddabc3cdd9994b85
[4a7a8d56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4a7a8d5684958a9c5bc85ffc4966ec7f03fe7c7f
[4a7e6798]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4a7e6798eb207fa01eb0dfb95fd2815ed8ecc410
[4a9cfcaf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4a9cfcaf3cbe414e565207fd50a394290ad8c776
[4ac84307]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4ac84307c9432001c1b010ff2aafec0be3b2d4cc
[4adfd641]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4adfd6418c49901b6bca52ad7639145dbb6f6030
[4b115c29]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4b115c29f5fe3a185588774f0f56e45cff2952c9
[4b328c5a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4b328c5a416cc6daeeeedf81f1dc76bc9bbf849a
[4b51b30d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4b51b30d598a6e076f3d2a8b9d3e170deea1325f
[4b582114]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4b582114be44e4cf364cc7cbf13aced5731badcb
[4b784a72]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4b784a723b8c02c7bdb4b51e7d7b76147f97d569
[4b80fa51]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4b80fa51f0034f4adf03fad5fc66329e23602f07
[4bad64e8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4bad64e89f4352d0f856ca1c24642454ebec944a
[4bd2d6db]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4bd2d6dbfcfcf3273c7e92b840d05cc0af4c4a63
[4be561af]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4be561af20268570dc6ebc3c26dafc8d4ef345d6
[4c49260a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4c49260a89addb41e8f48da1c0811b4a965a1c64
[4c7b6f1a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4c7b6f1adf7398dfa570224ca470f8a71870a831
[4c8fdcde]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4c8fdcde911f851e5ad7700706f2664f80706f5c
[4d14dbfc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4d14dbfc53c3270b667f8ce6f955abe250295af8
[4db00567]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4db00567813d9b236c77a49a33e399ad5c0c94ab
[4dc89cea]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4dc89cea3dc10fefc820e2669ebfc4cabc01906e
[4e07ec70]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4e07ec70fbf7804d95ae5b62ab599aaa625a3fd5
[4e32f5ce]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4e32f5ceab4b77a533fcc62ea68377d209b7a282
[4e72f534]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4e72f534df680c78070bd54b145f4102c7c127d5
[4e9d8eb7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4e9d8eb7f507914fa9b93a135b3aebb95703e34e
[4ecaaa46]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4ecaaa467234b4f4abec51d4662be69d917fe6c7
[4eef6221]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4eef6221708137928458ed8445b4f67196500082
[4f0b0c39]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4f0b0c39a6c3d3580d6d755d9765941c6d102e49
[4f2ea4e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4f2ea4e9c9f267b9f14ab6dbfe66f7db7ee03501
[4f3992cf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4f3992cf6256a7c248f803580909aef4c2c8e39f
[4f5175c0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4f5175c0ce7682bca234704147fd38233281600e
[4fc3720a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4fc3720a4f7b8e7af96e703f10b45f03d3dc159d
[4fda60f2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/4fda60f22505ebba9ff86873386d0524d364765c
[5029d051]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5029d0513e0a0e52bb0eb43603e41dc07ab7e47b
[503a5ae0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/503a5ae0e90160e04a148efb0054e9eafba774b1
[504d126e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/504d126ef2c70bf71ada912ad891c2282c87598f
[505280af]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/505280afb5459be37c9905f1e7b23983b2e7e287
[50870c1b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/50870c1bf3cca434ec2bd03624fd690fa59dd588
[50cc3c2d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/50cc3c2d086b15af580ec6e190059f2b59c0233c
[50e3c523]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/50e3c5235ab8aa95ffe58b7114bdf257d4bdeff5
[50fec201]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/50fec201f1094269a1dc53bca88b25e33d7ceec4
[513bb501]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/513bb50140ac284fd4609e6f7af1a22bf1aef27b
[518514e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/518514e9770a169f75e672382474d32abd4f2173
[51ba2e8b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/51ba2e8bc916886586b0e459cc1ada8836ab3b29
[51d5e563]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/51d5e56388ebaa57c24af0c51ae9e81c9711dd1b
[51ebb581]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/51ebb581965dce162f6c7f265af787692e3518dc
[51f0a78b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/51f0a78b2e9128db465ee8e1d4fb147b15a21dc3
[5218d4f8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5218d4f869d62e2d1ba8a0d8d75932848f36df9c
[5239fb75]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5239fb758f907c043362b302c29787f5dd955447
[524988dc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/524988dca0e06a9ea2e1b72bead518a354daa03e
[524e25e8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/524e25e81c55d6550a3f8ae37ba694aee0d0b39f
[524e269e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/524e269e8b6584fdcd60ff551a4f0a5d49e7384e
[52777c1e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/52777c1eb5ff968430cb678630f01a100763b967
[53260fca]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53260fca359c3e2a87af42262bd56fe0d40d2bd6
[5337dd90]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5337dd907a456236ebd038f7b3df6fa4e1687a68
[53419617]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/534196171964e292daae305a8c83ec1aa280061d
[53558dab]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53558dab635505ad61590eb67d616c70026b8d62
[53596672]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53596672c4cc4e9b47ee60d4e96af69aeb21d3dd
[53892a3a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53892a3a2d46c3b7dcad3b0fd7b4141118485e9e
[5390f7fa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5390f7fa3d92f1e05dfb22084b6f20ea7566b279
[53a3cabf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53a3cabfbbbbd4d0c7a48f2ecd31c6c61d620c15
[53cfb929]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53cfb929a25cb09e633db86cc1615bc41758ed44
[53d9cbc7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53d9cbc7bba17cda49f1afa774582b3a93b39766
[53e8a759]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53e8a75974dfe4bf11740ad80c0fe769dfa0ebdb
[53f8b909]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/53f8b909430782973609e0daaba70495bfbfbb73
[5431942b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5431942b57db8b3a20efc3e74edff6f047635141
[545e2804]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/545e2804233ab290498f26c97beb2833d388e923
[54b08603]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/54b0860302e2b691ef6ca54c1bde09fa97e1e3b2
[54b7d896]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/54b7d89622e43335c74c730d4df4ca825be27e19
[54c3e09c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/54c3e09ce49771e62fcc3bb2938f259767e002ba
[55113bc5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/55113bc5705d5c9ace1da6bde8b05c1260ddb935
[5543ff72]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5543ff7204b50723dbdcd9042bd9747b74821bfb
[556ed6c9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/556ed6c9b8a8897e0c344fe579be36beaa726cbb
[557301ae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/557301aee14b8ef638067e094eba38d9af03c2ed
[55a1e55e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/55a1e55ed94f8e8a4202098c1fd4f85e337bfae4
[55b8777a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/55b8777a54b334efff499db31f8a3565837e25ee
[56078e8b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/56078e8bb713fa861ccf9ebd1a58415ee6173819
[5618d051]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5618d051819a9874a5db33747523553ba1f906c9
[5624bb3a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5624bb3ad95aa05dc279e89d983269267a8f1c4d
[5627c971]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5627c97183033e99b57c17711e8f35619fb820c9
[56338193]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/56338193b79d76f5ef82ebcec17327b9f965bf04
[566b1b62]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/566b1b62bf7e5586a70f11086edd1379bd372a5a
[567394fb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/567394fb63861fa0ad809e5fc1da6bab9e790540
[56844a29]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/56844a29630759ea75a09f47a786e49a4c0630f7
[56adf881]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/56adf88149e87aebbf87c70fc4531545d2c11daa
[56c0743c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/56c0743c08220cdec119838bda764f89ca0fa5b2
[56d147ee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/56d147ee15b5d3c218df37cb46f4ff69c4e78c3b
[56f7b0bf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/56f7b0bfc6450399df5983507a8f4c757d48d9ab
[570d4820]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/570d48207f1103d61b2026870b9914c5ab76de51
[573b7739]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/573b773921d10fdd536960552bb62936f4f90a09
[573c4c7a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/573c4c7a06554b7657339d4a64dde61c12133e1c
[5750cf34]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5750cf34d80a90e7134cbf8c57619d3ff1abbd98
[57a149c6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/57a149c601e6dbfa2266c499370fded5e5a95290
[57bf4db5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/57bf4db5f04905605e7bf81a05300f0fc80eac66
[57d70052]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/57d7005211a6ef5bad04320be824c829b836cb75
[57e2a4d7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/57e2a4d7c2b8a169d83f52162b87c52e09d23f67
[581c6eb5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/581c6eb503c072c27b5e646236c01276c9561e25
[58289d1d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/58289d1d168a2afa4e76eb468b0f0be43ea167eb
[58d2f205]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/58d2f2059a7f43f7bdaff90043799e64ede338b6
[58d6ec9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/58d6ec9e31775e24ead4876dd543d8cb9cf559a4
[58d9d79a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/58d9d79a5482fa0d1a555623e33f588de9665bbd
[5914ce02]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5914ce023baa243931d4f0862195e84fd48db515
[5918660c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5918660c358847b8d4e4e8614e39ad7b8c9df766
[592cbba7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/592cbba7e099657ce0400b772f0cc9a3bab8021d
[596f5b9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/596f5b9e3d07b9dfb51275473151af2b3bba301b
[597c0807]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/597c0807e426e2d17f6a6b49a37665899b6bc074
[59890260]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/59890260416de09392aaac2c24b3c9c9ce9200d7
[59e39b73]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/59e39b73f4b4f3867ef8f6091f854ea128568579
[5ae0f033]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5ae0f033f00db93b45494ca7f0d93946e3633c63
[5af3dbf7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5af3dbf701cba5ccd809c133da64d762a8f7e5a9
[5b12554d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5b12554d16d927c6b888c7dbd8bfcb29bcf50c31
[5b2b6f80]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5b2b6f808c100077fc94c7019821a01896f7b652
[5b42c314]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5b42c314e48ece1fe2a87ecb9cee51f6f7a0742b
[5b4d35c6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5b4d35c6f81bfbf7febad51106f86e59e93ba693
[5b4d5fc8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5b4d5fc816cee8c9701ccdfdba85b5e49e7ee993
[5b9948b4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5b9948b40600e4a1447239880baf79f90795af18
[5bb8a5d1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5bb8a5d1c4331d8e4b0391509abae2277012215d
[5c811752]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5c811752a8a9b5b7b88f8d597947ecd5265f4ab6
[5c92c761]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5c92c761e1e3e7075ec322557a00b860d5f551a0
[5c99c2c5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5c99c2c5feaee149c9abaad3e2867b94ae7e10e3
[5caa8dd6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5caa8dd6b7f8f2735366a23ab1005df89aaf565f
[5cabf7b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5cabf7b91b44fb508cbb90ea8299fd78088323be
[5cf38ef9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5cf38ef94f30a0588be9c4ad16bc8a67f952b134
[5d2e4ae2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5d2e4ae25927561221f5d49f4cd412ece8222594
[5d4beee6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5d4beee623c868307cc3f9d5f738d88c0b9af71a
[5d5ce99c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5d5ce99c92198d0e843259c1d32f08ba87d0039b
[5d64753c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5d64753c520ec874b8c160454199a98a59335e98
[5d741156]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5d7411564efbd190fdf3aae8779b2e936bb85725
[5dba3193]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5dba319304173835841650b116297b54fac0e3be
[5dd008a2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5dd008a2e5b22bc4e8d3760ce54d6fa0f7a634bc
[5de3665a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5de3665a80724db7b4b1e3d4b70df8c8ed43c80f
[5de73699]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5de7369902c280e791f84c20f29add1e7dbff900
[5de9a326]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5de9a3260cc3b171df72886919b41abb44fb0517
[5dfc932d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5dfc932d8f62e295f93accafb98c533fd8e39625
[5e0ebce3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5e0ebce3dc201981a03e37b79e4a67f6f7c01f84
[5e4e41da]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5e4e41da81de9f2e13bfe8764611a33774b070ba
[5e7a4e84]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5e7a4e84ed5eae0bd32aaf7ad2f61f34614097e6
[5e975901]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5e975901312e13a35d8599fc06bd0536f4c61e9e
[5eaa48ff]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5eaa48ff8fe3b9db875ea81d6f0e7a7fb408b448
[5f494207]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5f4942071d96e64b27bc704b78fc5f2b7009ba1d
[5f5606de]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5f5606def37b70ac96d7045fa2ee36156b4d4f28
[5f588677]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5f588677c505aaa1af883f16ab813f6702f6240b
[5f77a0f7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5f77a0f7ddbc194ffdc1e45556e2c85910002af6
[5f8a63e4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5f8a63e4ececc6d35568835017fe1c3149b1c086
[5f93e4ad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5f93e4ad56fdbda6b5ceeaeca94848063064cc9a
[5fa013ec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/5fa013ec04e4830e9c78c3fa9a342f2cdd525da6
[6002cf1e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6002cf1e30b497e80bdd4ebf59c7112a30cc66b1
[6039af39]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6039af3920bcdd7daf93e0af2357be6eeec33b97
[607a23c0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/607a23c00aff0d9b34fb3d678bdfd5c14290582d
[60aa5d1c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/60aa5d1c1e983aad9b0921e3e066935742605b52
[610785f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/610785f3d98a4032fe7053076f9db45d4c1d1717
[612c4533]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/612c453362b92699806a195a995c6a3a8443a5d7
[61313d52]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/61313d5239ca137a193c6d6cd964c483a5ff0e2a
[616bef16]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/616bef16cacceb26dae625be830141b8ab2252e7
[619da415]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/619da4158649e2fc038bc0ecb9b36e82931508b6
[61f15e13]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/61f15e13d086a5d6c0e5a18d44c730ebe77a046a
[6215abed]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6215abed351ae52e952811854d1d72d39beda271
[62a6f0c3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/62a6f0c3a524408acf7086b9523b965cc2e892a7
[62e89e29]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/62e89e2917a36d73110a860a5f490e4fbb19a6b2
[630b8ce9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/630b8ce945dd2f87d88c357afec26a0a5bdbed60
[635ce62a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/635ce62a78604a726427fb5a026c206c5868c643
[63f7d592]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/63f7d5928d19cf140b9a775b6e9b39d7c6f66d1a
[64055794]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/64055794118b3e2a7b27d7e83f22efe219fcd9f4
[6408589c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6408589ca5c49c28822b5fc2e3cacb1903db1fca
[646d8335]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/646d833513f8c490213888d47d6c35121cdd2d52
[64b76e7c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/64b76e7c066defc6e22153f7881a41ba5b9df913
[64ff8e2b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/64ff8e2bb0d9beee4b0967e281cb1e02b97a3b82
[65794851]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/657948516a05c40cd0d9c35dc639d05eeafa5dc5
[659c824e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/659c824e36450d279d6fd684bdf848530da137f5
[65c00784]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/65c007844cc6c275b86b36a2ff1b48340622a681
[6602a97a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6602a97a01eb9ad37c10fe2828511624d5d43a71
[66414e9d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/66414e9db930cd116e78a692fa0590a3f574aea2
[66595090]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6659509095d19163bd65bd24a9a554cf25207395
[6660f686]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6660f686f02ca2d98c9cdfe3c72cc906e446df1f
[666c3cf2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/666c3cf2a339e4be7d4f263699fa6664cb2f9b70
[6671e40e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6671e40e458f0068097135fb37f7f5a279367396
[66c753ab]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/66c753ab9e6d46a5ce1c27e1e777e1ed8e78f45e
[66eea98e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/66eea98eb83cb5d80ff5ce094c8da7b63e8c74d6
[671bbeb5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/671bbeb564cd0b2f055440fc27db09bb1e725c38
[6723d4f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6723d4f3bc77e3d2dc734318d250e7bd410bdca8
[6745fb1d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6745fb1d2188953db8a24146f9fd5dcf838a9741
[676633a7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/676633a72f464a1f71b369281207390fb1c2efd5
[678196ee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/678196eeb7527d0fc06e27b049957f009d7aaea4
[6798a5ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6798a5ba576680384ef4d5cbd5706be62b353741
[67c5a8d4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/67c5a8d4e2fb420515eb4d6f4249d39651154478
[67cb45a4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/67cb45a47f6c277bc0afc9ac9689b2a05d7b5049
[67f779c3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/67f779c32db3b4d5857dbbcd76c6fee77e050505
[6805e2b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6805e2b9257cecb545417531a008ec139a0b5c54
[683cf870]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/683cf8702f2873329a8fa3c543c2124f2c9bbe20
[684d9043]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/684d9043580b42c3bdb2325b799e7042ac4d8fc8
[68583d84]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/68583d84a3722a27bec69a77984cd9e1167929bc
[68725bea]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/68725bea1a54d7f03cd79e1fe60a218350189f7b
[68e712a2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/68e712a2e6d8bfa6441b87353ebaa794832293c7
[69149fd5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/69149fd5c6853eb2bf91f832b098e3a516345284
[6947cb9f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6947cb9f746f187d0834db028f0873e44e32d07b
[694ce3d3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/694ce3d36610c3316dc51e078d918381b03dccd3
[6955a7b5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6955a7b5c389a9b16651bf7e2350e12df2bc22a2
[69767ad6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/69767ad626e27e0fda881c1e62b374165bd17825
[699015e0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/699015e02fa89058bb1379f5944bde296b0603e6
[69abc77c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/69abc77c352e813dc24128e9952da72c77979f1a
[69c80568]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/69c805681f123908bf1d778273411411df5c3bfd
[69ef9f7b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/69ef9f7b8d04b0afa5885040b51ef50c18873fea
[6a327d18]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6a327d18341b245c61839b70cba29dc91f888f1b
[6a4ef68e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6a4ef68e97e99f4cda9398e7dc9816e50a071797
[6a91fb78]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6a91fb78ea15aa2ba8066ee2b957fb237eb8e67e
[6ab2c273]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6ab2c2736ccff8e35be179a3b9b2935ba3c4d85c
[6ab5a2a5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6ab5a2a53053db5708cdb45154ef77045c4b72aa
[6ad1d90b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6ad1d90b8724e2f1f508dc3e3823ae2d127b1d80
[6afdf1bf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6afdf1bf9768138c46540eee53f6aecca230fb61
[6b47b9c1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6b47b9c1bc8b1cf297c987b3d4321cfe654238f5
[6b8d31e1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6b8d31e186f4b3998a0ccadd926d8f7255fabeae
[6ba2a65b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6ba2a65bb70a092b41c45c87099989a6b507c54a
[6baae7cc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6baae7cc71bf42de7584025bf53843f3c0ff8f6c
[6c1954e7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6c1954e7475b3ff64bedce86b8301b1e0f624c95
[6c23f866]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6c23f8661caec401479fdc99ec94d4470563552b
[6c29b163]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6c29b163a6b9b7f1000216aab8f20772f20dfe0b
[6c825065]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6c825065687ef0469d4a4d1a64b9ef9e75e9ebea
[6ca9634b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6ca9634b9e868e2f19c0a881f73c980bbf3c060e
[6d0db2ea]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6d0db2ea0d86db10533bc94162d092f88d05b761
[6d64fd6d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6d64fd6d8ee1b5338877004d22ecfaf18ed47ba7
[6d67cdbe]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6d67cdbe11918f47fc440ea285a73d2be7b870d3
[6d892736]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6d89273608a68aaf4abc32f52a6739cca4fe3a77
[6d9313f6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6d9313f633048ff4f0486135c868a6ca188c1fec
[6dd2195a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6dd2195ad33b243e17f36ab7c9a46fb5377206fe
[6def544d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6def544df7dd7d775f024003668b190ba97664d3
[6e0c5daf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6e0c5daf53c7495e71b647cdd8df661121c385bd
[6e14d696]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6e14d6965e189061f0374d335dcd8beeb4492e37
[6e284ff0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6e284ff06182c3f684c16d49c6bfba8795a862b6
[6e688085]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6e68808588e0bb24fee292f2b236ed4adcbcbfd2
[6e9c45f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6e9c45f1eb29bc52e156e4818226d0022148b4e6
[6eb17c7c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6eb17c7cba6ef01b1f03cf7c984406b41547ce3f
[6eff5c55]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6eff5c553e8d094df5a0c4fe16fde501f0ee08f2
[6f355371]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6f355371988c2056eb5c09beff91e391dcb610e2
[6f36d21e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6f36d21e829ced48a2de9dc1ee6ed4e51b02aa78
[6f48d443]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6f48d44310f9bc0bb94cedfb769edccd856a065f
[6f4a2557]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6f4a255762be50bcee6f667f6af422b6eb5a2fef
[6f5862e3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6f5862e346d005ab5087d012eb6a448bd65924d0
[6f722ae7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6f722ae79553a4ce8c226dbdbff8f5b05a95edbe
[6f7831f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6f7831f10b187cb72f0ec7568db8ae9c8482a146
[6fa05388]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6fa0538800ceeb730fcd90e00831a9a57cffd1de
[6fdcc1ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6fdcc1ba535a0aa76d78e31ade5d6e53251feb9d
[6ff633cc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/6ff633ccb93a9e75e0e0b7291a2571921d85092a
[702e54ad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/702e54ad9b9d46ee77cc03837c2dbe58bcca5486
[705dd66a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/705dd66a0d7771b67d2d1b57de9619cd969939f7
[707d4729]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/707d472928022d51dc7da2fe5322194928871f5b
[7087d4ff]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7087d4ff26c96c2ae3d50bb8eb5e367b80d9b4b6
[709b5293]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/709b52933631bfa5e9661a8273bcb0f544de6ad8
[70a30a2b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/70a30a2b0b651a438223f7249c6ce47931acaa92
[70dcb6e4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/70dcb6e4bebf26ed60cd26df4eb321417f106da5
[7109c46d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7109c46d835f4d6f32b6284681a6286b68179abc
[713bb735]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/713bb7354358091926e524d3f29330f16da3646e
[715cff55]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/715cff55bf0dd38c2538a3a522fa7503f2e86ec1
[7185ba47]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7185ba477d0d184f9cdf28eb485e3ec4e5963f3b
[7193ca3c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7193ca3c77fe38f736726cdf652ba17dc9bd8848
[71d1a55a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/71d1a55a39085d76b20f8ef7451b359afd0e53d3
[7202b7db]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7202b7db011d49a88bf0bafac2fe737a775c132f
[72aa6c8b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/72aa6c8ba53ea2b62c6b8a69e4fcb9945ea1f53b
[72bb96ee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/72bb96eeabc43824f51eca96e4a5312c20833eeb
[72ec4d9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/72ec4d9e0fbd18d29917ecbf012fa6b31a523d57
[732f8e65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/732f8e653e6b703586012a57498eec78b2562a21
[738bd507]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/738bd507f819eb011b1b6a3ff9677403f4e0401a
[73c1ac58]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/73c1ac586b1e2d510f82f33d489a126b5baf1395
[742e6f93]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/742e6f93c328e8c21be8298964621cd6f4d6053d
[7432b7c5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7432b7c5ea20165e443d6440fcc3cb84393f3a96
[746530e4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/746530e41d5830b35a0120b628a25f671aae376f
[749f8ce7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/749f8ce7aab2be9f0bf16133127a0d7fde3046c3
[74d74c11]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/74d74c11cc01287db9fc4bcb260a9afe14a611c9
[74e0ee80]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/74e0ee800aab688767c3a8f924a06f0fdcb5eb2e
[753c5c3a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/753c5c3a770359455221e411c0a75bb95ab936e4
[7557a59e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7557a59e99faf297d2055d5d9ea86b4fbfe8ba5e
[7559d3e1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7559d3e1c4e0514b139a73805076ba668aa92534
[755c269b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/755c269b19304b84c9901e5e4a76bd5cbf596148
[7592b273]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7592b27309dda5f0dd63f7cc75cbf3e875cee9d4
[75afb738]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/75afb738956ea70ba649fe5e2ce3fced28d00495
[75c16e41]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/75c16e41b15f72d10b8fbf714f5c17c22002a543
[75d48b0e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/75d48b0ea51d6cc73085bd253d1abd7989a3a059
[75f7c9fa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/75f7c9fa0fdb16e471281c701b71759e728df81d
[764689fe]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/764689fe0693c6a8588d13cde1c73f42e08b2a39
[7673f395]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7673f3950a1004f4e234fbc5d2a1dfd90fa1b563
[770a0aee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/770a0aee2f4867426d6c01d115cc4741b6d2404d
[77582527]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7758252710b99f7a822df05af0c89b383c8d80b5
[7764150f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7764150fe3836a2092f9239a2638f2c244c69f0e
[77c38995]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/77c38995ac688cfdafa118665d516e90fc024dea
[77cc4977]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/77cc4977191285f81e35e7d08234bc8b0c9cf577
[77fc6038]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/77fc6038df9b58b8996cdca65d8db204492b637e
[78106320]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/781063204d0437481e6033a3f1cf5c6c66db102f
[781fbebb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/781fbebbb4fa266cbe3f8dc4e6fb8a322d7232e9
[782abbf7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/782abbf72853982aff998d9da245e4a0a246c129
[78325c0e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/78325c0eb6e0b167ea9e13cda902ffdab80916f7
[78579a91]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/78579a9158dc463e33c7b5ce1d248258bac89ae3
[78581dba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/78581dba9d33c9565fa25f0a829ca383471335f2
[786b1543]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/786b15432580ab6eb7bb4fde9f4dedcab624a80b
[7887ee46]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7887ee46a7a8271f5e65462f4dfaae9a29aef965
[789265aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/789265aa3096ca580b45c9f135b0be7414362549
[78d26831]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/78d268318289907bd946e3fc1296b4252628a0b2
[78eea85d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/78eea85d6e63e5833550808ea3df1b19d83fdff3
[797ecd1c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/797ecd1c4fe3c45f151700e8c52e01ad2c1364be
[79c1ea1e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/79c1ea1edbed94e3376aed37b382d069144d6fab
[79f13700]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/79f13700effa9948204e4639ef8fe238471211b9
[7a22f9f4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7a22f9f4125fd49fcbb65a54c85f69c2ef728467
[7a3187bf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7a3187bf3d0b58aee3d25f3764945a91afe954fe
[7a448d6d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7a448d6d63086b469af658a6bd3ee8b361ee2ac0
[7a4558e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7a4558e9204726bbf69f6e654ec6dfe812d5f6a2
[7ab4f3bb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7ab4f3bb2df22ad85c611282f5a09ace3ebb9be0
[7ad9cc4a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7ad9cc4ad526b2e79208e8d3ca54d928847f09b8
[7b2037ef]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7b2037ef879097e583f5fde21899a6c53b1d47b5
[7b3593c6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7b3593c6dfd82b8f6475d2def0f8e4146d257f63
[7b973854]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7b973854883b53e04b912ee618a47dffc137ab66
[7bdfa4df]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7bdfa4df5ac857c185a7ce1dd4851637de86fa63
[7c0f4bb1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7c0f4bb157113c31552a704d5b8c94d8184e67e0
[7c335c2c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7c335c2c69ab0802d374bf0e4ccab930af4d2108
[7c51ee62]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7c51ee62e8477e4ae1dbc9681f76ab4c804d0059
[7c65a387]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7c65a3879a8566e7f7584eb7f805693606513e38
[7c701ac8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7c701ac893bbc90cefa01a54bea9317ee1228840
[7c94c990]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7c94c9908429b2cb09fb36e1bf3b378f501ce59c
[7ca59ef4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7ca59ef47803ae183c7d72461324730f4a65f25b
[7cae5f65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7cae5f65eb20872d32370332ded049aec6ce00fe
[7cc45a6e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7cc45a6eda2364573162f4764d67fe7ffef4ed9a
[7cc5fbcc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7cc5fbcc0c214c4daedfd3cc447fd788864fd9f9
[7cdaa4b6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7cdaa4b6ac2c12f3829f345c8c56bd7bf6c19b13
[7ce31155]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7ce31155d3f8940eab315ee2100d5e67ba10d12c
[7ce50613]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7ce506132069e9efebce410cd67317362992183c
[7ceb8e85]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7ceb8e85e551b1003e640c937a142ccf15a8cccc
[7d3305cc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d3305cc028ee0f963e0def854350e9d3eb69cb0
[7d5de9b5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d5de9b5692be5ce4d80c66a687242783e06db2a
[7d618a89]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d618a896c0d07526359dccf1f45eca28ce85bcf
[7d6d9f2d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d6d9f2d701713046452cae3eb740a7ea6c2ea59
[7d8d35aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d8d35aa8649386937ec73db7b20ea67eb7bd54f
[7d97c64e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d97c64e3d28115e395e28c89e56fadc8b26f0af
[7d9bf20b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d9bf20bd40e7c3f26c9b116af9e68d57ceca44d
[7d9f16f8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7d9f16f8325e780b2c222eb72d6b9b3344ee3198
[7daa34c8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7daa34c8b08e3f3d05aa8257b172d96441015321
[7db097f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7db097f35da98d6166b671a714d7c307b4f8958f
[7de7f5cd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7de7f5cdaa180b2b8b7d0c63909e60e5c0016661
[7dea777e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7dea777eb2add6209075fa882b4500aae8cf2184
[7e422759]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7e4227598054e4845ef2f4114b4ffa13313f0e9a
[7e94281e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7e94281e085d756131ff041a04b46300fe116db8
[7eed267c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7eed267ceaf0fd2aba8e34e9901d0bef0302fa3c
[7f01cdf0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7f01cdf0b3880d4ea99f6288056589acbbdc4932
[7f3911b0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7f3911b07cc4788fe2cdb4e8d421fe5f156cac59
[7f700e64]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7f700e642e89cc7e65d89430f8f02e501ad7cdd1
[7f751c12]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7f751c12debde6b2dcd7377d880b20d2aa834f40
[7f8118bc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7f8118bc8f1f29da2aeaa96b7223fec60c58286d
[7f92acab]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7f92acabc25d025e359078afca749d50d05358da
[7f9535da]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7f9535da6c2513ffa99d5b4888864d0c911000d6
[7fab005a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7fab005a52158c37d94d474efe85052bacecc5b5
[7fb6abe6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7fb6abe6a51d0fa63c6ef1a543d5888cd43d5550
[7fcd9015]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7fcd9015defb52c7e432f68ec5d16edf3e966ccd
[7fe0d4ea]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/7fe0d4ea7e3afa19f48fe1bac366879105ca3137
[801ab5c8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/801ab5c8360bdf47a16a28edb5f8d22ba703b077
[802166de]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/802166ded1eccfa794770aba3b6ea02005415b4a
[808ef682]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/808ef6827cce5519a37305d7d02062ba079299c9
[80d06ddf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/80d06ddf9245d7653827efa9aa8315ed2c634b11
[80e0b599]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/80e0b5990d863cd258cf461bda485ae048d17964
[810b1e55]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/810b1e55939fb65282c4378117653f1a4835a96d
[8123ef18]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8123ef1843ffed2f79a403105d3bdc819c9bb0ba
[81446a51]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/81446a51e3946b6af088adf758b119c65a6a0d2f
[816d4f14]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/816d4f1444e86f7466f102e4596ad02bcf655d75
[81959bd1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/81959bd1a9129ff2f8419150d7dcf5fae8feacd0
[81bb423f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/81bb423f8cb3a292a5e9fd5b694be00ec6a83899
[81c437b0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/81c437b02b967b56dcb9f5fa0a25b083dfa3ed25
[81cb3630]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/81cb3630aaa9ab6fc199645928dcd3fa0a9f9ad4
[81cff5d9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/81cff5d9b0bceb1bc56f3e116f5f8eb1550433ac
[81f94b8b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/81f94b8ba9c8e0d35fddd828b1a1c4f10a9202bc
[821707dd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/821707dde8b89c16024fb75aeb4e07c7c0a1469a
[82255c0c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/82255c0ccbe5a57f2906ebb5626b75047f1ce20e
[82259968]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/822599689d7cef3844b5b602352e3e18197a00b7
[822f58cf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/822f58cf7bbe96d5b090fe96a9166aa39b061b9b
[8265bff2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8265bff26c8e4088466c4b2550646e5a0a4bb25c
[830dbbd5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/830dbbd5e18ad8d53727026536b1b07c58411c35
[8314462d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8314462dc2155112b74a7bd91f27d54144f1c433
[831f3420]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/831f34203849e6dced78460d33dd6752b2cae62d
[835cdde6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/835cdde621649192f3b502a3fc98a82bb5cdace9
[837d6f50]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/837d6f5012a4bf8c19cb089a45f6b161409552cf
[83819c8c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/83819c8c16cef8347c88d14c0c0f82381590d371
[83a2d429]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/83a2d42959815253169c14640ea6e75349a1fb7a
[83bc25eb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/83bc25eb8a45c2b0973f1d353fdc176ba630c38f
[840191a6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/840191a6c216a3ca7a5314294e545999a1e6656b
[844c1e06]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/844c1e06cd698eced1c6cd6f50645180b340ee82
[84686ec1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84686ec1216d42b3058443a690d1870232835200
[84a92911]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84a92911775e1493e636af22e5230c2a5b50fb65
[84abef39]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84abef39f6067a82a3fd749938d17b372bbf660b
[84b0d3c9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84b0d3c9720bb19b83aacd0a56ee832eb8b540af
[84ba43d0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84ba43d0b276e12f243c54ba0df929349e87fad1
[84cc63c2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84cc63c2d8c1398a4aa11da4e4e2d07abed4c04b
[84ee6cca]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84ee6cca1eac64fcaff8d2484c847ea3f2e20326
[84f30592]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/84f30592bad9b4395dc770d44dc807125d2ced02
[850efa34]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/850efa341cb0e3dcd063b2f4ae603386805886f6
[8514bb9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8514bb9e5dd831640c5a05509c67ed7573c23975
[854bffd0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/854bffd0ad16ce174fc49f5c6bfb0c019d605424
[8575cd87]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8575cd87bd06ba3ad185a1be33aadd4022bbae40
[858a2276]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/858a227617f5f086ab74b16455082c5bf1b2983c
[85a86363]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/85a86363786f0231771400afd8d70d75ba846382
[85bb57fb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/85bb57fb3465c4ff46d220a69d02fef0c304fb4a
[85d51b6b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/85d51b6b68b108f4a7c8cb9455961420c2cfff43
[85d5ba25]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/85d5ba25c3b919f1c4b1159630de4702e126d5a9
[860b213f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/860b213f7690873f076c098c74b83bb8822a1ba9
[861be671]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/861be6713cfc5f4996251fe23e26f67dd80001d8
[86209a0a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/86209a0a4537d49868a44fdc9a0122fc20931e1e
[8633d667]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8633d667dbb20d7dafa673925ad445d3e4691afc
[86342232]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/863422325dcb628c127c701b18c9b623ef366666
[867a6fe2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/867a6fe259da60c9b4a5b9f4dc7b108605dca294
[868d5c9a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/868d5c9a81123f34aa5fa1f62b788c8c0c5e2f01
[86a539fd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/86a539fdeb1138a2311e2830321544d2941d227a
[86bb1ba7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/86bb1ba709926395b69d950338040c508eb69b49
[86befefa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/86befefa626ef6c9e0a695c0daf8b4f120c785a0
[86c59498]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/86c59498632bf3fa833bf8d4403cfe45468dfe59
[87227cc4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/87227cc4a04689d21e3ff3a521f9b8be235bac5c
[87325d0c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/87325d0ca00d3805225a956f770903bd81149cd1
[875587e7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/875587e713ad30e5d63b4160016783608fe8d2dd
[877177bc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/877177bc4a0ca42544ece0facd2f40273b86c239
[879f3512]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/879f35120bd0d213e7ec04402e8f8c1283359a9d
[87a884bb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/87a884bbfc07b43cf6b2cf8dadc64eab8bf7a702
[87aa9759]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/87aa97590d46a79531f3ed75123d6c03e6085d9c
[880e35ae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/880e35aeedfb5449626a03c9131a1ccd33e017e3
[8821b40c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8821b40c3d4992a09b44e1e69d676d91296277be
[88285895]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/88285895f713aac51e80d3204e167b818a5fa0e7
[883d0f01]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/883d0f01d29d4f8b4683a01ed5a54d2463b97ea9
[88519a06]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/88519a067c2bc8bad28184dc618ac3e726b84cab
[88bdfbb0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/88bdfbb096ee822387a92e319bcf810b466ac8e1
[88d1f864]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/88d1f8644c27a9b49f6d750bc16b58608c197c19
[88fa09ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/88fa09ba243fda22c3d3fb07d017e7ab709bdfbd
[89182037]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/89182037723a3c7582502aa77c456e9e5527c6d9
[89209f92]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/89209f92231ab984cf10552584f08e6bf7b2767b
[892bb61b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/892bb61b24d733da247f12483d4e043643a30dee
[89aca0bc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/89aca0bca438911651fa67f07f63ab87973d32ef
[89dd70d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/89dd70d5efd9d2a92645f37196554e43ec8678ad
[89e91d6b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/89e91d6b1a2dd7c97795c4e9e28a01a5f0163fe3
[89f67f30]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/89f67f3024dd9805313a2cff67ca6e0dc901fb40
[8a40accd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8a40accdeb16238606a202715082af00a61e795d
[8a47ad83]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8a47ad83cd68c7eec60db4ff734f8ead3d54b977
[8a50df10]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8a50df10142c2c8d6c81eaabfb10919d1c3efa0b
[8a6af558]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8a6af558540e081c7da03c451bae439fca147e49
[8ab1f705]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ab1f7058ba9257b221da2b9f2646e316b4dbe36
[8ab4b909]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ab4b9092ea46a5728e1a1313a8eb5a026037c51
[8ba2106c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ba2106c388aa4f785b02962ea408b1945b61280
[8ba36163]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ba36163eaf79b9f930a4add0dd575a351e9976e
[8bdeafdd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8bdeafddf2131da83ad916da83ddacb27c363132
[8be5e30f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8be5e30f4b82fc97cb03e05d086412e050b333db
[8bee504d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8bee504d983335151c4aae8ae6ec94e1ff04e949
[8bf2c812]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8bf2c81214b7da088b688badfbc40abd54e9c07b
[8c1cd0b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8c1cd0b9b9eefc15ec55874aa724ff341b2f75b1
[8c23fd05]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8c23fd059412310208b811d5d771caf617f3d0c0
[8c4a90d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8c4a90d5fe6f0c861598b192638be8c7bbe7d3fc
[8c5ca504]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8c5ca504e6f5052fe5a23dafea37c96a1f914269
[8c7b2393]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8c7b2393ff26eddf56cab7aa8973873549841626
[8c9a919d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8c9a919deb3aed74a11f45ca375f28ded421f4c5
[8cd40b52]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8cd40b522d2e87dd69dd21704c5f128d6d05847b
[8cd73fc7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8cd73fc7e4c86689f449a37603673c5cab805ba2
[8d487876]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8d487876af1303b41c5a73c786d268eac57de224
[8d5e6860]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8d5e6860ed3b6b5c3743bf5d9a5122a78cdccb3c
[8d6665b3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8d6665b388f43063930d2a1d5c3262ca1b372fde
[8d6eed86]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8d6eed865993e4958b99c29214d13cff9f32be1f
[8da8ed5c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8da8ed5c1b92cc01e9b861dadc962c6dc2c2a5f9
[8dcf0dcd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8dcf0dcd9fe76cdef788dcc5efa22457ba4d52b4
[8def2f69]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8def2f69f1d0b55c73ccb0fe7e35435b67d79c6f
[8e3b72a7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8e3b72a7c1e70dad9eacc62cb3171754799c79a6
[8e46a515]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8e46a5152d4f2e95703d35b5c4dcd5ea95257439
[8e46d08e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8e46d08ec9543912761fe24c8cae5162b417586d
[8e6fc80b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8e6fc80beb3c1cbc52fcab7bdd8aad57c84806fe
[8e98a8f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8e98a8f387132a3a13f53d359086a80caa484cfd
[8eb80483]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8eb80483cf954c8990c00aa988c65714a5d5e99c
[8ebef82d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ebef82dc66926ed25004f7efdf7083f6ce231a2
[8ec4d012]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ec4d012fffb16013940d723077cdc44af0b156a
[8ec6c9fa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ec6c9fa98af2d500520b802df6aeb2f78ae3236
[8ec6e240]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8ec6e2403291c0554fac111da98c4c89f6973801
[8f0eee74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8f0eee74270820d5b04eb0c6f48934969ed5bc4c
[8f143748]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8f1437483337f24a4c728b61d1754f9455ee0f5b
[8f29c2f8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8f29c2f8d5987e4771fb96ec81319a577ced4694
[8f38523f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8f38523fea7f20581f55ddd2ec697da8ddaf2c60
[8f750916]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8f7509161b267921fa4f4703c57280e6f1ede86f
[8f8a3b5e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8f8a3b5e67339e63a8ba270e7cc065f2573640d6
[8f8e8b7a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8f8e8b7a856f3b3fd3a529d85830032e73510a88
[8fbb9f8e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8fbb9f8e4a058e79ebd9ea45752c62133c14cac8
[8fc41818]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8fc4181814ff995835076b5ad2dbb77492c52e6a
[8fd66f41]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8fd66f41704885a990497436995a1568481ce1ab
[8fe8c8a2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/8fe8c8a21dfa4f82ea8b7f4a9c683d82831324b7
[903ee8aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/903ee8aab4ae04523b8869d06cae604f9e9c2025
[90556122]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9055612289c8499748001d18c2a232cbf23fe30f
[908b2f59]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/908b2f594fdbc1aa51313bba5f26db74ee332a4a
[9128a71a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9128a71a8131362709c35d506cf413db5b0bda00
[916259ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/916259ba70c903d2b2d85b4bd3eddffa98cec370
[916d7845]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/916d784599e0dc660d176d2730ac5bb6b9522271
[917c29d4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/917c29d4e1642e9b1835c945f57988013cf355a3
[919cc4fc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/919cc4fce18f1920f6cee21f3e487c58c7272dc9
[91a42744]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/91a42744e013bce22e05a01d6574022ad4e9e819
[91c3cfb2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/91c3cfb23253f1745969924aaed183e8f108412c
[91d33d02]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/91d33d02013968cc879e6c5d09e3d6e8eb9ee1e6
[91df1d01]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/91df1d0162814f42958283517137d62f1c0afed2
[91edf978]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/91edf9788b8f4d86a2813f27b64a50d894a1d5c3
[9204a453]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9204a453e1dd592745f9838aeedf904c64a647c4
[92097a56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/92097a567c96b6564ef111a0ba1413cd5fcf6241
[921db423]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/921db423048193078aa19f381c5d7dcbb7362299
[927d57b4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/927d57b41a466970c02854941291d9b1b9d61a4f
[92af83ad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/92af83ad3fcd2d31422abc7dd4d26549bed76ef0
[92b97540]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/92b97540ecc5b69f957552118a47498022d5a9c1
[93129e27]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/93129e27339a653d4cbb635de37a0bea31a16e99
[9325865b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9325865be02b8698001d116e548cd0a90a612ae0
[932986c4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/932986c4c3e7e81c5745d67e1ef8f6cf7602f53a
[93857c74]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/93857c74abdbcdf26e32f0ba9dcf80cbadb879fd
[93a58cc0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/93a58cc0254e8f4965fe7e3d5cd702489a237ee0
[93db291b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/93db291b59076eb10b1568e16d3a3ec95646c797
[942ab54b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/942ab54bb0f96c4fae444bfb088071126df3d36f
[943619be]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/943619bec76c9f49eac11ca7e94543bba2b8d8d7
[943ad625]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/943ad6258c6d01c3df3f97e35b7d0a2aa4f00136
[947407b6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/947407b687d1f46d0dd17beff046aba805fc2b65
[94d6862e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/94d6862e0d558e69f0e5b07db5a63ad7700d515b
[9503d81f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9503d81f3f0bcfc5785f06de98fb1fa034b9471c
[9522093d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9522093d0c183c35dec4c457214a219da905baa6
[95385332]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/95385332a3d41ff88c6753f71bc6102b10c22893
[95498c64]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/95498c647f7b6fc132bd76f1fab08ab253d9f570
[9556ac7b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9556ac7b8754dca989673053d05b840002a5e18f
[95aa7aa6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/95aa7aa6f14ee060466a511732627905d1ffe208
[95fe0fbd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/95fe0fbd776e7774db266e1db006c3a9e2082e98
[9614463b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9614463bc58917401c8254325ab6045fb876db13
[962de8e3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/962de8e3f4b5b65adace59a424b52adca40e5224
[962e7228]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/962e7228d0ca9a51756e6b0191b3ed34ce547ef2
[9648beeb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9648beeb87de47a6d9fddaa990e7dcd7cf904deb
[966c3366]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/966c3366ada13f8266e1d19b5761e05b1ef5fbbb
[96781a0a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/96781a0a28dbd4860c0256761b5cfb835e8acb80
[967869ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/967869ba8fd914c70f4c3dd5e0ea41b848b9e958
[96e36de0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/96e36de0add3c486c84c075828b58fb6d1540fb5
[97116211]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/971162112875af34f584612eed2e29ac2051dab9
[97329587]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/97329587f89d83d128325c9a06139c27f2faaee1
[9733f4b3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9733f4b365c9bbb883739d6be0a0d7ed9ff4bda6
[97b69b23]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/97b69b23cba21ec59e6a30a5e1fc1d6a642fccda
[97bb2f00]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/97bb2f009f6ea244d62a217436572de564d949f6
[97cf45e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/97cf45e94786b87b5a2d3fb2ecf2e696aeb4d1d9
[9806797c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9806797cb79515d4a4b4e8fa07adf76be4e865e0
[9848eab1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9848eab1f80eb435f38deb6598f47fafddf2733e
[988e563d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/988e563d3b909fc776e4c881699ead56f48478a3
[989ecdd9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/989ecdd98ce86d9e4156dbd693c067f9a185a8ba
[98b32b5d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/98b32b5df0d4eb65fce57c4fb29a8fc89b5bfc7c
[98b366b5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/98b366b59df6465fc17d2ca8cdcc6f70621957cb
[98c4b362]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/98c4b362743dbf5b5ef95234caa389e74dcac1ac
[98c7bbd9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/98c7bbd9f414375a9939619ff52a90b5f2318e8f
[98c95a0a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/98c95a0a22f0042b5776468f284bb27ec04ebfe4
[98ebe687]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/98ebe687ce608c985a5bce2d3e9410fa234a931a
[9948f0a9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9948f0a9035b0883644f0a37d63d16a77158be5b
[9957cc56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9957cc56452652f87ac037175d3b16f273a735ea
[995bfb07]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/995bfb074c2d01a91f940c30fb761b7833b50c22
[997a365d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/997a365d6a6c72f8a3e847f1c253b1f236f05a5f
[9982f699]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9982f6995a66ea5580cc6155b2106006ee7f087f
[99b8c469]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/99b8c469d683365998e278c50e7a4a400cfc61c6
[9a238499]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9a2384992a0bdb78769a7aeba8393ab4767713d1
[9a37f841]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9a37f841ad435b4c36bf8b4fe93da7645fe61865
[9a7502e5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9a7502e591ae25524605d24c48942586c912dd83
[9a80ee59]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9a80ee596cb756533680ee2b51b2ecf23e378e93
[9aa48cb0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9aa48cb0ffc3e59a6e3470a4350621ef9bbaeea7
[9ad1616b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9ad1616b91ffe3bfba3ab097dc04df90ba05845d
[9b08a080]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9b08a0808cb4a2d4aa502e42fd3c275e1c1789db
[9b299c66]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9b299c669c1e048ed507ce8ac888dbac26be0e91
[9b30dd2b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9b30dd2b58a46ecaa9fbf08e9681dd709a29faf4
[9b54b319]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9b54b31956550d7cd42a4affa3a36ee353dfe92b
[9b627bf2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9b627bf2e9887135425d1ca1f3eda293c7e3d496
[9b64afba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9b64afba56e426b5e0718fb67cf2544186fcb298
[9b64e57f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9b64e57f58e37419775501eae5fa3a6db5dfe367
[9ba41e35]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9ba41e3564b3058b238f0a05787373f788583b6e
[9bbc405b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9bbc405b0531c0d4a8c1b3e999f73222bd377fd2
[9c23d8a4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9c23d8a4459462a3cc91092a4205131f6e5db560
[9c4e8e9b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9c4e8e9b5006801ea8310baad780daffe6a7e0a9
[9c5fa576]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9c5fa576899d1529b06acf89221d44d262092d04
[9cb73644]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9cb73644833358b41040759c022095b99664480a
[9cdb6784]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9cdb6784706bd2cf70888eae6760b5af9e8d7c64
[9ceb5b48]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9ceb5b48698e16b62a380d2c1f577f54156c4ac2
[9d245cad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9d245cad65e5099c2ba4f43d5f7cc0520ae39a4d
[9d3ff3ec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9d3ff3ec738efcfd8240f6611953212c3aeb4a9b
[9d5f0ba2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9d5f0ba29c87a3ef0ad970da0a75781309e4c9af
[9d60fcc7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9d60fcc7852c23c2d6a13d80190d4316e04cd4f9
[9d677c03]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9d677c03df9d8cf66d83f91a2fdf60cc377d695f
[9d9179cf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9d9179cf63c4167ac46b5c398b2c6b718ea9a022
[9dbbb8aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9dbbb8aac0c838616765ad3c20d6d347b87f5d57
[9ddaaeed]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9ddaaeedecdd175672c38ba3d39c7521f08acc68
[9df010c8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9df010c8f067433abea5da1468748291c40dbe15
[9e27d9f0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9e27d9f0c87f2cf5e68a925b768ba08cf5ea792a
[9e2c2533]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9e2c2533a3409676e470d716a76fe18189ded5ae
[9e7fdf3a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9e7fdf3a23cc711670ab95bf7f6a69e74d4617c8
[9ee5e796]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9ee5e79687bba2e992ea01d990b86556ac8b7a85
[9ef5f4bb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9ef5f4bb0040b0e80281b6f1900f504903bfe886
[9f0de805]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9f0de8058a884ad23f809709a4a918d6116bf705
[9f194c5e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9f194c5e8cad20633c54a3293a7ddedebf1be9fa
[9f355b27]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9f355b275a716b124867b9f4f9c0a50b03fe38cd
[9f5391ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9f5391ba1ff4b7c8aa43d6ab3da57ee7693e0b9d
[9f942b0e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9f942b0e0b6a488078c8f0521f8dcc24d1fef31d
[9f9d7c3d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9f9d7c3d7f03372f266cd6e76b1737a7d1ad7c2c
[9fbc6318]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/9fbc6318f6931ba60d43843b387c4bf049d4742e
[a0115e09]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a0115e09860181710e074a262220245935b499e1
[a01e57d2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a01e57d237f55bba7e9541559c7bc0b6286cf8c0
[a0690573]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a06905735804f5b63c6963fc10b2eb512b32488a
[a0697763]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a0697763bf80d26b453027f27bbc22baec502052
[a08cc1de]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a08cc1dee13e4e69cecb5d8c70646290b48025b1
[a09e9f66]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a09e9f660ce2de3327a34879a5e184b3ef91a79e
[a0a2af25]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a0a2af2511cc9245c1fab83a39d4cabeb721ca5f
[a0edb157]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a0edb157905810d46d3418098b829744b3444d0f
[a0fb0c93]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a0fb0c9368fd53ec31722473c6384c98041dddf2
[a1378662]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a13786623e5b9117418dc6ff86c1f0519e9074f0
[a14ac6a2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a14ac6a2bad2a8a9fe161e24f782b42c6a6f8aff
[a178f0c9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a178f0c9f944f395a5b9c58b0131e97eaa43ac50
[a1a29564]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a1a29564a3f7fb1182e9da2badd25d1cd4836776
[a1d2eed8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a1d2eed8d05bfefcbe92b80f7048688f768aca81
[a1e1a680]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a1e1a680278843d4f871f5556bee679282a8d268
[a1f18eab]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a1f18eab8b9ccea63d5be200604d792108d9ff25
[a1f34aba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a1f34aba0d244e43b58a24fedbd055dd0e4f5a15
[a223eeb4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a223eeb41ce8408366fd8944cb70559d3fb50202
[a2569929]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a25699295eed0a20eeb3571e0c401d4c901928eb
[a2ebe40b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a2ebe40b2cf4a864a83a80ac302df1715e25d173
[a3141c2f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a3141c2f786d75527b2dc3060046f6cf1aa020ad
[a34a071d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a34a071d8b85d17359ca13a1349698ac10348ca3
[a3f0232c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a3f0232c09c5dfcfa4d0be6b337163f2652602c8
[a468ffad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a468ffadc41916adb608b633acf0dd8f45d255a9
[a469917c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a469917ce5b5abefd5a88283bd8079bd61c0b981
[a4eabe11]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a4eabe11f15c788abeabbad8d11a447a99d3414c
[a4fd91f4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a4fd91f4b1340a754754b8bec841eb60102988bf
[a503554d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a503554d9c0bbae7751b1e448156a7dc43f32def
[a5856518]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a58565180e1e81d8afbda97eb7e5f39bca4bbbdc
[a58b1a9d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a58b1a9d2c7d342d6f8c2f4202f1e471d4e070b8
[a58f2409]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a58f240943286c4ec7904408be3cf5a7820e9b18
[a5ad717d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a5ad717dc7d37586785b7375068defe352927e24
[a5d7efc9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a5d7efc914932bcdc41329e29b1e1325f3b61c70
[a6099bc3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a6099bc36884788f51bc5d9af5ad15451b1f2186
[a64be8e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a64be8e97cddfe1836aa5e1df20ff22836bc791f
[a66e2a95]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a66e2a95339c690069efcec431b54e9ff904bfc0
[a71c5e81]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a71c5e81761deb547c315296004167e13f82fe9b
[a746b065]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a746b065aa719a05f477224eba7fe551e62ebddc
[a7b7012a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a7b7012a3819216afff6d81eb40fca8ad514e331
[a7ba9e3c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a7ba9e3c12f84ac1054ae4bf5e76de93768e4f62
[a7cfd10e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a7cfd10ee270b2ed0c25a952c83b8ffe7235ea02
[a7d3a394]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a7d3a394bfd68656fccfe7c2794294361d8836fa
[a7e93c60]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a7e93c6068199f6b826e7aa1d21e2397d4c8e390
[a7fe1de4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a7fe1de4cbadd372e65987e433d74305e64b3cef
[a80facf0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a80facf0bb435346b0c8c3a05d22b8e428ba680f
[a82e25b5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a82e25b56c80e37c5ea6450c4a27a9ff1feb021b
[a86b5708]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a86b5708a0e1342b58a21f3786833872c6f506be
[a88c6625]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a88c662583ec0222b1842048b0d2f021582ebb6f
[a8a5f364]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a8a5f364fc531b08d8f5fb6245d64b0c70ab95ba
[a8a89bd2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a8a89bd22db468b3d845d113eb1a2a843f7dbc67
[a8adb983]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a8adb9833c850b1729b7ccef2412dc0c8798f7c7
[a8d5902d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a8d5902d3973b1c8fcbfa0925cb6870a28ba7db5
[a951fb3d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a951fb3d07656c78bb70e1fdd01a2bb59570091b
[a961e626]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a961e626b91d8cddebf422edf556f204ee75226e
[a967d783]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a967d783ec0b69a049019a4c8251f37e89b55a05
[a9902ebf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a9902ebf6d78778516d39b7d4150494616ae1bb3
[a9955bc1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a9955bc1406f4830569cb1aa2d3655d6c0683802
[a9b47992]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a9b47992efc8845bad937724c9ec38adc76a83fc
[a9d0009a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a9d0009aaa38b4aacece57ccbb5859a7241659fa
[a9de9e04]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a9de9e0423023b697383f30b7c856fc52d3d3719
[a9df5a66]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/a9df5a6696e43b7af64abb2a68b5bc61451b2a7d
[aa0990ac]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aa0990ace4c4edf5f97252ce0a779ca0052b6e1c
[aa1ece79]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aa1ece798c021b0cd444779c22c62937761d1ea7
[aa215046]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aa215046228b5635da6c916d4b266990c7e0cc81
[aa3992cc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aa3992cc919c644dc7fe3bc41abc2dd970fe3d2e
[aa4cf9d3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aa4cf9d3fbc0b046ecfa814b298a27769783d209
[aa5bdbbc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aa5bdbbcdbd2d36b08f11c0a252603526b7adce8
[aaaf78e1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aaaf78e17cfeeea087fe9562fc65907b3847bc9e
[aab71b50]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aab71b50e4464cae19f1add8b28613260345d9db
[aaf976b8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aaf976b84513bcdb2395fab5349fc035e4601068
[ab11dae7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ab11dae73a6e56dd68815c076c1e86cbbf6ac058
[ab579207]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ab579207ea14141d3d4327f39b5fd23830a89f3a
[ab724b88]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ab724b8829884fc52b26c611c4370a8c646b8d9d
[abb0706e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/abb0706e5feb2529fb78c33548bd6e0fbb75b039
[abdd5bac]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/abdd5bace78c295e5ac8c5a15fabd39b452b826d
[ac0ce998]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ac0ce9983b621fbd35ee5c7848c8be7f79690af7
[ac1136b1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ac1136b107ad83d419777ce496e25bc215e3c394
[ac39b5e9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ac39b5e91fd960013187a4242ee73db421079234
[ac509a2c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ac509a2c0f12541ac4db4107a423ada59732c4dd
[ac5749d3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ac5749d32d335f800fc8f3636cfecd321ebefb42
[ac8d29bb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ac8d29bb53de9b0bc06572f85073a1ac06f54087
[acc34edc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/acc34edc4502c691381df03a3bf9c2aebde1a038
[acc9b5b8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/acc9b5b8722b130d1551ea716628f096e7805c9b
[ad557228]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ad5572283ee4ee8263e337284a82db986f13742a
[ad96b358]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ad96b358a134c80fbcd7a27a5db53ce360f5d666
[ad985fad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ad985fad7258fad128bf761d4d2d4e99a2d51f28
[add02632]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/add02632b87052635b053ac3c8cdcedc0b07b50d
[addbc642]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/addbc642be40f93ba3df1588dcb165cbc9b4f0d1
[adf40070]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/adf400700122f4eb23fd63971b3f048e014d1781
[adf89152]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/adf89152a97120f0ee5698eeb88b98bc94774df8
[ae27b1f9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ae27b1f94f2af2b563210820b039e95cd5f637db
[ae5bbaec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ae5bbaecb182211a8a96a5dd64c53fca7316c568
[ae5edd9d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ae5edd9dce82d7d1ed3edc23ba33d58dadfc3562
[ae78d857]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ae78d8576c8839cb1b52bcc8c70cd8c8ffa3d59d
[ae811c9e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ae811c9e34b71f42f78bd680b3d070afed82ad90
[aeb3e899]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aeb3e899f520eb8e9f567aff8663e10cf8570929
[aec8cebc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aec8cebca812844afec8050e30d93fb8fa3bb203
[aee27e45]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aee27e45bc52c5a6839a66266d03a304d2608351
[aeed87f6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/aeed87f64b21581afb83924035cd22fd98d3dddc
[af468519]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/af46851919ced5582dd8d6c5b236edd3ac078061
[af549c65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/af549c6535d7810de80ee3fb7a0d21ff2f10eed7
[af851e2d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/af851e2dfe1dc6a42824df5fe7bf5083a01eefd0
[af93d662]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/af93d662852bbed6a3c13ca4f54ae4a63af56c20
[afad2413]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/afad2413faca1d37a8dca7365e3024757acdda7a
[afc0dab5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/afc0dab53064bef4aec0f5181e25b8f96e0169f4
[afe78eee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/afe78eee8a80dc56a732b80bd56ed9bb0790c32b
[b03da488]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b03da48883f07bd1e089f080dc4bc6fa9cfc8578
[b088be72]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b088be725c367aabae07d4b60553693a5c2ddd80
[b09eac86]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b09eac867af0214f682572776a365ea532766b38
[b0ade003]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b0ade003bea1149486c0a41240c6e66283a36e0d
[b0d5d189]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b0d5d189d50635166a9e152948de5be8fd1371e3
[b1926711]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b19267115599107ed9abc5264de15a9bc4d30a99
[b199791b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b199791bdf4a131fc6873e84400fd17fdf802c68
[b1acadcc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b1acadcc75c1b160281308cbf061e23abbf6a5b0
[b1bffe27]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b1bffe2731a225adb8b166c8182345c1d9f1d909
[b1d4fee0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b1d4fee0ff0107e6fcc7f5f43ac42827df536e6d
[b1dc6f92]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b1dc6f927b19e6f1d722454b6792d467834096df
[b227f531]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b227f531a6f348cdd9b3fa5fe010adf979dd8e98
[b2530a58]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b2530a582f9edcab94d80f9e53142ee801c8335f
[b25ec234]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b25ec234b015a43d333f082cbe4f0cee31ff6656
[b269c9d7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b269c9d75da9c2c86cc2cd22b94950a1b876f604
[b2a0ab53]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b2a0ab53877db5bf91b216baf3ba5e08853da559
[b2d4ff0f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b2d4ff0fdb786e35f90c5152ee2342bbe96750a4
[b2d6de50]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b2d6de5072f1506077fa649b15912b7cb3064211
[b2e639c0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b2e639c03eaaac4bb211d274855e4f7a053c9fe8
[b3320944]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b332094475f0e620cba4152b6b7e06afd43449e5
[b339c881]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b339c881ae94eb1b14c02462042c4c8e8416e951
[b371ef11]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b371ef116f0ff7e4bf8f8d82d58e9a0daafb8427
[b3752d11]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b3752d11e5f827b380cf64510e6b27ea8e8fb1c9
[b3bfd4d9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b3bfd4d96c7bbdcbff881bc9720ad98ebcedf255
[b3c6f4d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b3c6f4d5db39652ee58e83fd9ef92e86eae1c8a2
[b3db4313]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b3db431311b0fffef2a318e5d795d49fadde85fa
[b42e0f54]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b42e0f54b9807062f50124b8b011f04473e98970
[b461b647]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b461b6472c5b1db19f550da6142e543d045b599e
[b4e9cf24]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b4e9cf241b87b9a0aecfea1d59a9e110bf7d7091
[b50ce993]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b50ce9932b2b8502a113b85714f6ac9564c2645d
[b5433fd4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b5433fd4236500ea97f5822f0135f671a48875a1
[b5505730]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b5505730100a9780877eb3e1cb4d280f02845863
[b55341ec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b55341ecd717344211bd79557f56f7fecaad2479
[b566804d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b566804d3a6f6bc0197dfd88f3f85ba2f166f299
[b57cfb21]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b57cfb212da3ec2fd300c95258e7124399b7b8a7
[b5d4d917]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b5d4d91779599bae9fc15d78c5e3db3f4a43f18b
[b5dd71e8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b5dd71e8e19bc0094fa253ef1345a12996ced46d
[b63af4ed]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b63af4ed1cef420aa8482a2d7f259d138173ae20
[b63e8480]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b63e84804022258362d0238f7a5a7b199da524f8
[b6aa5625]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b6aa56256967d7e434f21b7f454bc62a9d3c8269
[b6d359fe]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b6d359fe3efb94ba8f85c7eaa1788665c392021d
[b6ebdcad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b6ebdcadf5c19370c6fab18a3c39e4c3322776ef
[b6f5e925]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b6f5e925222c7701844184bfa46144af92de3eef
[b7078ffb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b7078ffbc6e372a59824be931d15d810a9bb32c9
[b715be55]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b715be5564b434b10e566aba23a1737860ccc37f
[b723fed8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b723fed816b98dc1bfa9484909c53a8078a1335d
[b76c05d3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b76c05d3bd5dd062408b55a1b4202c4ef5aeda90
[b76c930d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b76c930d03db05a3b00ed7616e8d40748a6ed5be
[b7898f97]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b7898f975a3d92f4067a6743e36f6afb363c715d
[b7a25d09]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b7a25d0905f7aa8426eb97ada89a516620d81e77
[b7d11d37]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b7d11d371b951f3cb4de8f4fa7e01982c433dbbe
[b875e10f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b875e10f8792292446465e8855b7dbb7048d4c4a
[b8989f3f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b8989f3f0e848138b6de90b81b2c774e775a015d
[b8a8c34b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b8a8c34b650c47b815fc307346aecc69f35d192a
[b8a8ea75]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b8a8ea751cd1db7cf2e973872f6a3f2d483c800e
[b8b69662]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b8b6966253c99ae097d6d251d7b0f75c3aa5059e
[b8deef34]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b8deef3439f8e8b9a949a0a1cfa16d2c027c391f
[b94f74dc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b94f74dca23b0cffaa08eb29bd094b86b5466027
[b95aded3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b95aded39ab9729082b1b419a6a8e0d63ad64f7a
[b989a7e7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b989a7e7631d71668c68be1868eb42fed384b078
[b9c196f8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b9c196f815eece26c0b5906c262bb12b2b897453
[b9d4c2c2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/b9d4c2c24c13c8f629c7ca6cab36941a1dc7a4b5
[ba2d4f40]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ba2d4f403dfd490340fdce7e1811096451b290ce
[ba3402f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ba3402f142e852468c384d780ec329d75c3bb980
[ba4c049c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ba4c049cb1194066d59db83083249614fa71be98
[ba75da98]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ba75da987cde9b47cd1f3f298f0a350873898125
[ba7e2b5c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ba7e2b5cce99b2b7eea89022eb5915da6cffd259
[ba98940f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ba98940fca2449c9bced6ade1a5ee4342f1ccd00
[bab5ee53]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bab5ee53d297fd4d3cb21ce411cef4c01748d082
[baff6da9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/baff6da9a9bc313db65a613d5edae82d67aee4c2
[bb02aeec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bb02aeec223d51920edf9edecb5ec4e9b087efd9
[bb32b517]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bb32b51717dcd9a812a7e2e1cf77dad46294577b
[bb3ee6f9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bb3ee6f9e9d7821aa04206d2ea3f0cda0394db99
[bb8a1d7d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bb8a1d7dc034737da79753da8e837494f245e046
[bbb1418a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bbb1418a64882b32d9c0020f666ffa9764d86b68
[bbb85cf8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bbb85cf82738a77b054c5b063d3f7f5a8c6e6488
[bbdb2cb6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bbdb2cb61fabac44421596f4e3c64e725532e5c7
[bbe3b006]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bbe3b00626693af8310454616c08b8358fedb042
[bbfa880a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bbfa880ac2f188abb21da4e77253a65be8fb3f07
[bbfb384c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bbfb384c8d89cca44526a899966bf65bc0e04cf7
[bbfe27bc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bbfe27bc21cf0cf38267db1308ff5703aef67e9e
[bc386776]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bc386776038546911ac1e8672eb843bd1ee2db45
[bc411286]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bc4112866bb713538fc48c209408313c634306b2
[bc58af34]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bc58af3466a3513570103d8b5ac4bbb53266f561
[bc5ea85c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bc5ea85c879fad5c7c4ae88cdc9de71290455d6a
[bd069b03]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bd069b037bc90da7f33fc8b2e058a84cca2fba14
[bd1ec3ec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bd1ec3ec7e0a07373794e9197923b0052fb4ccaa
[bd29d11b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bd29d11b49c2f0c8c2640200231df82eba1ed4ab
[bd44896a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bd44896a30627bafefa64c1cbc78229113130b9d
[bd49cdc8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bd49cdc8220e8adcfea71f04c6ebcfb51946336b
[bd49e68c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bd49e68ca2edebbd9517a62793a225341c18cb59
[bd9b494e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bd9b494e083a2861f8c991cfe75f80f61d72ddef
[bdafe96f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bdafe96f41ea33ec27a840dbda74ed909f6f7532
[bdb9fb26]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bdb9fb265be073d516309becf893082d49bc341a
[bdd4c333]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bdd4c3337e4726afdb620149614317c20aaa01e5
[bdf4b239]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bdf4b239c3cef91ad0e9074cc323a9f712b3c7ff
[be67657e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/be67657ecc4c9b0271cecd261ca02f80170cd333
[be714133]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/be7141330793f4b48865fbf98ca9eba5e65347d0
[be8f4865]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/be8f4865b83264d91f5d38e3ed64bbd84e8e91a1
[be9e0dfd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/be9e0dfd8e7db4f991980eb97a78709209ad7915
[bea60aae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bea60aae98c6f7b6ffbb23a30fc58d825397a3e0
[beac51ae]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/beac51ae52ae7c2427b649837c66dcc5ff5e2099
[bebf6e9d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bebf6e9d9c825935206b47a321ab0bb0fe01ff74
[bf556e61]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bf556e61252444a1bd308dfaed4e53db5534690a
[bf604a33]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bf604a33a27667d2ac28e6c358bee78ae5b8b9b2
[bf7596e3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bf7596e325ac8a7514ca0427695c79074951fb2d
[bfa153de]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bfa153de69ef340930107995f127149a57c7618a
[bfac49f8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/bfac49f810578b6afb3c84fb88656077d018f7c5
[c0c12172]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c0c12172201c65a25696a6381503856617fdb1cd
[c1262d43]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c1262d43fcdfdf9b2d3604786757bdf3a8ed77cf
[c133d6ce]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c133d6ce65c23b967a26bb0e693574d4749b3d7c
[c13d0820]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c13d0820b77a54b1e15a0f42ecea6d6b250a9fc2
[c219df0e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c219df0e3dbe7f58592859ac0385705a76d299c4
[c225eb65]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c225eb65b2330d6f61580c37504421144308febc
[c23f7d1f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c23f7d1f58233d4d5c45e4a8d703bb6bf7ecf4f9
[c240b82c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c240b82c292586648b2dbd345eba39d716ddd43f
[c246c067]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c246c067f84cdac8df843e945d088be04acde36a
[c2487ce4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c2487ce4e7be6767843fea108ad28a911f829083
[c24a5f09]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c24a5f09310d6eb2154f4ddd5acde0229663bf39
[c251114c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c251114cf81820fa74a7f9c0e611f5f7cc9f1e0f
[c27311e7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c27311e7e178615e9b4d4f33e531f8f605d20156
[c2a2fc73]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c2a2fc73c451a652c8bf917031567903ca8bf75a
[c2d7cc41]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c2d7cc41f10e4e2a0fea3c3da3a043e19bd60d91
[c2dae0c7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c2dae0c7b0623d5dfe4e7f6fd11a84eed41e76c4
[c3058eb9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c3058eb986f09aaff68a559023b8a1403d6faa09
[c332a733]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c332a73363492a1e1874e68fc0c12e3bfd2b96ae
[c348408e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c348408e8d88bfe1cdf4a69040bd63849478368b
[c35066cd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c35066cd2cc01344259f00559186fbd1a12db527
[c3a4d2a8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c3a4d2a86af1558b40677645ad08c531dba756be
[c3d0621b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c3d0621bef3a9d3ca2c3d9967860f839b4389fd6
[c4398e1e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c4398e1e5369180e41bab329153c0aed54d8498e
[c442d822]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c442d8225879a40798eca2032bda23e5a5a9587c
[c463f6d8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c463f6d8c25397c6a4fb66bf993b80b320cc7843
[c4648b6b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c4648b6b5bdb4a77dd3c4b0db5b24a090d82642f
[c4c7f301]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c4c7f3014b51280932244d5c132031f23642cf79
[c4f86ec5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c4f86ec51cd2e3b21260d7314398b34d0661fdd7
[c5a231bf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c5a231bf8673921301928656b36f601c5755ebf5
[c5db71aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c5db71aa4d80bd0c7713bd41a43bf637220c755e
[c618a199]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c618a199f59706ad2cfca64e2c37bbe4b615faf1
[c6221743]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c62217434c9ff7efb5ec9067464d5ba8b841ffb9
[c63ff966]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c63ff966f75849c14e76187f14288a1b63485490
[c650d61f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c650d61fd5cb968068b38701567952447f231ef3
[c6f18991]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c6f189916c9fc9cbc4f69ea7a42c110497e7e819
[c70cf1ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c70cf1ba47c9a09d7f355aed59707e7766b092ce
[c734d635]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c734d635ac3e72168e1f63e82767960b6a038eda
[c7ec4ebf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c7ec4ebfe41385a409265ef9dcb3ff4fa9222b03
[c8084707]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c808470750b9e7567d0ee341f6402eac42e60627
[c80a0449]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c80a0449b8729d3c64775e56de8fe27f21017c6f
[c8328f3a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c8328f3ab256bf76a92b205f8eeebc49447bd25e
[c83e433c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c83e433c50687c9611cb298e64823ba9a2dcec6a
[c87ceff1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c87ceff11912ec3788f390cf454b1a84db5fd8a3
[c8fc525d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c8fc525dff93e1b29c0df61bf6cc593376910043
[c95b376e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c95b376ec3eda181ee9125d159f2fb86d8261bb2
[c9852d2a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c9852d2a508c042d3da68c51bac16a7cf761cebb
[c98bf252]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c98bf2527c00302e0f8c7aabbfe6de8c44136322
[c9a5de8f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c9a5de8fb611785c5d8bfd6e6942f48006cf9814
[c9bc19ec]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c9bc19ecd6cad88742cfa3758e48fd606f489220
[c9dc70a5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c9dc70a51be61bc46b43082e7227f873cb77ac10
[c9df16b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c9df16b92863388e5971d5f9a43521668083e1bf
[c9e6afa5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/c9e6afa57ba1971637503bc34d9e9bbbe8de1071
[ca1c967a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ca1c967a1dd169b73f3002f120c40c7127060041
[ca375c23]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ca375c23942b7d01001524b58d63b60054dc7196
[ca77ad21]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ca77ad21a37731d480046325ab773679bd791e31
[ca786de1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ca786de151d854e28b9a80ea5c9e28e3b95c329b
[cad2b80e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cad2b80ea9e74f2eaf757f3b3432eed7f21738bf
[cae98770]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cae987706e31a6c223e5af997fee32b537714efd
[cb267457]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cb267457882d2111219bf68b90fa66ea0cf6fae5
[cb633edb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cb633edb305921d25af0003c76c4aa8cebf3f608
[cb698ec9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cb698ec9a74329fc7947db489769472f0cbaf4e8
[cb74da32]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cb74da327e27b73e9724d8a28aafc164e6c9e0df
[cb8ed82e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cb8ed82e9ee18dc1b0f3ffdd2c22d99402a8c870
[cb9a91ed]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cb9a91ed337581abe68ce57ecada73b5bbbe5b26
[cc1cb8aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cc1cb8aa305b3dc17f9df7c0ad8c898bc931b0c2
[cc3ee9db]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cc3ee9dba69eaf508d11b59684d79ca637b44191
[ccab6fc0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ccab6fc0bd2eecc64d2aac15084b85ce4eaebcdc
[ccc9e298]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ccc9e298bb0ca751f50adb0f43ffaf634e7220f8
[ccd4f0c9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ccd4f0c94c53343c239d61e5cee680e7df8d312b
[cd52d9d6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cd52d9d639e7f03836a5a36b197a04806c9a4ca6
[cd58d05e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cd58d05eb045790547aae848ac982f8b48004571
[cd6584d7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cd6584d727529af79bf61e7d8ef13876f8cf9b04
[cd8a8c83]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cd8a8c833bc700576e4021fe1baca6fcf14db092
[cda6e991]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cda6e991ea22413221c90eacd9b5a16c875ef316
[cdaad462]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cdaad462bfea78e0e079853e198a32ec89a5d7bc
[cdd64dfe]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cdd64dfe9773aa85ccdcf1099290b273519169d6
[cdf70636]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cdf70636d3d2e5e622d5d83e45b8fe17f889c2d2
[cdfa031f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cdfa031fcebabd78dd228c1098601fc6a2c965c6
[cdfbc9fe]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cdfbc9fea8d30306aaeb6deb1a50e5394c5500c5
[ce8518ea]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ce8518ead73f36025e708c567cfaa1d9d74d5f2c
[cea1c6f8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cea1c6f892bc15f7aaa0f6b3e4cdeff074f7d5b5
[cebf2818]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cebf2818fe60d8509af94fe623bf0d1f7ff44b17
[ced4667f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ced4667fd5f16682a46e70d435a9a473885c70b6
[cef0ea0b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cef0ea0b200a2d24a263956668ab2316bab50428
[cef66e50]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cef66e50bd7b149177a635d0f2bb17e1b77799ec
[cf2e8ec8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cf2e8ec8a2283ebae8a6ba695771d1a3bd44b5f3
[cf3b3e66]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cf3b3e66a5238da65f0227349ba3c31f142fb61a
[cf9153bc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cf9153bc3cec7f038ae47397c9d0a9942d5f364e
[cf91c1d2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cf91c1d2808a7658da8eb6263c3aca0ff3e5fb04
[cff93366]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/cff93366ae59c85d01f5d818ea2e8c8c73cedb87
[d00d725d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d00d725db9c6cf78c097e2853c4195682a9d8f64
[d03737cd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d03737cda4c53aba353a32f33fd32f7fa74738ad
[d03812d6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d03812d655788b74b1d77fe74c5dae147c561548
[d063dd17]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d063dd174ae14ec1e30842374ac7c41bdddc82a0
[d0736743]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d0736743fca8af8a4dde7d8317b72de269f5655b
[d091a792]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d091a792aa369ea4bff566bd321a4a9c9cbb589c
[d0959042]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d0959042e33401ee44d45ebd17ad51fe981ae53e
[d0f4166b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d0f4166b6610b624b6bb2d28a7acba407aea7ca5
[d12f1c15]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d12f1c150b772f4e74b5de76c6c65fe56e094257
[d13dffa8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d13dffa8e80c9f30c04ce18143e669a04e817a45
[d159553e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d159553e8eaf2166c6d3b6187c007ad3dfc21400
[d19d64c8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d19d64c8a642940cb19cbc34d402b4f3f4add518
[d1b7a07c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d1b7a07c6a62e96088ce288e8f38a5361ff23024
[d1c548dd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d1c548dd264b06af1fa49745a3d702c777c7487b
[d1dfe28a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d1dfe28abda8f6f8b46a1aebaee5750521fb5854
[d1f63b14]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d1f63b14fb9481b901a4e088d9ca504f97390297
[d205c419]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d205c419687d0908828ff4f06f4e56351a7ea2f4
[d2158ee2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d2158ee2b1b23a68b3c4dd764863acadec08d6bb
[d23220a2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d23220a23c9bd0a3eb7ba6a10bab7322b6523fb9
[d288501a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d288501af5f4350c6cc9ffc52fecada302e64adb
[d28b437f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d28b437f48d57d41bf739aee84ce00276d262bba
[d2973817]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d297381745b679a8a4c564e0f38f55e117e0e878
[d2a6d981]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d2a6d9819593dcc23eb49350908037ffb2be430c
[d31c1456]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d31c14564c2bc27ed4e7790a54b16d09a01c3be9
[d3313e4c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d3313e4cbdd983a9d44089338e4bab133e7fc6f2
[d340bd29]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d340bd2985295b3ccf4559c4ab1ac3588501ca4b
[d36e9a2b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d36e9a2b1fd169e61a378f4be4638c95736dd284
[d395d94c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d395d94cddeea82f7117682882407feb35258fad
[d3e11249]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d3e11249b358df9fe1d00f6d29b0f62402b58ca9
[d3f5d8a4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d3f5d8a4cd60ec6007977e7ebe4558c4a14789cd
[d3f723ed]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d3f723ed85b0c433c1c6c0a424ccf33ccb11a17d
[d4060a99]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d4060a99ee69974ab4fc34ebc41522fd0a89ad4d
[d48099c0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d48099c07d95b49914e4e4271679b4846dd6b608
[d4ca06c1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d4ca06c13588f5b4d562f1c180bd7734be3437e4
[d4de1431]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d4de143138de5fae7bd646802dc115c24afe3dd6
[d4ed03e0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d4ed03e046d292888e555de3b6955b396ef7fad0
[d4eee96c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d4eee96cc26b4191d2bd01a29f40af1f44b1fd61
[d4f5e0af]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d4f5e0af96e5ce9d83c12e46a345dc5525d27a95
[d519d782]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d519d782583b1fbaa71495f890806f960ad90741
[d52775ad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d52775ad0e75bcb510d0277cc4e497f825c043ba
[d56af20c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d56af20cd7a7b740c1e44472c976d74dadae803c
[d5a855e2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d5a855e2cb54bd176da6113776800143f13e6aec
[d5af77de]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d5af77deed057d599fd1c4b5c1f6222a7edba4c3
[d5b91372]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d5b9137254262069587a8b6db9d0e780c1a47869
[d6223438]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d622343816e40928304231021a1f3ee8ebd3c9ad
[d62350d0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d62350d064237141145b5b4f5687f19f2f855f6a
[d6bb2d1d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d6bb2d1da026c16c4a301fa675653d8a0688a679
[d6caa25b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d6caa25bdea45f7a70323efa4bb7f93624644024
[d70104fb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d70104fb19ee3e133188a14d49f2c57ab0a55e06
[d7183047]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d71830478ed05facd4ab3b3d2c51791dd1a1caf7
[d75fdfc0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d75fdfc0fb7b34f4e6b5ac2cfbcbfca7df0ccf59
[d7d264ba]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d7d264ba4c9eb13170cfbcf5297ade21df622d48
[d8283448]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d8283448b23a06d69b846b4c0619ed4423ea8922
[d882f968]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d882f968ae9011b112cb8f195171e5357747a6af
[d8af716b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d8af716b6de4335d7a5016274436d031d35cb78d
[d8d56414]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d8d56414c28f5ca7ba2db10420c1805270d80d7b
[d8faf4fd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d8faf4fd010e303dad42c8a0a51520c03fd197b8
[d92039a3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d92039a375656b776c436edd84e25ac0da054130
[d9788a97]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d9788a97a708b29bb51bed29a6fc5e78af7ab5e2
[d97f0ab7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d97f0ab7ba5ef0cfd4a7ea0ed9cb21f3770fc5da
[d98ed95e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d98ed95e2ef97ffc4be4ec2dba5d0f468b1d34c1
[d9c0b1bd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d9c0b1bd4b737fae136a357190b1f6dee22e0d7a
[d9c3bf3b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d9c3bf3b11b81434c738b00a40d13fd53a51f250
[d9cf2120]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d9cf2120ca3b38d63a24579d6bb53951caab66b7
[d9f70ce8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/d9f70ce89f21bc8e48184856257fddac0a0372e1
[da32db19]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/da32db197b4274f846783700f663b9617399c5c6
[da392e6d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/da392e6d23a7ab2490d26b94b1ad8c213c0db4bb
[da6f91fa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/da6f91faf961dff4f1adbf528cd4025d98cd3624
[da980d8b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/da980d8bdcb4ac506db0862b11987de8eb859179
[daded23c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/daded23ce694301aadd19c09a07bb1d384668ce9
[db12695f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/db12695f17b45070635d7a149dfe8cab2a86cd41
[db27f5c2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/db27f5c20243b90b05da720934ba73a26591ae07
[db2e8f3c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/db2e8f3cf4db912d32e74fcbdf09094c8b2f5128
[db5b6a5f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/db5b6a5fbc301716f84682c4dae7e1691fcba413
[db649b16]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/db649b168aac146bea274fa1de64a23b0487f6d7
[dbb4e4f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dbb4e4f34c9452fc16096ebc86c30b30c7124985
[dc170648]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dc170648f88426308ea6e6622215b7c3618c3a32
[dc30ca63]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dc30ca638c88714942f282de4cd464336e41f8de
[dc35dfd3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dc35dfd3dd707b3e613c67acf2e67034e82d2944
[dc7b7c27]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dc7b7c27f7d239fcf02d78981ea13a5563c88f88
[dc8f3631]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dc8f36311ac904984ce8f7c6b62e426c21b5c084
[dd224ee0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dd224ee0aa9e3636be97a697063ea7f4e9caa66b
[dd3637a7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dd3637a70efb9a639e0b4e7a3db9d6c868752bfd
[dd4f4232]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dd4f4232bc3d97f2f3c93885a94b61cfe32b066b
[dd53fdc4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dd53fdc4efd62e33dc15267f4ecd174cad2c8bb7
[dd63214e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dd63214e4877ab17811d0e1db6867cff6bb72e61
[dd8248c3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dd8248c388a6f8df54c12f5dd010de613a0e21ee
[ddc2a803]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ddc2a8036d9ea83b75bdc5cf506c365f5b09a3a7
[dde421c2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dde421c2914b3846ffedf49d33c957ba822f2c60
[de2fe8c1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/de2fe8c1974f714023b4c50b70ff5f0b863736db
[de94d527]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/de94d527d8d8b9e2da03f920c9bbe510bbb97a64
[df2bcdef]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/df2bcdef317ceb778093641fe97b8cf5664bf4bd
[df2d3132]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/df2d3132ef7d30100e8eef22c414ee32b5cb6ab1
[df5ff095]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/df5ff095da1f2f12fd3db9e356941672fd76c46e
[df628a72]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/df628a72730e677e16a3053988983f752d71940a
[df63898c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/df63898c454d766b16ef7841d44d7807ede60a3c
[dfab1e70]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dfab1e709a370d468ffb3540f3c6d3e280e97017
[dfd2898d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dfd2898d64411f280bbe7d04280a9c73d3a3b310
[dfd5f6b8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dfd5f6b83b1c667dcf166e37fb2a2eaac7d3c99b
[dfd60d4b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dfd60d4b29ce3ba0afe581c746d643cc5a6eccfa
[dff29276]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/dff2927698abcac250fd3f0df7910c02818f6776
[e01919ff]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e01919ff7764b78902e0d2082d18520a60284adb
[e023b80a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e023b80aa7fcaabec759c183589a761fd784db74
[e05286e8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e05286e804e152b630db915d5643723a12e91db2
[e083e73e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e083e73e8fbdeab7c9421e729521c08bd9c77fbc
[e0b96709]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e0b96709ff1239fd7ca03db85965b01ae5ffcacf
[e1216edf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e1216edfefaffb0cc86f3ba0caac3655d3a2bb0a
[e154f451]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e154f45199877992d520d137d6f671367e9ac878
[e189fd21]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e189fd21f8689048e404ddf19c279ad743203924
[e1bfd835]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e1bfd8355254ac10842569ed485f99c89570fa54
[e1dfb0a2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e1dfb0a281d3d922ada33f53013accb2c765bd9d
[e1e46068]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e1e4606847459e742f9c5e51a860b8903b2bc5ce
[e1fff225]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e1fff225b6c62f82421e0094c8dce82b8bb0013a
[e2019f37]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e2019f37dc953fce2a2e27196ea967fe32a81631
[e23c849b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e23c849b196c3d2ad9ebf28782bd3899ca105df7
[e2841e3b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e2841e3b6e488ae2db5230b8cc4411fe846b194f
[e2952681]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e2952681309b004990a52915c49def2287fa9f4d
[e2d36cee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e2d36cee0887613e90d00b91ba5926715f28cea9
[e30fd625]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e30fd625450cd6a6dd103487e881dec747daa8f2
[e3126e4d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e3126e4d9d531707b56b65d442ee3442a70fa229
[e317c87e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e317c87ece614341553f2d4b7926f1614a1a5b5a
[e346e184]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e346e184d9ab0af7969a796ef4c43814267aa7a3
[e35c450d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e35c450d3a62cdd7aab66079354a79eb8c4b832c
[e3755777]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e37557772d193a2e812598ed06ea0ab8656dd293
[e3b58f92]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e3b58f922ce8b312ee1fc9b04b39e5dcf75cf1c4
[e3c0e0a4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e3c0e0a430d6e27060b00db05c17f01d68361547
[e3ca0e22]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e3ca0e225065cf4fe610fd0f49748dc8cab48f71
[e42d021c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e42d021c20b90e50c464541fb3d358ac24ce3b3a
[e43b6879]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e43b6879d87bcb4e3c9227694b9f06a4aea264ba
[e43d48bf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e43d48bfd451fe4aac2f90b0e19a357bf5a1c1b9
[e456f50c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e456f50c20a6b4052b7e3b83d47461cafb36f8ee
[e4658658]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e465865895bb3a07c3a60e2205fda4e9c18f848b
[e46b1f94]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e46b1f943753dc0a5bf1b45b458f0fde643ebdf5
[e4b6a981]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e4b6a981d366fc6d97a561015f169592355f8ac8
[e4f8059e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e4f8059e97c3ba25401f8752607a66fea4dee10d
[e51c30f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e51c30f16a3fb478829bade3350a429d54ee3e94
[e52abe58]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e52abe58af70ebbe6dc2545e9408cb3a8ed4bfa9
[e5828040]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e582804025a40e0ce8bcfb5e5f949277b7965c36
[e594436e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e594436e697b05db2ce27396cc882f332cf65583
[e5c4c6ed]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e5c4c6ed1500f2e5636155f5935dda25e73ae5cb
[e5e7f45a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e5e7f45a1bc577211908f98bc9a9bbbf335cf332
[e6048fb3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e6048fb30287a2e965c23f7ffd9c9a42f2185040
[e64e0e47]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e64e0e478b6dd5695a7094ee4caf648c6dfc29dc
[e6931ed9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e6931ed967f7ea795ecdecfaeeead533642445f5
[e6d03e4d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e6d03e4d12efe115374a0279fd14fa2616275918
[e6d8b93d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e6d8b93d887ec8a2cdad57ef090999004f7964a0
[e6fdc373]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e6fdc373dfcb0e7662a92655703aca730f166172
[e7172e45]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e7172e4519383c352ed147aa42b3aeca646a690e
[e72a329e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e72a329eeae12eedfc591142dd7a09c5476f6699
[e736a714]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e736a714f4b2a84e4b5d578c8789049c1bbc4df6
[e73e2c18]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e73e2c1863ec4a908ac4707604562929771659e9
[e73ffe75]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e73ffe754d1185b3d8c2256115ac49bf763ef372
[e75a099b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e75a099b6ed08f31710bded4c3a840872b274304
[e75daad2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e75daad23339e77fe0f36b3ef666c68f9d28b60a
[e76f18b4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e76f18b4faebaeb2dd1210265dd06501fd98ced6
[e790e681]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e790e68174ba0c42c75d5cf1783aac6a101e9c11
[e7ce0712]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e7ce0712e2bbe3aafa8198f295479ee0252f4fb9
[e7ef323c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e7ef323cb0497793074caaefafb86ae9ad1f4e4c
[e804087b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e804087b18c63ca48db2f70dffc67415a57643f6
[e806aee6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e806aee60ff558dc2ef781f8d85e761b8a7cfd8e
[e807cd11]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e807cd119704cef760b5a5d83f9c3c4870e1903c
[e80df386]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e80df38691580c8377c5e3fd30a02617765ee69d
[e83fdb5d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e83fdb5dcaa382b60dec609c1c23f6881f12b268
[e8406c56]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8406c56c77869ad8e70e9e1e7e448a0f458a204
[e8887e16]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8887e161d70846f60ccd2cf8a38bd3bb5157dce
[e8890aaf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8890aafbd8e3d5c7327855166ff51380fd3e095
[e889d5e0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e889d5e052df5fe93ab309bd5542805d5d745919
[e89197d0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e89197d0506d205eb9c695ebbde0c0f4c67b964b
[e8ae1ae5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8ae1ae59efad8249b6c86a7e2dca15dd7c231a7
[e8b1a1db]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8b1a1db047b2c08f5e817725c0af06c8247db2f
[e8b786c5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8b786c529b59b1d1e04934fe964f0d0649a7bd0
[e8d041fd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8d041fdbad88476cc8ef927ea5b6d8f246c444f
[e8df9ba1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8df9ba1db6132bade7a282bd709c628d53d93e0
[e8e7c9ca]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8e7c9ca3934f030d2410e92880b836e2ec23868
[e8fec2c8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e8fec2c80ad78a784651f86e1584dca25fd881e7
[e916a2d1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e916a2d13572d3c45e357df8b7db1c5a63ac840e
[e921e671]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e921e6718b67f1d8294a0449c0689e7f8c6289b3
[e947a92f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e947a92f45ca10c55917a50b0aa8c78438152a04
[e94f9e81]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e94f9e81cc28f9d0195013665a976743781c3672
[e9b501fc]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e9b501fc77259d0c1c050bedc5a61c3516e4c307
[e9dda18e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e9dda18e3d6686be96e9889c3b174c50beb629bb
[e9ea121a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/e9ea121ac4a53e44e02f63f4f5ffee16c83dd72a
[ea56625a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ea56625a23b270cc5bab87ca6e22541e80b89b34
[ea7f856e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ea7f856e88f0e40914b513e96f8e726c00bb07e0
[eaf713ee]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eaf713eeab8a7f617c32e2d3885b03777b3969b1
[eb02c426]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eb02c42686c10a454c0f990d438ff0b750120511
[eb063b0a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eb063b0aabb16ffa178c982255cd81d060724089
[eb212f72]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eb212f7286812d79d4598757c41b7990af640de8
[eb5a1c33]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eb5a1c333d495d5ffdd95c390992de1e2a26e92d
[eb602d99]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eb602d99d39d85e38b4fdfc019e5ea1b01de6261
[ebf4fd3f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ebf4fd3f494ad12521f1f9ef1d4548282447e8d0
[ec82a500]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ec82a5009bcf7a16aaa694eb478216b9567c87c1
[ecbdca19]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ecbdca19f3817c77ba330e9e6413d3af94713422
[ecc279c9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ecc279c961f79f18173d8663c6850388a1cb37db
[ecd0e724]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ecd0e7247fde331cddf08e3a9f7fb10977774df2
[ed3d1feb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ed3d1feb788121161ba66f9c1826a67ded941337
[ed5c04ad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ed5c04ade1af13f2e22afc184336f9713f2b76e0
[edc67891]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/edc67891fe65bb52bfad8d2aa57642c069a0e0eb
[edfdee91]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/edfdee918cbd7e13aa31798a41714904e2c04202
[ee094b2d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee094b2d8d1beb4a1b43f272652536e40afe3b21
[ee2e1500]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee2e150060517bfd26c432843d24db921d15584d
[ee4515f1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee4515f1fd7e5161b5eab5bce0262971996f843f
[ee4c9d0c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee4c9d0c34ef366f008f83767e0b2b88a9e90a4d
[ee73c533]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee73c533910803c02f60e39b2095546f5eef317c
[ee8101d6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee8101d6063d8fba494c4e74558f0f7e0be76cda
[ee95b636]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee95b6364d51c7d8a6bd4259ceda8ec63d13f56b
[ee9d6c0f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ee9d6c0f865bd38f0e0feb0b92cba6252fa83cd7
[eeb20bb8]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eeb20bb8431bf75c9e2be3fbba8e64daafae3098
[eeb3632b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eeb3632bd465d4937204f1d4c3e5f72a953bcfa6
[eed4e97d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eed4e97d82b05b6c45a9d8362e1274de455621fd
[eeeeb164]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/eeeeb1649b4d5ffa799c195805e1c5941555aeca
[ef4e2665]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ef4e26650970e160394b3a6999316ff40b0e427f
[ef547255]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ef547255aec7a779e1b4d1f553e9e5adf7fd9156
[ef6c6d46]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ef6c6d46e3133c426ccfc9a36e6e0e370c532d1b
[ef6e975a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ef6e975a31ada2d4be4bbe7d6f4d1bbc62daeba6
[ef73bd5c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ef73bd5c114916a2f430dcd9c26eb49ec98f3fcc
[ef7d59a6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ef7d59a60d323655aa6b3616f0d10a78ab11b565
[ef80a0d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ef80a0d5d844ce2b8ec80391305f0b71fc18b518
[efb694df]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/efb694dfe1f34fee33210b9b5e3a749cb9468be4
[effffe87]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/effffe87f8390d5894ab8dcf1806b2dd5b54e493
[f00cf12c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f00cf12c2aaaeab6cbd273bdd5b937e96796c7dd
[f00ebc4c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f00ebc4ccb3e82ae2d54787d9e39a6bce3044032
[f0146fc0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f0146fc0172a0f95718c22f531d43494740166f7
[f03803b0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f03803b0001f7d23989e566b7daba6d833e052a6
[f045423e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f045423e8fa16116b1188e5d76837964aed31a98
[f05b395b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f05b395b3006536c1221dff369c23644ed6a58b5
[f08dd9c4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f08dd9c4bdc950c70d380d0a98c9546d8efd8c00
[f0efa728]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f0efa728990d7aa09051b0c19fcfb8d8054ff71b
[f126a04c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f126a04c1b121b39d6cc2edee9a71fff3111a578
[f1717a9b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f1717a9bf4bcc57ea17bf6b6028aa7ce157b50f2
[f19bab1a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f19bab1a95a2d94a74e54c516b2f2b0b3becf5b4
[f1a7dd70]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f1a7dd70c8bf69a2c1dce07b50800ae51c8f59dc
[f1baa4d5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f1baa4d5f07e31c179c983a0b855cbc240903859
[f2199b30]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f2199b30ca34e9d46d1e51436b2cfba7c9b2f64c
[f22620fa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f22620fa54e1ae47344004a64b0c824ca5e31122
[f271285d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f271285d727e9aee05398bd101cf64628ff9e008
[f298fc98]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f298fc98145d97f8f299d6ecab71f3781ffc421e
[f2e5b4d1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f2e5b4d1afb28d371ada56f0f4d206e67fb9b644
[f31e2d99]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f31e2d992e5c880af2f79b7dc9a012a586ee6bd7
[f32d7650]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f32d7650d306ec4a5b0c6686dd7461f6323f6f12
[f355964d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f355964d7b4c6bcc0d5cd726df4ff360f2adac23
[f3584ab6]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f3584ab6ae798da30e09caa54fb48807a667b435
[f363682c]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f363682cfb202a8fcfaf591c0db5f6fbbd472fa1
[f3749f5b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f3749f5ba0323c8b5c685ff5bed0b63f472be3e9
[f381e929]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f381e929652ffb79a0577e0f3b894c815a126709
[f38900fe]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f38900fe4a05c02963f1406bc33975e385e5b5bb
[f3cf3672]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f3cf3672eaa4783a7d7de06b46d1b5a53af51d08
[f3f4f7d4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f3f4f7d4ac7cdbbcb6fa94f909a6e35163c145e8
[f3fc4fc3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f3fc4fc311a9ea0fd7a185b40710f0f22df3318c
[f4203933]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f42039339adc2bbb24d983232ba5c9f52cf03316
[f46be6a0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f46be6a032569b5726d4df69efc519cec1e8fb29
[f472247e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f472247e15a1e966ff59b285084eae3ba9a20b42
[f47f89b7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f47f89b779dedf06938e46274238be497611a549
[f49fb33d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f49fb33dab085714a8050d36442c04bf504f731e
[f4c8eccd]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f4c8eccdb06751cc1708fff9185528682220fd4c
[f4cdd62e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f4cdd62e98bd1edb356650f70f116c44927f9673
[f500cf73]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f500cf73361fe44cac8ae2a50983673ed70f14c7
[f54e5814]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f54e58140cc9c27cf1ea0cc811805e59fd53e8a9
[f56045aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f56045aa6c147246f30635240835e92bea224520
[f561976f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f561976f16b56d34bf2cb194f13e1bbcc30b2c5e
[f563b7aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f563b7aa0ebaf45d0b841553eb9ac8e3fe0782a5
[f58f506f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f58f506f17d6b76343d5bd814749259e3b380cc2
[f5903af7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5903af7a8560a93364eb8e67fea01c5477d0c73
[f5abc7a1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5abc7a12684e6ebf12721a64c95e76a7a620c6b
[f5b42761]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5b4276170f306b24baf664332b2ef1c5bc2b976
[f5bd4e3a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5bd4e3a8260d8bc5224c5cb851ac0dfe854ee7e
[f5bf771e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5bf771e6f26407fd2066f4765193adb250955c9
[f5c0bd9d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5c0bd9dc3d21263e7156771ae3d4bdce5b44c91
[f5d9f894]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5d9f894525051000b6779c4d8cc38675f45f37f
[f5f2be2d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f5f2be2dd7d45cf1cc4df2638b6ec3e98a0075b3
[f65650f0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f65650f0b47d22e0f7c41f3ddd1f3cae43cf708d
[f662d18a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f662d18a3165ba2e5a847bcc328b997cf2345a25
[f6a72ff1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f6a72ff1328766f733fe6314ecdbc1429bb57e61
[f6b28c8b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f6b28c8b819135e35facc425f739c2a0b7c20c4d
[f6b52fc2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f6b52fc20a8893ce30443bdd27f8da11108d0e17
[f6dd6e18]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f6dd6e184b72ebbb646e009c8bef1fa9bfd5c584
[f708e15e]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f708e15eab0ca601699461565b7a396f84394526
[f764c6fb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f764c6fbac33459a4fb5d12e320c8341a7aaafd0
[f76b06eb]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f76b06eb5191a825a61a42761164fffdaa0668d7
[f76f5165]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f76f51656d67c2eb4801ec824d8bcfc86db28d34
[f784a31a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f784a31a0ab6efe1eb7fb6650ae6cc1296f99609
[f7b4533f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f7b4533f180ccc94c27f8e42b9806199d147f5c1
[f7fc2fad]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f7fc2fada083c52f2f81df502e18034a28006b6c
[f810a846]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f810a8465f60d7a1de286f8f264eab9af5789455
[f8214de1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f8214de1d7c2f82bffc4e1881eaecd81acc53f5d
[f858a3df]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f858a3dfadbd074b8c519b1a178cd705fb703607
[f8657df1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f8657df13a889a2026bc63751eb85584ef4b60e7
[f8953e5a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f8953e5a92b7fb5f650d58803b5213636adbd920
[f8c1c7d2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f8c1c7d2337b89637d64ade1039c85dcd42b4153
[f8c42de5]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f8c42de5f2305276b6113a07359443820b9b2c6b
[f8f977d1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f8f977d1bde282c350758aa2ebcca56eaef81c4a
[f94d2a0d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f94d2a0dddd5aa8afe73eb06963af6c3b40e3b01
[f968b462]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f968b462d74e05c806bf5560f356799aa40b7104
[f9ceca94]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f9ceca94d49df4dffeef57e0d263a777b701adb0
[f9d1638d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/f9d1638d861792c53f4fa0fb656c890d857b6ead
[fa0e54b9]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fa0e54b950313bcf8b4c1f1a70a2a164dfce3614
[fa384bc1]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fa384bc1cd8375d40f59cb636568c9e3398cb0c2
[fa3bb518]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fa3bb5182eb2ff5c7dc61aacd7ca1ee206015725
[fa3e01aa]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fa3e01aa80430df6ba6a887c54bfc34edda65301
[fa5ff732]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fa5ff7329049623be8379968adf2946360a780cb
[fada0fc4]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fada0fc46b9d0516b4876a0d67593e43079084cd
[fb51c813]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fb51c8134c6b63c7a34bf211579813b6e1994fc9
[fb70a847]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fb70a8476ae1ee7601855aef632ef73344203268
[fb822555]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fb822555116d65cc3c7b43fd066e397b071f31de
[fbad9a40]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fbad9a407b4cc23aedc6cdd0f48fc2fb1880bad9
[fc176f0f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fc176f0f8db7c6efd8168ce2d13775eede1b9574
[fc2a8379]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fc2a8379ad2f848990c749418ebe4123cacbcf8b
[fc548235]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fc5482359615f1f1a0d83c4f34a1ca89834d38ff
[fc872faf]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fc872fafd9cd85588b332f0956f436b531822c72
[fc88b9f2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fc88b9f2d84e21be4be5831cb62554f0c52429f2
[fcf91c96]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fcf91c96ea0dd598594aec0fac23726426b4cd3b
[fcfc7294]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fcfc7294018d7e2e559b42be2f70fd1df853514f
[fd1a1da0]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fd1a1da02ab3378b0327d1ccaf19a08434f60baf
[fd429436]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fd4294360f772a330f81322b23130af86cc2bb49
[fd481296]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fd481296e3f6ef13ee36ec430ee7f036f17c0258
[fda30e59]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fda30e592981b402a192fe6f74ac36febdc946c8
[fda61f8f]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fda61f8ffc7ddd95556f4109b9e735cdde2c1b93
[fdc1899a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fdc1899ac00ddde0355f09a5c6aaf6d79a1aeec7
[fdc69b7a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fdc69b7a044478191f4a28504d399ce258516726
[fe33e542]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fe33e54259f22713ccfd44ae22ec8bce4f112d54
[fe422d64]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fe422d64df17d550cac10ae4306b02f5bf99964b
[fe51c9ab]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fe51c9ab1ca6f2619db9785ec1ee0584fd7fc10f
[fe5f8757]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/fe5f875720cd7d9043ffa71b662a2fd83f4e0443
[febb71c7]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/febb71c7463cfe0c66fbf2900168a48f10c2b9ed
[febfd00d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/febfd00d66ac8586584882ec6c7a5b2a97683571
[ff09253a]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ff09253a08731982cbda3835af2f736139db17ad
[ff0f4695]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ff0f46959254dd193a3b7abb63699ac58106e204
[ff2a4a40]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ff2a4a401d1576b6780dc79f3eb06c3c2b3ddc21
[ff2cd81b]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ff2cd81bbd533c59df2c8bac3c6ff2afea4c1048
[ff46a3f3]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ff46a3f34039a3b7b58cc862b9eb8aa403ef10c4
[ffbbfac2]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ffbbfac2c7959e3bfbb565bbd3c5637dca7a7c43
[ffe8d68d]: https://github.com/jtmoon79/super-speedy-syslog-searcher/commit/ffe8d68d1446f5e0fda5d05726e7d89de4f5c222
